# $Id: //prod/main/sarf_centos/tests/zeus1350/common_regression_tests/reporting/reporting_2.txt#8 $
...
Set Mail Flow Policies To TLS ${value}
    ...
    Mail Flow Policies Edit  ${PRIVATE_LISTENER.name}  default  ${policy_settings}
...
Verify TLS Reporting On SMA
    ...
    ...  ${failed_preferred}  ${unencrypted}  ${dane_success}  ${dane_failure}  ${total_connections}
...
SSLconfig To TLS
    ...
    SSHLibrary.Open Connection  ${ESA_IP}  prompt=>  timeout=30
    SSHLibrary.Login  ${DUT_ADMIN}  ${DUT_ADMIN_SSW_PASSWORD}
    SSHLibrary.Write  sslconfig
    SSHLibrary.Read Until Prompt
    SSHLibrary.Write  GUI
    SSHLibrary.Read Until Prompt
    SSHLibrary.Write  2,3
    SSHLibrary.Read Until Prompt
    SSHLibrary.Write Bare  \n
    SSHLibrary.Read Until Prompt
    SSHLibrary.Write  OUTBOUND
    SSHLibrary.Read Until Prompt
    SSHLibrary.Write  1,2,3,4
    FOR  ${i}  IN RANGE  1  3
       SSHLibrary.Read Until Prompt
       SSHLibrary.Write  \n
    END
...
Initialize Tvh663323c
    ...
    Set Mail Flow Policies To TLS Required
    Commit Changes
    SSLconfig To TLS
    Null Smtpd Start  tls-required=${True}
...
*** Test Cases ***
Tvh663106c
    [Documentation]  Verify that Incoming mail report can be generated in\n
    ...  language chosen from "User Preferences" menu
    ...  http://tims.cisco.com/warp.cmd?ent=Tvh663106c
    [Tags]  srts  Tvh663106c  wsrts
    [Setup]  Run Keywords
    ...  General Test Case Setup
    ...  Initialize Tvh662881c
    [Teardown]  General Test Case Teardown
    Set Test Variable  ${TEST_ID}  Tvh663106c
...
Tvh663629c
    [Documentation]  Verify user is able to view system capacity data in different time
    ...  ranges\n
    ...  http://tims.cisco.com/warp.cmd?ent=Tvh663629c
    [Tags]  Tvh663629c  srts  wsrts
    [Setup]  Run Keywords
    ...  General Test Case Setup
    ...  Initialize Email Reporting Group Test
    ...
    ...  Finalize Email Reporting Group Test
    ...  General Test Case Teardown
    Set Test Variable  ${TEST_ID}  Tvh663629c
...
Tvh664007c
    [Documentation]  Verify user is able to view DLP incident data in different time
    ...  ranges\n
    ...  http://tims.cisco.com/warp.cmd?ent=Tvh664007c
    [Tags]  Tvh664007c  srts  wsrts
    [Setup]  Run Keywords
    ...  General Test Case Setup
    ...  Initialize Tvh664007c
    [Teardown]  Run Keywords
    ...  Finalize Tvh664007c
    ...  General Test Case Teardown
    Set Test Variable  ${TEST_ID}  Tvh664007c
...
Tvh664192c
    [Documentation]  Verify user is able to view content filters data in different time
    ...  ranges\n
    ...  http://tims.cisco.com/warp.cmd?ent=Tvh664192c
    [Tags]  Tvh664192c  srts  wsrts
    [Setup]  Run Keywords
    ...  General Test Case Setup
    ...  Initialize Tvh664192c
    [Teardown]  Run Keywords
    ...  Finalize Tvh664192c
    ...  General Test Case Teardown
    Set Test Variable  ${TEST_ID}  Tvh664192c
    ...
    ...  Year
      ${expected_columns}=  Create List
      ...  Content Filter           ${Tvh664192c_FILTER_NAME}
      ...  Messages                 ${total_msgs_count}
      Verify Mail Reporting On SMA
      ...  table_name=Incoming Content Filter Matches
      ...  expected_columns=${expected_columns}
      ...  left_chart=Top Incoming Content Filter Matches
      ...  right_chart=Top Outgoing Content Filter Matches
      ...  time_period=${time_period}
    END
...
Tvh663946c
    [Documentation]  Verify user is able to view Virus Types Detail in different time
    ...  ranges\n
    ...  http://tims.cisco.com/warp.cmd?ent=Tvh663946c
    [Tags]  Tvh663946c  srts  wsrts
    [Setup]  Run Keywords
    ...  General Test Case Setup
    [Teardown]  Run Keywords
    ...  General Test Case Teardown
    Set Test Variable  ${TEST_ID}  Tvh663946c
    ...
    ...  Year
      ...
      ...  time_period=${time_period}
    END
    Email Reporting Check Chart Presence  Top Incoming Virus Types Detected

Tvh663346c
    [Documentation]  Verify user is able to view User Mail Flow Details
    ...  data in different time ranges\n
    ...  http://tims.cisco.com/warp.cmd?ent=Tvh663346c
    [Tags]  Tvh663346c  srts  wsrts
    [Setup]  Run Keywords
    ...  General Test Case Setup
    [Teardown]  Run Keywords
    ...  General Test Case Teardown
    Set Test Variable  ${TEST_ID}  Tvh663346c
    ...
    ...  Year
      ${expected_columns}=  Create List
      ...  Incoming Clean   ${total_msgs_count}
      Verify Mail Reporting On SMA
...
Tvh664306c
    [Documentation]  Verify user is able to view sender details data in different time
    ...  ranges\n
    ...  Verify user is able to view destinations data in different time ranges\n
    ...  http://tims.cisco.com/warp.cmd?ent=Tvh664306c
    ...  http://tims.cisco.com/warp.cmd?ent=Tvh662837c
    [Tags]  Tvh664306c  Tvh662837c  srts  wsrts
    [Setup]  Run Keywords
    ...  General Test Case Setup
    ...  Initialize Tvh664306c
    [Teardown]  Run Keywords
    ...  Finalize Outgoing Destinations Reporting Test Case
    ...  General Test Case Teardown
    Set Test Variable  ${TEST_ID}  Tvh664306c
    ...
    ...  Year
      ${expected_columns}=  Create List
      ...  Clean               ${CLEAN_MSGS_COUNT}
      ...  Total Messages      ${CLEAN_MSGS_COUNT}
      Verify Mail Reporting On SMA
...
Tvh663077c
    [Documentation]  Verify that domain based reports with "by email address"
    ...  option are preserved during upgrade\n
    ...  http://tims.cisco.com/warp.cmd?ent=Tvh663077c
    [Tags]  srts  Tvh663077c  wsrts
    [Setup]  Run Keywords
    ...  General Test Case Setup
    ...  Initialize Tvh663077c
    [Teardown]  Run Keywords
    ...  Finalize Tvh663077c
    ...  General Test Case Teardown
    Set Test Variable  ${TEST_ID}  Tvh663077c

    ${VERSION}=  Get DUT Build
    ...
    Page Should Contain  ${DOMAIN_BASED_REPORT_TITLE}
    Click Element  xpath=//a[contains(text(),'${DOMAIN_BASED_REPORT_TITLE}')]
    Page Should Contain Element
    ...  xpath=//input[@id='data_source_cg_address' and @checked='checked']

Tvh663514c
    [Documentation]  Verify that scheduled domain based report with
    ...  "by server" option are preserved during upgrade\n
    ...  http://tims.cisco.com/warp.cmd?ent=Tvh663514c
    [Tags]  srts  Tvh663514c  wsrts
    [Setup]  Run Keywords
    ...  General Test Case Setup
    ...  Initialize Tvh663514c
    [Teardown]  Run Keywords
    ...  Finalize Tvh663077c
    ...  General Test Case Teardown
    Set Test Variable  ${TEST_ID}  Tvh663514c
    ...
    Click Element  xpath=//a[contains(text(),'${DOMAIN_BASED_REPORT_TITLE}')]
    Page Should Contain Element
    ...  xpath=//input[@id='data_source_cg_server' and @checked='checked']

Tvh663509c
    [Documentation]  Verify that Total messages on "Outgoing Senders: IP Addresses" page display
    ...  the correct number of recipients processed in case when new data arrive\n
    ...  http://tims.cisco.com/view-entity.cmd?ent=663509
    [Tags]  Tvh663509c  srts  wsrts  1ESA1SMA
    [Setup]  Initialize Tvh663509c
    [Teardown]  Finalize Tvh663509c
    Set Test Variable  ${TEST_ID}  Tvh663509c

    ${msgs_count_1}=  Set Variable  10
    ${msgs_count_2}=  Set Variable  5
    ${log_pattern}=  Set Variable  Dropped by DLP

    FOR  ${sent_msgs_count}  ${total_msgs_count}  IN  ${msgs_count_1}  ${msgs_count_1}
    ...  ${msgs_count_2}  ${${msgs_count_1} + ${msgs_count_2}}
      Inject Messages
      ...  mbox-filename=${ZIP_ATTACHMENT_WITH_EXCELFILE}
      ...  num-msgs=${sent_msgs_count}
      ...  inject-host=${ESA_PRIVATE_LISTENER_IP}
      Switch TO ESA
      Verify And Wait For Log Records  ${log_pattern} >= ${total_msgs_count}
      Switch TO SMA
      Wait Until Keyword Succeeds
      ...  ${DATA_UPDATE_TIMEOUT}
      ...  ${RETRY_TIME}
...
Tvh663146c
    ...
    ...  for counter CONN_LAST_TLS_STATUS\n
    ...  http://tims.cisco.com/warp.cmd?ent=Tvh663146c
    [Tags]  srts  wsrts  Tvh663146c
    [Setup]  Run Keywords
    ...  General Test Case Setup
    ...  Initialize Tvh663323c
    [Teardown]  Run Keywords
    ...  Finalize Tvh663146c
    ...  General Test Case Teardown
    Set Test Variable  ${TEST_ID}  Tvh663146c

    Switch To SMA
    Verify TLS Reporting On SMA
    ...  Outgoing
    ...  successful_required=${CONNECTIONS}
    ...  successful_preferred=0
    ...  failed_required=0
    ...  failed_preferred=0
    ...  unencrypted=0
    ...  last_tls_status=Required - Success

    Navigate To  Email  Reporting  TLS Connections
    ${start_time}=  Get Time
    Click Element  xpath=//td[@id='ss_0_5_0-links']/span[2]  don't wait
    ${path}=  Wait Until Keyword Succeeds  10m  10s
    ...
    Log List  ${csv_data}
    List Should Contain Value  ${csv_data['Total TLS Connections']}  1
    Remove File  ${path}
