#!/usr/bin/bash
# $Id $
# $DateTime $
# $Author $
#===========================================================#
# Common script for running test suites from Jenkins
# Required environment variables for Jenkins node
# configuration that is going to use this script:
#  - WSA: hostname of appliance attached to the node
#  - SLICE_SERVER: hostname of the FreeBSD server attached
#                  to the node
# Required Arguments:
# 1. SUITE_NAME
# 2. DUT_VERSION
# 3. PREVIOUS_RUN_LINK
# 4. One or more test suits to run
#===========================================================#
# Get scripts directory
DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"

export SARF_HOME=$DIR/../..
export PYTHONPATH=$SARF_HOME/resources:$SARF_HOME/variables:$SARF_HOME/testlib:$PYTHONPATH

#Sync the code
PREV_DIR=`pwd`
. ~/.bash_profile
. ~/.sarf
cd $SARF_HOME
p4 sync
cd $PREV_DIR

# Enable or disable ipv6 based on parameter ${INET_MODE}
$SARF_HOME/bin/run/enable_disable_ipv6.sh $@

#clear lingering selenium process
ps auxww | grep -Ei "\-port|pybot_run" | awk '{print $2}' | xargs kill -9


#Make all .sh scripts to execute
chmod +x $SARF_HOME/bin/run/*.sh


#===========================================================#
# Global Variables Definition
#===========================================================#
export LD_LIBRARY_PATH=/usr/local/lib/firefox3/

# name of the test suite is taken from first argument
SUITE_NAME=$1
# remove suite name from argument list
shift

# directory where output files will be placed
SARF_OUTPUT_DIR=$HOME/public_html/$SUITE_NAME

# Hostname of DUT is taken from second argument
DUT=$WSA

# Version of AsyncOS on applinace is taken from third argument
DUT_VERSION=$1
# remove version from argument list
shift


#GET LAB name
LAB=`echo $DUT | cut -f2 -d"."`

if [ "$LAB" == "cs3" ]; then
    ENVIRONMENT_FILE_NAME="cs3_lab.py"
elif [ "$LAB" == "cs20" ]; then
    ENVIRONMENT_FILE_NAME="cs20_lab.py"
else
    ENVIRONMENT_FILE_NAME="ibauto_lab.py"
fi


#Get the customer choice from job configuration
if [ "$NETINSTALL_REQUIRED" == "NO" ]; then
        NETINST_RQD="FALSE"
else
        NETINST_RQD="TRUE"
fi


# Previous Run Link
if [[ $1 == http://* ]]
then
    echo "Preparing for re-run of failed test cases using $1"
    $SARF_HOME/bin/pybot_run --dut $DUT \
    -v link:$1 $SARF_HOME/tests/common/unittests/scp.txt
    python $SARF_HOME/tools/gather_failed_tests.py /tmp/output.xml /tmp/failed_tests.txt
    files="--argumentfile /tmp/failed_tests.txt"
    shift
fi

#===========================================================#
# Script Execution
#===========================================================#

# clean output directory
rm -rf $SARF_OUTPUT_DIR/*
# clean tmp directory
sudo rm -rf /var/tmp/customProfileDir*

# clean the sarf output directory
python $DIR/clean_sarf_output_folder.py $SARF_OUTPUT_DIR

# Clear the tmp files
$SARF_HOME/bin/run/clear_tmp.sh

# clean orphan processes
$DIR/delete_orphan_processes.sh

#Check and make slot offline it needed
pybot  --variable DUT:$DUT_1  \
       --variable HTTP_SERVER:$SLICE_SERVER  \
       --variable BUILD:$WSA_VERSION \
       --variable BUILD_URL:$JOB_URL  \
       --variable SARF_HOME:$SARF_HOME  \
       --variable NODE:$NODE_NAME  \
       --variable JENKINS_URL:$JENKINS_URL  \
       --variable JOB_NAME:$JOB_NAME  \
       --outputdir $SARF_OUTPUT_DIR  \
       --output check_pre_requisites.xml  \
       --report check_pre_requisites.report.html  \
       --log check_pre_requisites.log.html \
       --debugfile check_pre_requisites.debug  \
       $SARF_HOME/tests/common/check_pre_requisites.txt

if [[  $DUT_VERSION == coeus*  ]];
then
    META_DUT_VERSION=`echo $DUT_VERSION | tr "." "-"`
else
    META_DUT_VERSION=coeus-`echo $DUT_VERSION | tr "." "-"`
fi
# common arguments to both netinstall and test suite commands
common_args=(
	--dut $DUT
	--variable WSA_VERSION:$DUT_VERSION
	--outputdir $SARF_OUTPUT_DIR
	# Use new environment file so that the Tools server is not over consumed
	--variablefile $SARF_HOME/variables/environment/$ENVIRONMENT_FILE_NAME
	# Set the suite names as common, so that common arguments can be passed around.
	--output $SUITE_NAME.xml
	--report $SUITE_NAME.report.html
	--log $SUITE_NAME.log.html
	--debugfile $SUITE_NAME.debug
	--metadata dut_build:$META_DUT_VERSION
)

# We are not using WGA and Auto labs anymore. So this, mostly makes no sense.
# Let us save some time here.
# python $SARF_HOME/tools/wsa/BatWatchdog.py --dut=$DUT

# FIX FOR TEST COUNT MISMATCH #
echo "---------------------------------------------------------"
echo Dryrun to Update Dashboard to show NOT_RUN. Starting.
tests=""
for i in "$@"; do
    if [ -e $i ]
   then
        tests=$tests' '$i
    fi
done
echo Suites: $tests

echo Dryrun to Update Dashboard to show NOT_RUN. Starting.
pybot --dryrun --listener $SARF_HOME/tools/DynamicAutoTriage/DryrunListener.py \
    $tests  2>&1 > pybot-dryrun.log

echo Dryrun to Update Dashboard to show NOT_RUN. Done.
echo Check $SARF_HOME/pybot-dryrun.log if there are errors.
echo "---------------------------------------------------------"

# netinstall
flag=/tmp/netinstall_started
if [ "$NETINST_RQD" != 'FALSE' ]
then
    $SARF_HOME/bin/pybot_run \
    ${common_args[*]} \
    -v NETINST_FLAG:$flag \
    $SARF_HOME/tests/common/install_wsa_build_if_necessary.txt

fi
# Test Suite will be run only if netinstall passed
# If netinstall failed, recheck the DUT if we can bring it online.

if [  -e $flag ]
then

    #NetInstall has failed. Script does not exit from here.
    echo "ERROR | Netinstall failed. Suite $SUITE_NAME has not been run."
    exit

else
	echo "SUCCESS | Netinstall Completed. Starting the Tests."
	echo "=================================="

	echo Rename files to Install_wsa_build
	#Rename the existing log files back to NetInstall logs.
	mv $SARF_OUTPUT_DIR/$SUITE_NAME.xml $SARF_OUTPUT_DIR/install_wsa_build.xml
	mv $SARF_OUTPUT_DIR/$SUITE_NAME.report.html $SARF_OUTPUT_DIR/install_wsa_build.report.html
	mv $SARF_OUTPUT_DIR/$SUITE_NAME.log.html $SARF_OUTPUT_DIR/install_wsa_build.log.html
	mv $SARF_OUTPUT_DIR/$SUITE_NAME.debug $SARF_OUTPUT_DIR/install_wsa_build.debug

	echo Common Arguments: ${common_args}
	echo Starting tests...

	$SARF_HOME/bin/pybot_run \
	${common_args[*]} \
	--listener $SARF_HOME/tools/DynamicAutoTriage/Listener.py \
	--exclude skip* \
	$files \
	$@

	echo "=================================="
fi
# This was never used. So lets not run this. If needed, we will enable.
#$SARF_HOME/bin/pybot_run    ${common_args[*]}    -v collect_output:$SARF_OUTPUT_DIR/collect.tar.gz    tests/common/useful_scripts/collect_logs.txt

#Copy the Test Results from the Client to Jenkins.
# The test results copying to Jenkins has been taken care of by Jenkins Job itself. Avoiding duplication.
# If jenkins results publishing is failing, re-enable the following code.
#python $SARF_HOME/tools/copy_result_xml_to_jenkins.py $SARF_OUTPUT_DIR

echo "Copying App directory to sarf_output_directory"
BUILD_ID_NUMBER=`echo $META_DUT_VERSION | sed -E 's/coeus-//g'`
cp -rf $HOME/public_html/auto-triage/BATS/$BUILD_ID_NUMBER/$WSA/app_faults/appfault_info.txt $HOME/public_html/auto-triage/BATS/$BUILD_ID_NUMBER/$WSA/sys_cores/coredump_info.txt  $SARF_OUTPUT_DIR


# Disable ipv6 (call without parameters)
$SARF_HOME/bin/run/enable_disable_ipv6.sh
