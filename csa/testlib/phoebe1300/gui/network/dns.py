#!/usr/bin/env python
# $Id: //prod/main/sarf_centos/testlib/phoebe1300/gui/network/dns.py#1 $
...
DNS_DEL = lambda dns, index, td: 'xpath=//tr[@id="%s_row%s"]/td[%d]/img[1]' % (dns, index, td,)
GET_FIELD_REGEX = lambda dns: '%s\[(\d+)\]\[IP\]' % (dns,)
_err = lambda x: \
    "Invalid input value '%s' for 'DNS Servers' it should be either 'Local' or 'Root'" % x
...
class Dns(GuiCommon):
    ...
    def _dns_delete(self, dns='Local', dns_ip=None, alt_ip=None):
        ...
        - `dns`: 'Local' to delete DNS server(s) defined under 'Use these DNS Servers',
                 'Root' to delete DNS server(s) defined under 'Use the Internet's Root DNS Servers'
        ...
        - `alt_ip`: string of comma separated values of 1 or more alternate DNS server's IP address.
        """
        self._open_page()
        ...
        if dns_ip is not None:
            ...
            self._perform_delete_operation \
                (regex_dns=REGEX_USER_DNS, ips=dns_ip)
        if alt_ip is not None:
            if dns.lower() == 'root':
                ...
                self._perform_delete_operation \
                    (regex_dns=REGEX_USER_ALT, ips=alt_ip)
            else:
                raise ValueError(_err(dns))
        self._click_submit_button()

    def _perform_delete_all_operation(self, regex_dns=None, table_column=3):
        num_entry = int(
    ...
    def _add_alt_servers(self, alt_servers=None, dns=None, entry_index=None):
        ...
        if alt_servers is not None:
            ...
            if entry_index is None:
                entry_index = \
                    int(self.get_matching_xpath_count(DNS_TBODY_ROW(regex_alt)))
            for i, obj in enumerate(alt_servers):
                ...
                if entry_index == 1 and \
                        self.get_value \
                                    (DNS_TABLE_FIELD(regex_alt, 0, 'IP')).strip() == '':
                    entry_index = 0
                else:
                ...
                self.input_text \
                    (DNS_TABLE_FIELD(regex_alt, entry_index, domain_field),
                     domain_text)
        ...
        if dns_servers is not None:
            ...
            if dns.lower() == 'local':
                ...
                for i, obj in enumerate(dns_servers):
                    ...
                    if entry_index == 1 and \
                            self.get_value \
                                        (DNS_TABLE_FIELD(REGEX_USER_DNS, 0, 'IP')). \
                                    strip() == '':
                        entry_index = 0
                    else:
                        ...
                        self._info("Clicked 'Add Row' button")
                    self.input_text \
                        (DNS_TABLE_FIELD(REGEX_USER_DNS, entry_index, 'Pri'),
                         obj.priority)
                    self._info('Added Priority "%s"' % (obj.priority,))
                    self.input_text \
                        (DNS_TABLE_FIELD(REGEX_USER_DNS, entry_index, 'IP'), obj.ip)
                    self._info('Added IP "%s"' % (obj.ip,))
                    entry_index += 1
            else:
                raise ValueError(_err(dns))
    ...
    def _select_interface(self, interface=None):
        """
        - `interface`: Interface for DNS Traffic. Use 'Auto' or one of configured interfaces.
        Allows to use string that 'begins with',like
        'a001.d1' while the actual values is 'a001.d1.sharon.qa'.
        Case-insensitive.
        """
        interface_select = 'interface'
        _option_r = lambda f: 'regexpi:^%s.*' % f
        if interface is not None:
            try:
                self.select_from_list(interface_select, _option_r(interface))
                _selected = self._get_selected_label(interface_select)
                self._info \
                    ("Input was: '%s'. Selected Interface: %s" % (interface, _selected))
            except Exception:
                OPTIONS_NUM = int(self.get_matching_xpath_count \
                                      ("//*/select[@name='%s']/option" % interface_select))
                OPTIONS = \
                    [self.get_text("//*/select[@name='%s']/option[%s]" % \
                                   (interface_select, i)) for i in range(1, OPTIONS_NUM + 1)]
                self._warn \
                    ("Invalid option '%s' for 'Interface for DNS Traffic'. Select one from: %s" % \
                     (interface, ', '.join(OPTIONS)))
    ...
    def _set_wait_before_timeout(self, timeout=None):
        ...
        if timeout is not None:
            ...
            self._info \
                ("'Wait Before Timing out Reverse DNS Lookups': %s seconds" % timeout)
    ...
    def dns_add_alternate_server(self,
                                 dns='Local',
                                 dns_ip=None,
                                 domains=None,
                                 FQDN=None):
