#!/usr/bin/env python
# $Id: //prod/main/sarf_centos/testlib/phoebe1210/cli/ctor/licensesmart.py#1 $ $DateTime: 2019/05/07 03:16:10 $ $Author: bimmanue $
                    ...
                    if transport_connection.lower() == 'transport_gateway':
                        if gateway_url != None:
                            ...
                            try:
                                ...
                                self._debug(
                                    "Transport Settings Changed to gateway URL {} Successfully".format(gateway_url))
        ...
        for data in self.full_status_output:
            ...
            if match:
                ...
                if status_output_key == 'expires on':
                    self.smart_licensing_status['authorization expires on'] = self.smart_licensing_status.pop(
                        'expires on')
                if status_output_key == 'smart licensing is':
                    self.smart_licensing_status['smart licensing status'] = self.smart_licensing_status.pop(
                        'smart licensing is')
