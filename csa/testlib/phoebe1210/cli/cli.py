...
def get_cli(host, user, password, logger=None, model=None, transport='ssh'):
    if transport == 'ssh':
    ...
    sess.wait_for_prompt(timeout=60)
    sess.writeln('setttymode 0 1')
    sess.wait_for_prompt(timeout=60)

    return IronCli(sess, hostname=host, model=model)


class IronCli(object):
    def __init__(self, sess, hostname=None, model=None):
        self._sess = sess
    ...
    def __getattr__(self, name):
        ...
        if 1:
            ...
            #   self.setgateway.set_hostname(self._hostname)
            ...
            getattr(self, name).set_hostname(self._hostname)  # update hostname
