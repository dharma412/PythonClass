...
# $Id: //prod/main/sarf_centos/testlib/phoebe1210/gui/monitor/reports/reports_parser.py#1 $ $DateTime: 2019/05/07 03:16:10 $ $Author: bimmanue $
...
SYSTEM_STATUS_PAGE_PATH = ('Monitor', 'System Status')
RESET_COUNTERS_BUTTON = "//input[@value='Reset Counters']"

DELIVERY_STATUS_PAGE_PATH = ('Monitor', 'Delivery Status')
RETRY_DELIVERY_BUTTON = "//input[@value='Retry All Delivery']"


class ReportsParser(GuiCommon):
    """
    Parses reports tables under Monitor tab.
    ...
    * Allows to open needed item(click link) in the table and then fetch results.
    ...
    def get_keyword_names(self):
        return ['reports_set_random_data_mode',
                ...
                'reports_search',
                'reports_open_page',
                'reports_retry_all_delivery',
                'reports_counters_reset',
    ...
    def _open_page(self, path):
        if isinstance(path, basestring):
            ...
            raise ValueError('Expected path types are ' \
                             'string or tuple/list. {0} is given'.format(
                type(path)))
    ...
    def reports_open_item(self,
        ...
        *Examples*:
        | Reports Open Item |
        | ... | page=Monitor, Delivery Status |
        | ... | name=Outgoing Destinations Status |
        | ... | item=the.encryption.queue |
        | ${res}= | Reports Parse |
        | ... | name=Delivery Status Details |
        | Log List | ${res} |
        """
    ...
    def reports_parse(self,
        ...
        *Examples*:
        ...
        | ${res}= | Reports Parse |
        | ... | page=Monitor, Outgoing Destinations |
    ...
    def reports_open_page(self, page=None):
        ...
        *Examples*:
        | Reports Open Page | page=Monitor, System Status |
        """
        self._open_page(page)

    @set_speed(0)
    def reports_counters_reset(self, should_navigate=True):
        """
        Press 'Reset Counters' button.
        'Monitor > System Status' page.

        *Parameters*:
        - `should_navigate`: whether to navigate to 'Monitor > System Status'
        page prior to click the button. ${True} by default

        *Return*:
        Action result as String.

        *Examples*:
        | Reports Counters Reset |
        """
        if should_navigate:
            self._open_page(SYSTEM_STATUS_PAGE_PATH)
        self._wait_until_element_is_present(RESET_COUNTERS_BUTTON, timeout=60)
        self.click_button(RESET_COUNTERS_BUTTON)
        return '%s: %s' % self._check_action_result()

    @set_speed(0)
    def reports_retry_all_delivery(self, should_navigate=True):
        """
        Press 'Retry All Delivery' button.
        Like on 'Monitor > Delivery Status' page.

        *Parameters*:
        - `should_navigate`: whether to navigate to 'Monitor > Delivery Status'
        page prior to click the button. ${True} by default
        ...
        *Examples*:
        | Reports Retry All Delivery |
        """
        if should_navigate:
            self._open_page(DELIVERY_STATUS_PAGE_PATH)
        self.click_button(RETRY_DELIVERY_BUTTON)
        return '%s: %s' % self._check_action_result()
    ...
    def reports_export(self,
        ...
        *Examples*:
        ...
        if columns is not None:
            controller.set_options({'columns': columns})
        return controller.export_as(dest_path)
