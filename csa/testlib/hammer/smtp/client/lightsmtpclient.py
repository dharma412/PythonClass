...
from utils import MboxUtils, FatalOption

__version_info__ = ('1','0','017')
...
class LightWeightSpam():
    ...
    def __init__(self):
        ...
        self.recp_host_list = []
        self.message_list = []
        self.mbox_header_dict = OrderedDict()
        ...
        self.delay = 0
        self.tls_max_connection = 100.0
        self.cache_mbox = 1
        self.mbox_message = []
        self.tls_msg_count = 0
        self.custom_headers = OrderedDict()
        self.msg_type = ''
        self.msg_size = 512
        self.msg_subject = 'Test Mail lightsmtp client'
        self.journal_mail_to = None
        self.mbox_len = 0
    ...
    def get_message(self, args):
        ...
        #multiple mbox supported on mbox file option
        if args.mbox_file and self.cache_mbox:
            self.msg_type = 'mbox'
            self.mbox_file = True
            mbox_list = MboxUtils().check_multiple_mboxes(args.mbox_file)
            for each_mbox in mbox_list:
                messagelist.extend(self.get_mbox_messages(each_mbox))
            return messagelist

        if not self.cache_mbox:
            self.msg_type = 'mbox'
            mbox_list = MboxUtils().check_multiple_mboxes(args.mbox_file)
            self.mbox_len = len(mbox_list)
            for each_mbox in mbox_list:
                self.mbox = mailbox.mbox(each_mbox)
                self.mbox_message.append(itertools.cycle(self.mbox))

        if args.msg_body or self.msg_body:
            message = MIMEText(args.msg_body, 'plain')
            self.msg_type = 'mime-text'
        ...
        if args.custom_header:
            if self.msg_type not in ['mime-text']:
                for header_key, header_value in self.custom_headers.items():
                    message[header_key] = header_value

        if args.msg_size:
            msg_body = self._get_message_for_msg_size(int(args.msg_size))
        ...
        if not args.attachment_file and not args.msg_file and not args.mbox_file and not args.msg_body:
            messagelist.append(self._get_plain_message(args))
            return messagelist

        if args.msg_file:
            for emlfile in args.msg_file.split(','):
                MboxUtils().check_option(emlfile, "eml file: %s does not exist." % emlfile)
    ...
    def convert_journal_messages(self, msgid, sender, recipient, subject, message):

        if self.journal_mail_to:
            recipient = self.journal_mail_to
        return '\r\n'.join(['From karnayak@mar-esa.com Mon Jun 29 15:17:42 2020',
        'X-IronPort-RCPT-TO: %s' %recipient,
        'Received-SPF: Pass (esa1.hc152-28.nap5.ironport.com: domain of',
        '  karnayak@mar-esa.com designates 104.47.101.58 as permitted',
        '  sender) identity=mailfrom; client-ip=104.47.101.58;',
        '  receiver=esa1.hc152-28.nap5.ironport.com;',
        '  envelope-from="karnayak@mar-esa.com";',
        '  x-sender="karnayak@mar-esa.com"; x-conformance=spf_only;',
        '  x-record-type="v=spf1"; x-record-text="v=spf1',
        '  ip4:40.92.0.0/15 ip4:40.107.0.0/16 ip4:52.100.0.0/14',
        '  ip4:104.47.0.0/17 ip6:2a01:111:f400::/48',
        '  ip6:2a01:111:f403::/48 include:spfd.protection.outlook.com',
        '  -all"',
        'Authentication-Results: esa1.hc152-28.nap5.ironport.com; dkim=none (message not signed) header.i=none; spf=Pass smtp.mailfrom=karnayak@mar-esa.com; dmarc=fail (p=none dis=none) d=gmail.com',
        'IronPort-SDR: N1m9BIUi6oHjq9WDXtxWU+X1Q6hIqkJjfEhncxYKsAJpyU9Qm6h0/L9ptv4HwY5DbKulf2yYZS',
        ' hV2QJ4tPuOQjQYHMf2tRf+uUMZZn7Bp/WXofTv4xDi+bNleoNBBBd9LgnlVYacezpP8a+08YQ3',
        ' Av2+FjHZF61aCk/eBKly0kLFi487wyGWqQgGcshBp1DR7cGv6V934DEIU94r8S2q1iQXjGKkYw',
        ' Rg+ay43yvB+h6ypuYtX4mE2/ppad4q/Tku8E5XtMaBI1sBigkqoSpo2PgMAg3t7wQMwDNE8hD7',
        ' neD6Y2Pu+HjC3IKmJ1m9v2yz',
        'Received: from mail-bo1ind01lp2058.outbound.protection.outlook.com (HELO IND01-BO1-obe.outbound.protection.outlook.com) ([104.47.101.58])',
        '  by esa1.hc152-28.nap5.ironport.com with ESMTP/TLS/ECDHE-RSA-AES256-GCM-SHA384; 29 Jun 2020 15:17:41 +0500',
        'Content-Type: multipart/mixed;',
        '	boundary="_30953590-55eb-4883-903f-36163e907a7b_"',
        'Subject:Journalled email for subject: %s' %subject,
        'To: journalreceiver@journal.com',
        'From: journalsender@journal.com',
        'MIME-Version: 1.0',
        'Sender:',
        '	<MicrosoftExchange329e71ec88ae4615bbc36ab6ce41109e@maresa.onmicrosoft.com>',
        'Message-ID: %s' %make_msgid(),
        'Date: Mon, 29 Jun 2020 10:17:39 +0000',
        'X-MS-PublicTrafficType: Email',
        'X-MS-Journal-Report:',
        'X-MS-Exchange-Parent-Message-Id: %s' %msgid,
        'Auto-Submitted: auto-generated',
        'X-MS-Exchange-Generated-Message-Source: Journal Agent',
        'X-MS-TrafficTypeDiagnostic: BMXPR01MB2502:JournalingReport',
        'X-OriginatorOrg: maresa.onmicrosoft.com',
        'X-MS-Exchange-CrossTenant-AuthSource: BMXPR01MB2502.INDPRD01.PROD.OUTLOOK.COM',
        'X-MS-Exchange-CrossTenant-AuthAs: Internal',
        'X-MS-Exchange-CrossTenant-Network-Message-Id:',
        '	5e977504-5059-40c6-a85d-08d81c15a6f4',
        'X-MS-Exchange-CrossTenant-FromEntityHeader: Hosted',
        'X-MS-Exchange-Transport-CrossTenantHeadersStamped: BMXPR01MB2502',
        '\n--_30953590-55eb-4883-903f-36163e907a7b_',
    ...
    def add_msg_headers(self, msg_from,  msg_to, message):

        """ Add the msg headers ex: Subject: <>"""

        message_headers = []
        if self.custom_headers.has_key('To'):
            message_headers.append('%s: %s\r\n' %('To', self.custom_headers['To']))
            del self.custom_headers['To']
        for header_key, header_value in self.custom_headers.items():
            if header_key not in ['From']:
                message_headers.append('%s: %s\r\n' %(header_key, header_value))
        if self.custom_headers.has_key('From'):
            message_headers.append('%s: %s\r\n' %('From', self.custom_headers['From']))
        if self.msg_type in ['plain', 'mime-text']:
            message_headers.append('From: %s\r\n' % (msg_from))
            message_headers.append('To: %s\r\n' % (' '.join(msg_to)))
        if self.msg_type in ['plain']:
            message_headers.append('Subject: %s\r\n' % (self.msg_subject))
            message_headers.append('Date: %s \r\n' % (formatdate(localtime=True)))
            message_headers.append('Message-Id: %s\r\n' % (make_msgid()))
        if self.journal_mail_to:
            return self.convert_journal_messages(make_msgid(), msg_from, ' '.join(msg_to), self.msg_subject, message)
        else:
            return ''.join(message_headers)+message
    ...
    def build_mime_msg(self, mail_from=None, mail_to=None, subject=None, attach_file_list=None, msg_body=None):
        ...
        if attach_file_list:
            for filename in attach_file_list.split(','):
                ...
                if maintype == 'video':
                    with open(filename, 'rb') as attached_data:
                        msg = MIMEBase('application', 'octet-stream')
                        msg.set_payload(attached_data.read())
    ...
    def smtp_send_message(self, sender, recipient, sendmsg, servertype=None, printchar='.'):
        ...
        if self.duration:
            msg_sent_count = self.max_msg_per_connection
            self.client_send_message(sender, recipient, sendmsg, msg_sent_count, servertype=servertype,
                                     printchar=printchar)
        elif self.max_msg_per_connection and (self.max_msg_per_connection != self.message_count):
            while (self.total_message_count <= self.message_count):
                if (self.message_count - self.total_message_count) < self.max_msg_per_connection:
                    msg_sent_count = self.message_count - self.total_message_count
                else:
                    msg_sent_count = self.max_msg_per_connection
                self.client_send_message(sender, recipient, sendmsg, msg_sent_count, servertype=servertype,
                                         printchar=printchar)
        else:
            msg_sent_count = self.message_count
            self.client_send_message(sender, recipient, sendmsg, msg_sent_count, servertype=servertype, printchar=printchar)
        return

    def client_send_message(self, sender, recipient, sendmsg, msg_sent_count, servertype=None, printchar='.'):
        ...
        client.send_mail(sender, recipient, sendmsg, printchar, count=msg_sent_count,
                         dot=self.dotcount)
        client.disconnect()
    ...
    def execute_on_duration(self, queue_enabled=0):
        ...
        if self.duration:
            ...
            while ((time.time() - starttime) < self.duration):
                if self.address_list_file:
                    with open(self.address_list_file, 'r') as filedata:
                        for host_addr in filedata:
                            if self.rcpt_append_str:
                                host_addr = host_addr + self.rcpt_append_str
                            if self.mbox_message:
                                execute_status = self.execute(duration=self.duration, start=starttime, \
                                                              ...
                                                              sendmsg=str(self.mbox_message[random.randint(0, self.mbox_len-1)].next()))
                            else:
            ...
            if host_addr:
                for sender in self.mail_from_list:
                    ...
                    if (time.time() - start) > int(duration):
                        return 1
                    if queue_enabled:
                        if DURATION_QUEUE.full():
                            time.sleep(2)
                        if not DURATION_QUEUE.full():
                            if self.tls_max_connection < 100:
                        ...
                        sendmsg = self.add_msg_headers(sender, host_addr, sendmsg)
    ...
    def execute_default(self):
        ...
        if self.debug:
            print "MSG TYPE:%s" %(self.msg_type)
        for sender in self.mail_from_list:
            for sendmsg in self.message_list:
                if self.address_list_file:
                    recpt_multiple_list = []
                    with open(self.address_list_file, 'r') as filedata:
                        for host_addr in filedata:
                            recpt_multiple_list.append(host_addr.strip())
                    for eachrcpt in [x for x in range(len(recpt_multiple_list)) if x % self.addr_per_msg == 0]:
                        sendmsg = self.add_msg_headers(sender,
                                                       recpt_multiple_list[eachrcpt:eachrcpt + self.addr_per_msg],
                                                       sendmsg)
                        self.smtp_send_message(sender,
                                               recpt_multiple_list[eachrcpt:eachrcpt + self.addr_per_msg],
                                               sendmsg)
                else:
                    sendmsg = self.add_msg_headers(sender, self.recp_host_list, sendmsg)
    ...
    def process_option(self, args):
        ...
        if args.msg_size:
            self.msg_size = int(args.msg_size)

        if args.msg_subject:
            self.msg_subject = args.msg_subject
        ...
        if not args.num_msgs and not args.max_msg_per_connection:
        ...
        if not args.mail_from:
            if args.bounce_mailer:
                if str(args.bounce_mailer) == str(True):
                    self.mail_from_list.append('')
                else:
                    self.mail_from_list.append(args.bounce_mailer)
            else:
                self.mail_from_list.append('testuser%s@%s' % (random.randint(0, 1000), socket.gethostname()))
        ...
        if args.address_list:
            ...
            MboxUtils().check_option(addr_file, 'address list file: %s does not exist.' % addr_file)
            ...
            for eachrcpt in recp_list:
                if self.verify_mail_address(eachrcpt):
                    ...
                    if '.' in eachrcpt and '@' not in eachrcpt:
                        self.recp_host_list = ['user%d@%s' % (random.randint(0, 1000), eachrcpt)]

        if args.journal_mail_to:
            ## journal mail to will support only single recipients..
            if args.addr_per_msg:
                print "ERROR:--journal-mail-to and --addr-per-msg cant be used together.."
                exit()
            self.journal_mail_to = args.journal_mail_to
            if self.duration:
                self.address_list_file = MboxUtils().generate_journal_addresslist(args.journal_mail_to, self.duration)
            else:
                # disabling address list for num of msgs..
                self.address_list_file = None
            self.recp_host_list = [args.journal_mail_to]

        if args.addr_per_msg:
            addr_per_msg = int(args.addr_per_msg)
            self.addr_per_msg = addr_per_msg
            if not args.address_list and not args.rcpt_host_list:
                print 'ERROR:The --addr-per-msg option must be used in conjunction\nwith the --address-list/--rcpt-host-list option'
                exit()
            if args.address_list:
                ...
                if addr_per_msg <= len(self.recp_host_list):
                    ...
                    print "ERROR:addr_per_msg is needs to less than equal to recp_host_list/ address list entries"
                    exit()

        if args.tls_max_connection:
            if not args.duration:
                print "ERROR:tls_max_connection can be used along with duration"
                exit()
...
def initialize_work_queue(starttime, duration, dut, queue_target, username='admin', password='Cisco123$', prompt='> ',
                          queue_csv=None):
...
def monitor_work_queue(starttime, duration, session, queue_target, queue_csv):
    ...
    while ((time.time() - starttime) < duration):
        ...
        if msgcount < 0:
            ...
            msgcount = int(msgcount + (queue_target * 2.5 / 100.0))

        high_level = queue_target + (queue_target * 2.5 / 100.0)

        if (size - high_level) > 0:
            continue
...
def execute_concurrent_workqueue(options, duration, debug=False):
    ...
    print "Concurrent Connections::", options.concurrent
...
def process_duration_queue(starttime, duration):
    while ((time.time() - starttime) <= duration):
        if not DURATION_QUEUE.empty():
            ...
            send_msg = spam.add_msg_headers(sender, recipient, send_msg)
            spam.smtp_send_message(sender, recipient, send_msg, server_type, printchar)
    spam.message_list = []
    exit()
    return
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Light smtp client for spamming email / inject')
    ...
    parser.add_argument('--msg-size=', dest='msg_size', help='Size of each message to add on it')
    ...
    parser.add_argument('--journal-mail-to=', dest='journal_mail_to', \
                        help='journal-mail-to send mail with to the recipient only,\
                        will override the address-list/rcpt-host-list option. mail will delivered to this recipient only')

    parser.add_argument('--bounce-mailer=', dest='bounce_mailer', \
                        help='this option to send bounce mailer: user options: True/<usermailer>')
    ...
    if args.queue_target:
        ...
        BUFFER_SIZE = int(queue_target + (queue_target * 5 / 100))
        ...
        if args.concurrent:
            ...
            try:
                ...
                print "Pressed Ctrl+C. Sending mail stopped"
    print "\n"
    print "-"*60
    print "Total Messages Sent:%s" %(spam.total_message_count)
    print "MPS:%0.3f mps" %(spam.total_message_count/ (time.time() - starttime))
    print "Time taken:%0.3f s" % (time.time() - starttime)
    print "-" * 60
    exit(0)
