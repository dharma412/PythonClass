#!/usr/bin/env python
# $Id: //prod/main/sarf_centos/testlib/phoebe1350/cli/ctor/interfaceconfig.py#1 $ $DateTime: 2020/01/06 01:25:43 $ $Author: saurgup5 $
...
class interfaceconfig(clictorbase.IafCliConfiguratorBase):
    ...
    def _fill_common_input_params(self, param_map):
        param_map['configure_ipv4'] = ['like to configure an IPv4 address', DEFAULT]
        param_map['netmask'] = ['Netmask (Ex:', DEFAULT]
        param_map['configure_ipv6'] = ['like to configure an IPv6 address', DEFAULT]
        param_map['prefix_length']= ['Prefix length', DEFAULT]
        ...
        param_map['EUQ_HTTP'] = ['enable Spam Quarantine HTTP', DEFAULT]
        param_map['EUQ_HTTP_port'] = ['use for Spam Quarantine HTTP?', DEFAULT]
        param_map['EUQ_HTTPS'] = ['enable Spam Quarantine HTTPS', DEFAULT]
        param_map['EUQ_HTTPS_port'] = ['use for Spam Quarantine HTTPS?', DEFAULT]
        param_map['use_demo_cert'] = ['use a demo certificate', DEFAULT]
        param_map['cert_to_use'] = ['Please choose the certificate to apply',
                                    DEFAULT, 1]
        param_map['confirm_demo_use'] = \
            ['Do you really wish to use the "Demo" certificate', NO_DEFAULT]
        param_map['HTTP_redirect'] = ['should HTTP requests redirect', DEFAULT]
        param_map['EUQ_redirect'] = ['Spam Quarantine HTTP requests redirect',
                                    DEFAULT]
        param_map['use_def_ipas'] = ['default interface for your Spam Quarantine',
                                     DEFAULT]
        param_map['use_custom_base_url'] = ['custom base URL in your Spam Quarantine',
                                            DEFAULT]
        param_map['RSA_Manager'] = ['enable RSA Enterprise Manager Integration', DEFAULT]
        param_map['api_http'] = [' Do you want to enable AsyncOS API HTTP on this interface?', DEFAULT]
        param_map['api_http_port'] = ['use for AsyncOS API HTTP', DEFAULT]
        param_map['api_https'] = [' Do you want to enable AsyncOS API HTTPS on this interface?', DEFAULT]
        param_map['api_https_port'] = ['use for AsyncOS API HTTPS', DEFAULT]

    def new(self, input_dict=None, **kwargs):
        param_map = IafCliParamMap(end_of_command='Choose the operation')
        self._fill_common_input_params(param_map)
        param_map['if_name'] = ['enter a name', REQUIRED]
        param_map['address'] = ['IPv4 Address (Ex:', REQUIRED]
        param_map['ipv6_address'] = ['IPv6 Address (Ex:', REQUIRED]
        param_map['hostname'] = ['Hostname:', REQUIRED]
        param_map['custom_base_url'] = ['Enter the custom base URL',
                                        NO_DEFAULT]
        param_map['configure_helo'] = ['configure custom SMTP Helo to use in the SMTP conversation', DEFAULT]
        param_map['custom_helo'] = ['custom SMTP Helo to use in the SMTP conversation', DEFAULT]
    ...
    def edit(self, if_name, input_dict=None, **kwargs):
        ...
        param_map['address'] = ['IPv4 Address (Ex:', DEFAULT]
        param_map['ipv6_address'] = ['IPv6 Address (Ex:', DEFAULT]
        param_map['hostname'] = ['Hostname:', DEFAULT]
        param_map['custom_base_url'] = ['Enter the custom base URL', DEFAULT]
        param_map['change_confirm'] = ['want to change it?', DEFAULT]
        param_map['filter_confirm'] = ['one or more filters', DEFAULT]
        param_map['configure_helo'] = ['configure custom SMTP Helo to use in the SMTP conversation', DEFAULT]
        param_map['custom_helo'] = ['custom SMTP Helo to use in the SMTP conversation', DEFAULT]
        param_map['change_helo'] = ['change SMTP Helo used in the SMTP conversation', DEFAULT]
        param_map['action'] = ['interface hostname is used', DEFAULT, 1]
        param_map.update(input_dict or kwargs)
