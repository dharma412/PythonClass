...
# $Id: //prod/main/sarf_centos/testlib/phoebe1350/gui/monitor/reports/reports_parser_def/dashboard_table.py#1 $ $DateTime: 2020/01/06 01:25:43 $ $Author: saurgup5 $
...
class DashboardTable(BaseReportingTable):
    ...
    def _get_container_tables_mapping(self):
        ...
        for table_idx in xrange(1, 1 + tables_count_in_container):
            current_table_obj = self.gui._selenium.find_element_by_xpath(
                self.TABLE_HEADER_IN_CONTAINER_BY_IDX(table_idx))
            current_table_name = current_table_obj.text.strip()
    ...
    def _get_headers(self, table_name):
        ...
        for header_idx in xrange(1, 1 + headers_count):
            header_obj = self.gui._selenium.find_element_by_xpath(
                self.COL_HEADER_BY_IDX(header_idx,
                                       table_name))
            header_value = header_obj.text.strip()
    ...
    def _get_row_idx_by_cell_value(self, col_idx, cell_value, table_name):
        ...
        for row_idx in xrange(1, 1 + rows_count):
            cell_obj = self.gui._selenium.find_element_by_xpath(
                self.CELL_BY_COL_ROW(
                    col_idx, row_idx, table_name))
            current_cell_value = cell_obj.text.strip()
    ...
    def click_cell(self, row_coords, dest_col_name):
        ...
        dest_cell_locator = self.CELL_BY_COL_ROW(dest_col_idx, dest_row_idx,
                                                 table_name)
        if not self.gui._is_element_present('xpath=%s' % dest_cell_locator):
        ...
        dest_link_locator = self.LINK_BY_COL_ROW_IDX(dest_col_idx,
                                                     dest_row_idx,
                                                     table_name)
        element_text = self.gui._selenium.find_element_by_xpath(
            dest_cell_locator).text.strip()
        if self.gui._is_element_present('xpath=%s' % dest_link_locator):
            self.gui.click_element('xpath=%s' % dest_link_locator)
            ...
            if cell_value is None:
                cell_obj = self.gui._selenium.find_element_by_xpath(
                    self.CELL_BY_COL_ROW(col_idx, row_idx,
                                         table_name, use_any_cell))
                cell_value = cell_obj.text.strip()
            return cell_value
