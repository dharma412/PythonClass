#!/usr/bin/env python
# $Id: //prod/main/sarf_centos/testlib/phoebe1350/gui/manager/dlpmanager.py#1 $ $DateTime: 2020/01/06 01:25:43 $ $Author: saurgup5 $
...
POLICY_MATCHING_REGEX = "xpath=(//textarea/following-sibling::div[@class='text-tip']/preceding-sibling::textarea)[1]"
POLICY_MATCHING_WORD_PHRASE = "xpath=(//textarea/following-sibling::div[@class='text-tip']/preceding-sibling::textarea)[2]"
...
class Dlpmanager(GuiCommon):
    ...
    def dlp_policy_new(self,
        ...
        if matching_regex:
            self.input_text(POLICY_MATCHING_REGEX, matching_regex)
        if matching_word_phrase:
            self.input_text(POLICY_MATCHING_WORD_PHRASE, matching_word_phrase)
    ...
    def dlp_policy_configure_filter_message_tags(self,
        ...
        if apply_if:
            if apply_if.lower() == 'present':
                self.select_from_list_by_label(MSG_TAGS_APPLY_DD, "present")
            elif apply_if.lower() == 'absent':
                self.select_from_list_by_label(MSG_TAGS_APPLY_DD, "absent")
