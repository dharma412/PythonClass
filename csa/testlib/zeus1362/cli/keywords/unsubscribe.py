#!/usr/bin/env python
# $Id: //prod/main/sarf_centos/testlib/zeus1362/cli/keywords/unsubscribe.py#1 $ $DateTime: 2020/06/10 22:29:20 $ $Author: sarukakk $
...
class Unsubscribe(CliKeywordBase):
    ...
    def unsubscribe_print_options(self):
        """Print Global Unsubscribe options.

        Examples:
        | Unsubscribe print options |

        | ${unsubscribe_alerts}= | Unsubscribe Print Options |
        | Log | ${unsubscribe_alerts} |
        """

        output = self._cli.unsubscribe().print_options()
        self._info(output)
        return output