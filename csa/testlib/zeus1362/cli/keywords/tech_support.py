#!/usr/bin/env python
# $Id: //prod/main/sarf_centos/testlib/zeus1362/cli/keywords/tech_support.py#1 $ $DateTime: 2020/06/10 22:29:20 $ $Author: sarukakk $
...
class TechSupport(CliKeywordBase):
    ...
    def tech_support_ssh_access(self, tmp_password, confirm='yes'):
        ...
        * it must be different from the administrator's password.
        ...
        curr_status = self._cli.techsupport().sshaccess(tmp_password,
    ...
    def tech_support_disable(self, confirm='yes'):
        ...
        curr_status = self._cli.techsupport().disable(self._process_yes_no(confirm))
        self._info(curr_status)

    def tech_support_tunnel(self, tmp_password, port_num='443', confirm='yes', delay='5'):
        """Allow an Cisco IronPort customer service representative to remotely
        ...
        * it must be different from the administrator's password.
        ...
        curr_status = self._cli.techsupport().tunnel(tmp_password, port_num,
                self._process_yes_no(confirm), int(delay))
        self._info(curr_status)
