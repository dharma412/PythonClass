#!/usr/bin/env python
# $Id: //prod/main/sarf_centos/testlib/zeus1362/cli/keywords/licensesmart.py#1 $
...
class licensesmart(CliKeywordBase):
    """
    This CLI command is used to manage smartlicenses for the SMA
       ...
       - `register`: Register the SMA with CSSM
       - `Deregister` : Deregister the SMA
    ...
    def license_smart_register(self, *args):
        """
        Registers the SMA with CSSM. There is a force register option
        ...
        *Parameters:*
        - `token`: token Id to be used for SMA registration with CSSM
