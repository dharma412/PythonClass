#!/usr/bin/env python
# $Id: //prod/main/sarf_centos/testlib/zeus1250/cli/ctor/alertconfig.py#3 $ $DateTime: 2019/06/07 02:45:52 $ $Author: sarukakk $
...
class alertconfig(clictorbase.IafCliConfiguratorBase):
    ...
    def new(self, address=REQUIRED, alert_classes=DEFAULT,
                                   severity_levels=DEFAULT):
        self._query_response('NEW')
        self._query_response(address)
        self._expect('\n')
        raw = self._read_until('>')
        if string.find(raw, 'already configured') > 0:
            if alert_classes:
                self.newlines = 2
            else:
                raise ConfigError, """alertconfig: To edit %s, alert_classes
                                      parameter is required""" % address
        else:
            self.newlines = 1
        self._writeln(alert_classes)
    ...
    def edit(self, address=REQUIRED, alert_class=REQUIRED,
                                     severity_levels=DEFAULT):
       self.newlines = 2
    ...
    def setup(self, input_dict=None, **kwargs):
        ...
        param_map['auto_support'] = \
                                      ['enable Cisco AutoSupport', DEFAULT]
        param_map['reports'] = \
                                      ['weekly AutoSupport reports', DEFAULT]
        param_map['max_alerts'] = \
                                      ['Maximum number of alerts', DEFAULT]
