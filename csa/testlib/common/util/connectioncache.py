...
class ConnectionCache:
    ...
    def get_all_current(self):
        """ Return list of all registered connections"""
        return self._connections[:]

    def delete(self, index_or_alias):
        """
        Delete given connection from connection cache.
        """
        try:
            index = self._get_index(index_or_alias)
        except ValueError:
            raise ValueError("Non-existing index or alias '%s'" % index_or_alias)
        del self._connections[index - 1]
        if self._connections:
            self._current = self._connections[index - 2]
            self._current_index = index - 2
        else:
            self.current = self._no_current = _NoConnection(self._no_current_msg)
