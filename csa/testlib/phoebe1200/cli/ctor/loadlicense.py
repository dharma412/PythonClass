#!/usr/bin/python
# $Id: //prod/main/sarf_centos/testlib/phoebe1200/cli/ctor/loadlicense.py#1 $
...
class loadlicense(IafCliConfiguratorBase):
            ...
            try:
                ...
                if self._expectindex != 0:
                    if buf.find('Do you accept the above license') != -1:
                        self._query_response('Y', timeout=90)
    ...
    def _load_license_from_cli(self, paste_conf):
        ...
        while 1:
            try:
                buf = self._read_until('agreement? []', timeout=5)
                if buf.find('Do you accept the above license') != -1:
                    self._query_response('Y', timeout=60)
                    license_info = self._wait_for_prompt()
                    return re.search(RESULT_SEARCH_PATTERN,
                                     license_info, re.DOTALL).group(0)
            except TimeoutError:
                break
