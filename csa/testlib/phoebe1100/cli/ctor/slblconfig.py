#!/usr/bin/env python
# $Id: //prod/main/sarf_centos/testlib/phoebe1100/cli/ctor/slblconfig.py#1 $
...
class slblconfig(clictorbase.IafCliConfiguratorBase):
    ...
    def Import(self, filename=DEFAULT, ignore=DEFAULT, timeout=40):
        self._query_response('IMPORT')
        self._query_select_list_item(filename)
        self._query_response(ignore)
        result = self._read_until('Choose the operation', timeout=timeout)
