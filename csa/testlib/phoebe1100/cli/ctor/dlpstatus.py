#!/usr/bin/env python
# $Id: //prod/main/sarf_centos/testlib/phoebe1100/cli/ctor/dlpstatus.py#1 $ $DateTime: 2019/03/22 01:36:06 $ $Author: aminath $
...
class dlpstatus(ccb.IafCliConfiguratorBase):
    ...
    def __call__(self):
        ...
        if lines.find(pat) >= 1:
            return pat
        mo = re.match('.*DLP Engine[\s+](?P<version>[\s\d\.]+)[\s+](?P<last_updated>[\w\d\s]+)[\'\r\n|\n\']+', lines,
                      re.DOTALL)
        res = CfgHolder()
