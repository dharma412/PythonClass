...
# $Id: //prod/main/sarf_centos/testlib/phoebe1100/gui/monitor/reports/reports_parser_def/summary_table.py#1 $
...
class SummaryTable(BaseReportingTable):
    ...
    def get_data(self, is_full_text_mode):
        ...
        def get_cell_value(col_idx, row_idx, use_any_cell=False):
            ...
            if cell_value is None:
                cell_locator = self.CELL_BY_COL_ROW(col_idx, row_idx, use_any_cell)
                if self.gui._is_element_present(cell_locator):
                    cell_obj = self.gui._selenium.find_element_by_xpath(cell_locator)
        ...
        for row_idx in xrange(1, 1 + rows_count):
            ...
            if items_count_in_row == headers_count_in_row:
                ...
                header = self.gui._selenium.find_element_by_xpath(
                    self.HEADERS_IN_ROW(row_idx)).text.strip()
    ...
    def click_cell(self, row_name):
        ...
        for col_idx in xrange(1, 1 + items_count):
            ...
            cell_value = self.gui._selenium.find_element_by_xpath(
                cell_locator).text.strip()
