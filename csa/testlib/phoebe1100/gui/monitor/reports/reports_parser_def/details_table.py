# $Id: //prod/main/sarf_centos/testlib/phoebe1100/gui/monitor/reports/reports_parser_def/details_table.py#1 $
...
COLUMN_CHECKBOX_BY_IDX = lambda idx: \
    "({0}//input[@name='table_options_columns[]'])" \
...
class DetailsTable(BaseReportingTable):
    ...
    def get_available_table_names(cls):
        return ['User Mail Flow Details', 'Incoming Mail Details',
                ...
                'Past Year Virus Outbreaks', 'Incoming Malware Threat Files',
        ...
        if columns_to_show.lower() == 'all':
            for cb_idx in xrange(1, 1 + checkboxes_count):
                self.gui._select_checkbox(
                    'xpath=%s' % COLUMN_CHECKBOX_BY_IDX(cb_idx),
                    modify_locator=False)
        else:
            ...
            for column_name in column_names:
                self.gui._select_checkbox(
                    'xpath=%s' % COLUMN_CHECKBOX_BY_NAME(column_name),
                    modify_locator=False)
