#!/usr/bin/env python
# $Id: //prod/main/sarf_centos/testlib/zeus1350/cli/keywords/diagnostic.py#1 $ $DateTime: 2019/09/18 01:46:35 $ $Author: sarukakk $
...
class Diagnostic(CliKeywordBase):
    ...
    def diagnostic_network_smtpping(self, hostname_or_ip='aol.com', network_interface="1",
        ...
        curr_status = self._cli.diagnostic().network().smtpping(hostname_or_ip, network_interface,
                                                                select_mx_host, mx_host, send_email, from_email_id,
                                                                to_email_id, subject_message, message_body)
        self._info(curr_status)
    ...
    def diagnostic_network_tcpdump_setup(self, max_size='', stop='yes', interface='',
                                         operation='', port='CLEAR', client_ip='CLEAR', server_ip='CLLEAR',
                                         custom_filter=''):
        ...
        curr_status = self._cli.diagnostic().network().tcpdump().setup(max_size,
                                                                       self._process_yes_no(stop), interface, operation,
                                                                       port, client_ip,
    ...
    def diagnostic_reload(self, confirm='no'):
        ...
        curr_status = self._cli.diagnostic().reload(
            self._process_yes_no(confirm))
        self._info(curr_status)
