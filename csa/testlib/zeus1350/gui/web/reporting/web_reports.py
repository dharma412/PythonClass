#!/usr/bin/env python
# $Id: //prod/main/sarf_centos/testlib/zeus1350/gui/web/reporting/web_reports.py#1 $ $DateTime: 2019/09/18 01:46:35 $ $Author: sarukakk $
...
class WebReporting(ReportsBaseZeus835):
    ...
    def web_reporting_check_table(self, section_name, expected_values, data_in_first_column=True, number_precision=None,
                                  time_precision=None):
        """
    ...
    def web_reporting_move_column(self, section_name, column_name, target_column):
        ...
        if int(self.get_matching_xpath_count(_first_column_loc)) < 1:
            raise guiexceptions.GuiValueError(
                "Column '%s' was not found in section '%s'." % (column_name, section_name))
        ...
        if int(self.get_matching_xpath_count(_second_column_loc)) < 1:
            raise guiexceptions.GuiValueError(
                "Column '%s' was not found in section '%s'." % (target_column, section_name))
