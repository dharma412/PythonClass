# $Id: //prod/main/sarf_centos/testlib/zeus1350/gui/web/cm120/cm120_custom_url_categories.py#1 $
# $DateTime: 2019/09/18 01:46:35 $
# $Id: //prod/main/sarf_centos/testlib/zeus1350/gui/web/cm120/cm120_custom_url_categories.py#1 $ $DateTime: 2019/09/18 01:46:35 $ $Author: sarukakk $

from coeus1201.gui.manager.custom_url_categories import CustomUrlCategories
import common.gui.guiexceptions as guiexceptions
from common.gui.guicommon import GuiCommon
import time


class Cm110CustomUrlCategories(CustomUrlCategories):
    """
    Keywords library for WebUI page  Web -> Configuration Master 12.0 -> Custom URL Categories
    """

    table_id = "//table[@class='cols']"
    delete_column = 5
    name_column = 2

    def _open_page(self):
        self._navigate_to('Web', 'Configuration Master 12.0', 'Custom and External URL Categories')

    def get_keyword_names(self):
        return [
            'cm120_custom_url_categories_add',
            'cm120_custom_url_categories_delete',
            'cm120_custom_url_categories_edit',
            'cm120_custom_url_categories_get_list',
        ]

    def cm120_custom_url_categories_get_list(self):
        """
        Returns: list of custom url categories

        Examples:

        | ${policies}= | CM120 Custom URL Category Get List |
        | Should Be True | len(${policies}) == 2 |
        | Should Be True | 'category1' in ${policies} |
        """
        self._open_page()
        return self._get_policies().keys()

    def cm120_custom_url_categories_add(self,
                                        name,
                                        sites=None,
                                        order=1,
                                        regexes=None,
                                        feed_type="local",
                                        ext_feed_type="cisco_feed",
                                        ext_feed_location=None,
                                        ext_feed_auto_update=None):
        """Adds the custom URL category

        *Parameters*
        - `name`: name of the custom URL category. Mandatory.
        - `order`: processing order. Optional, '1' by default.
        - `sites`: partial urls or IP addresses the category should match.
             A string with comma separated values. Define either a URL
             or regular expression. At least one of these
             fields  must contain a value.
        - `regexes`: regular expressions for the urls the category
             should m atch. A string with comma separated values. Define either a URL
             or regular expression. At least one of these
             fields must contain a value.

        *Example*
        | CM120 Custom Url Categories Add | nameOfCategory | site1.com, site2.com | order=1 |
        | CM120 Custom Url Categories Add | nameOfCategory | order=1 | regexes=regex1, regex2 |
        """
        if feed_type == "external" and ext_feed_type == "office_365_service":
            ext_feed_location = "https://endpoints.office.com/endpoints/china?ServiceAreas=skype&TenantName=Cisco"
        self.custom_url_category_add(
            name,
            sites=sites,
            order=order,
            regexes=regexes,
            feed_type=feed_type,
            ext_feed_type=ext_feed_type,
            ext_feed_location=ext_feed_location,
            ext_feed_auto_update=ext_feed_auto_update
        )

    def cm120_custom_url_categories_edit(self,
                                         name,
                                         sites=None,
                                         order=None,
                                         regexes=None):

        """Updates the custom URL category

        *Parameters*
        - `name`: name of the custom URL category. Mandatory.
        - `order`: processing order. Optional, '1' by default.
        - `sites`: partial urls or IP addresses the category should match.
             A string with comma separated values. Define either a URL
             or regular expression. At least one of these
             fields must contain a value.
        - `regexes`: regular expressions for the urls the category
             should match. A string with comma separated values. Define either a URL
             or regular expression. At least one of these
             fields must contain a value.

        *Exceptions*
        - GuiControlNotFoundError:xxx Custom URL Categories

        *Examples*
        | CM120 Custom Url Categories Edit | nameOfCategory | site1.com, site2.com | order=1 |
        | CM120 Custom Url Categories Edit | nameOfCategory | order=1 | regexes=regex1 |
        """
        self.custom_url_category_edit(
            name,
            sites=sites,
            order=order,
            regexes=regexes
        )

    def _get_table_row_index(self, name):
        table_rows = self.get_matching_xpath_count('%s//tr' % \
                                                   (self.table_id,))
        print "table_rows:", table_rows
        for i in xrange(2, int(table_rows) + 1):
            policy_name = self.get_text('%s//tr[%s]//td[%s]' % \
                                        (self.table_id, i, self.name_column)).split(' \n')[0]
            if policy_name == name:
                return i
        return None

    def _click_delete_link(self, name):
        row = self._get_table_row_index(name)
        print  "row:", row
        if row is None:
            raise guiexceptions.GuiControlNotFoundError(
                'Custom URL category "%s" missing' % (name,),
                'Custom URL categories')
        cell_id = 'xpath=%s//tr[%s]//td[%s]//img' % \
                  (self.table_id, row, self.delete_column)
        self.click_element(cell_id, "don't wait")
        self.click_button("xpath=//button[text()='Delete']")

    def cm120_custom_url_categories_delete(self, name):
        """Deletes the custom URL category

        *Parameters*
        - `name`: The name of the edited policy. String. Mandatory.

        *Exceptions*
        - GuiControlNotFoundError:Custom URL category "xxx" missing

        *Examples*
        | CM120 Custom Url Categories Delete | nameOfCategory |
        | CM120 Custom Url Categories Delete | Category1 |
        """
        # self.custom_url_category_delete(name)
        self._open_page()
        self._click_delete_link(name)
