# $Id: //prod/main/sarf_centos/testlib/zeus1350/gui/web/cm75/cm75_identities.py#1 $
# $DateTime: 2019/09/18 01:46:35 $
# $Author: sarukakk $

from coeus75.gui.manager.identities import Identities


class Cm75Identities(Identities):
    """
    Keywords for Web -> Configuration Master 7.5 -> Identities
    """

    def _open_page(self):
        self._navigate_to('Web', 'Configuration Master 7.5', 'Identities')

    def get_keyword_names(self):
        return [
            'cm75_identities_add_policy',
            'cm75_identities_edit_policy',
            'cm75_identities_delete_policy',
            'cm75_identities_enable_policy',
            'cm75_identities_disable_policy',
            'cm75_identities_is_enabled_policy',
            'cm75_identities_get_list',
        ]

    def cm75_identities_get_list(self):
        """
        Returns: dictionary of identities from Configuration Master 7.5.
        ...
        | ${policies}= | CM75 Identities Get List |
    ...
    def cm75_identities_add_policy(self,
                                   name,
                                   description=None,
                                   order=None,
                                   subnet=None,
        ...
        """Adds new identity policy from Configuration Master 7.5
        ...
        - `subnet`: a string of comma separated or a list of subnet where
             identity is member.
        - `protocol`: a protocol which identity is a member.  Either 'all',
             'http', or 'ftp'.
        ...
        Exceptions:
        - ValueError: Invalid protocol selection: "xxx". Valid are:['all', 'http', 'ftp']
        ...
        - ValueError: Invalid authentication schemes: xxx.  Valid are: ['Basic', 'Basic or NTLMSSP', 'NTLMSSP']
        - ValueError: Invalid action for transparent user identification failure: xxx.  Valid are:['support', 'enforce']

        Examples:
        | CM75 Identities Add Policy | subnetID | description=This is a subnet identity | subnet=1.2.3.0/24 |
        | CM75 Identities Add Policy | protocolID | description=This is a protocol identity | protocol=http |
        | CM75 Identities Add Policy | ldapID | description=This identity required LDAP authentication | auth_method=requires |
        | | auth_realm=myLdapRealm |
        | CM75 Identities Add Policy | ntlmID | description=This identity required NTLM authentication |
        | |  auth_method=requires | auth_realm=myNtlmRealm | auth_scheme=Basic or NTLMSSP |
        | CM75 Identities Add Policy | advancedID | description=This identity has advanced options | url_categories=${webcats.ADULT}, ${webcats.ARTS} |
    ...
    def cm75_identities_edit_policy(self,
                                    name,
                                    new_name=None,
                                    description=None,
                                    order=None,
        ...
        """Edits existing identity policy from Configuration Master 7.5
        ...
        Example:
        | CM75 Identities Edit Policy | subnetID | new_name=New Name | subnet=1.2.3.0/24 |
        """
    ...
    def cm75_identities_delete_policy(self, name):
        """Deletes existing identity policy from Configuration Master 7.5

        Parameters:
        - `name`: name of identity to be deleted.

        Example:
        | CM75 Identities Delete Policy | myID |
        """
        self.identities_delete_policy(name)

    def cm75_identities_enable_policy(self, name):
        """Enables existing identity policy from Configuration Master 7.5

        Parameters:
        - `name` name of identity to enable

        Example:
        | CM75 Identities Disable Policy | myID |
        """

        self.identities_enable_policy(name)

    def cm75_identities_disable_policy(self, name):
        """Disables existing identity policy from Configuration Master 7.5
    ...
    def cm75_identities_is_enabled_policy(self, name):
        ...
        Examples:
            | ${enabled}= | CM75 Identities Is Enabled Policy | myID |
        """
