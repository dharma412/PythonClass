...
SELECT_APPLAINCE = lambda \
    row: "//div[@id='datatable_container']/div[3]/table//tbody[2]/tr[%s]/td[1]/div/input[@name='wsa[]']" % (row)
DOWNLOAD_INSTALL = "xpath=//input[@id='downloadinstall']"
CLICK_NEXT = "xpath=//input[@id='fetch_upgrade_button']"
SELECT_VERSION = lambda \
    row: "//div[@id='datatable_container']/div[3]/table//tbody[2]/tr[%s]/td[1]/div/input[@id='available']" % (row)
...
DELETE_ICON = lambda version: "//td[5][contains(text(), '%s')]/span/img[contains(@onclick,'doConfirmDelete')]" % (
    version)
...
class CentralizedUpgrade(GuiCommon):
    ...
    def centralized_upgrade_download_and_install(self, applainces, appliances_ip, version=None, mask_password=True,
                                                 email=''):
        self._open_page()
        self.click_button(UPGRADE_APPLIANCE, 'dont wait')
