#!/usr/bin/env python
# $Id: //prod/main/sarf_centos/testlib/zeus1350/gui/management/administration/ssw.py#1 $
...
NEXT_STEPS_PATH = lambda protocol, hostname: '%s://%s/system_administration/system_setup/euqssw_next_steps' % (
protocol, hostname)
...
class SystemSetupWizard(GuiCommon):
    ...
    def _process_timezone_settings(self, region, country, timezone):
        ...
        self.select_from_list(region_option_combo, "label=%s" % region)
        self.select_from_list(country_option_combo, "label=%s" % country)
        self.select_from_list(timezone_option_combo, "label=glob:%s*" % timezone)
