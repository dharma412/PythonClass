# $Id: //prod/main/sarf_centos/testlib/zeus1350/gui/management/administration/users.py#1 $
...
ENTRY_USER_TABLE_LINK_EXTUSER = lambda table_name, row, col: '//%s/table[@class=\'pairs\']/tbody/tr[%s]/td[%d]' % (
table_name, row, col,)
ENTRY_USER_NAME_LINK_EXTUSER = lambda table_name, row: '//%s/table[@class=\'pairs\']/tbody/tr[%s]/th[1]/text()' % (
table_name, row,)
ENTRY_USER_VALUE_LINK_EXTUSER = lambda table_name, row: '//%s/table[@class=\'pairs\']/tbody/tr[%s]/td[1]/text()' % (
table_name, row,)
...
ENTRY_USER_NAME_LINK = lambda table_name, row: '//%s/table[%s]/tbody/tr/th[1]/text()' % (table_name, row,)
ENTRY_USER_VALUE_LINK = lambda table_name, row: '//%s/table[%s]/tbody/tr/td[1]/text()' % (table_name, row,)
...
class Users(GuiCommon):
    ...
    def users_edit_external_authentication(self, auth_type, radius_servers=None,
        ...
        try:
            ...
            self._click_continue_button(check_result=False)
        finally:
    ...
    def users_get_external_authentication(self):
        ...
        *Examples:*
        ...
        self._info('users_get_list')
        self._open_page()
        if self._is_text_present('External Authentication is disabled.'):
            return {}
        else:
