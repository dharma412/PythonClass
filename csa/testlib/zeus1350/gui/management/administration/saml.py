# $Id: //prod/main/sarf_centos/testlib/zeus1350/gui/management/administration/saml.py#1 $
...
CUSTOMER_SP_TABLE = '//table[@id="user_sp_profile"]'
CUSTOMER_SP_EDIT_LINK = lambda name: "%s//a[normalize-space()='%s']" % (CUSTOMER_SP_TABLE, name)
CUSTOMER_SP_DELETE_BUTTON = lambda name: "%s//td[.//a[normalize-space()='%s']]" \
                                         "/following-sibling::td/img[@title='Delete...']" % (CUSTOMER_SP_TABLE, name)
CUSTOMER_IDP_TABLE = '//table[@id="user_idp_profile"]'
CUSTOMER_IDP_EDIT_LINK = lambda name: "%s//a[normalize-space()='%s']" % (CUSTOMER_IDP_TABLE, name)
CUSTOMER_IDP_DELETE_BUTTON = lambda name: "%s//td[.//a[normalize-space()='%s']]" \
                                          "/following-sibling::td/img[@title='Delete...']" % (CUSTOMER_IDP_TABLE, name)
...
DEVOPS_SP_TABLE = '//table[@id="devops_sp_profile"]'
DEVOPS_SP_EDIT_LINK = lambda name: "%s//a[normalize-space()='%s']" % (DEVOPS_SP_TABLE, name)
DEVOPS_IDP_TABLE = '//table[@id="devops_idp_profile"]'
DEVOPS_IDP_EDIT_LINK = lambda name: "%s//a[normalize-space()='%s']" % (DEVOPS_IDP_TABLE, name)
DEVOPS_SP_DELETE_BUTTON = lambda name: "%s//td[.//a[normalize-space()='%s']]" \
                                       "/following-sibling::td/img[@title='Delete...']" % (DEVOPS_SP_TABLE, name)
DEVOPS_IDP_DELETE_BUTTON = lambda name: "%s//td[.//a[normalize-space()='%s']]" \
                                        "/following-sibling::td/img[@title='Delete...']" % (DEVOPS_IDP_TABLE, name)
...
class Saml(GuiCommon):
    ...
    def saml_edit_sp_and_idp(self, sp_name, idp_name, settings={}):
        ...
        *Examples:*
        ...
        if local_settings.has_key('User Role'):
            ...
            user_role = 'admin'
        self._check_saml_locator_and_click(user_role, CUSTOMER_SP_EDIT_LINK(sp_name), DEVOPS_SP_EDIT_LINK(sp_name), \
        ...
        self._saml_add_sp(sp_name, local_settings)
        self._check_saml_locator_and_click(user_role, CUSTOMER_IDP_EDIT_LINK(idp_name), \
                                           DEVOPS_IDP_EDIT_LINK(idp_name), \
                                           'Service Provider or Identity Provider profile named "%s" does not exist or it is \
        ...
        *Example:*
        ...
        if sp_name:
            self._check_saml_locator_and_click(user_role.lower(),
                                               CUSTOMER_SP_DELETE_BUTTON(sp_name), \
                                               DEVOPS_SP_DELETE_BUTTON(sp_name), \
        ...
        if idp_name:
            self._check_saml_locator_and_click(user_role.lower(),
                                               CUSTOMER_IDP_DELETE_BUTTON(idp_name), \
                                               DEVOPS_IDP_DELETE_BUTTON(idp_name), \
                                               'Service Proivder or Identity provider profile named "%s" does not exist \
    ...
    @set_speed(0)
    def _saml_get_table_details(self, table):
        """
        ...
        """
        result = {}
        KEYS_TABLE = "//*[@id='%s']" % (table)
        if not self._is_element_present(KEYS_TABLE):
            raise guiexceptions.GuiValueError('Table not found..')
        SP_HEADER_NAMES = "%s/tbody/tr[2]/th" % (KEYS_TABLE,)
        CELL_BY_IDX = lambda row_idx, col_idx: "%s/tbody/tr[%d]/td[%d]" % (KEYS_TABLE, row_idx, col_idx)
        row_index = [3]
