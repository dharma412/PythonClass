#!/usr/bin/env python
# $Id: //prod/main/sarf_centos/testlib/zeus1350/gui/management/services/system_status.py#1 $ $DateTime: 2019/09/18 01:46:35 $ $Author: sarukakk $
...
class SystemStatus(GuiCommon):
        ...
        for k1 in exp_dict.keys():
            if k1 in found_dict:
                ...
                if (exp_value == '*'):
                    ...
                    message += "\r\n'%s' was '%s' instead of expected '%s'%s." % (
                    k1, fnd_value, exp_value, section_name)
            else:
    ...
    def _find_table(self, section_path):
        ...
        if tables_count < 1:
            raise guiexceptions.GuiControlNotFoundError("Table was not found in '%s' section." % section_path,
                                                        "System Status")
        return table_loc
    ...
    def _get_section_values(self, rows_loc):
        ...
        for row_num in range(1, rows_count + 1):
            ...
            last_row = self.get_matching_xpath_count(
                "(" + rows_loc + ")[" + str(row_num) + "]/self::*[contains(@class, 'last_row')]")
    ...
    def _get_table_values(self, tables_loc):
        ...
        for table_num in range(1, 1 + tables_count):
            ...
            for tr_num in range(1, 1 + int(self.get_matching_xpath_count(table_body + "/tr"))):
                row_list = []
                for td_num in range(1,
                                    1 + int(self.get_matching_xpath_count(table_body + "/tr[" + str(tr_num) + "]/td"))):
