...
#$Id: //prod/main/sarf_centos/testlib/coeus1201/gui/admin/smart_license_config.py#1 $
...
URL_EDIT_BUTTON = "//a[contains(@onclick, 'edit_sl_transport_settings')]"
...
TG_OK_BUTTON  = "//button[@id='yui-gen6-button']"
...
REMOVE_AND_REGISTER_BUTTON= "//button[@id='yui-gen14-button']"
...
class SmartLicenseConfig(GuiCommon):
    ...
    def _register(self, **args):
        if args:
            ...
            if args.has_key('token_id'):
                self.input_text(TOKEN_TEXTBOX,args['token_id'])
            else:
                raise ValueError('Token id is mandatory for registration/reregistration')

            if args.has_key('force_register_flag'):
                if args['force_register_flag'] is True:
            ...
            if self._is_element_present(REGISTER_BUTTON):
                ...
                raise GuiError('Register button is not available')
