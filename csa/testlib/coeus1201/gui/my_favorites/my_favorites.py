#!/usr/bin/env python
# $Id: //prod/main/sarf_centos/testlib/coeus1201/gui/my_favorites/my_favorites.py#1 $
...
class MyFavorites(GuiCommon):
    ...
    def _select_page_path_from_list(self, path):
        ...
        assert len(path) == 2, 'List path should contain 3 elements'
        (menu, submenu) = path
        LIST_ELEMENT = \
            "/optgroup[contains(@label,'%s')]" \
            "/option[starts-with(text(),'%s')]" % (menu, submenu)
