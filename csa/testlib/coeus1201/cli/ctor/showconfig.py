#!/usr/bin/env python
# $Id: //prod/main/sarf_centos/testlib/coeus1201/cli/ctor/showconfig.py#1 $
...
import clictorbase
from sal.containers.yesnodefault import *
import re
import time

class showconfig(clictorbase.IafCliConfiguratorBase):
    def __call__(self, include_pwd='Y'):
        self.clearbuf()
        ...
        include_pwd = str(include_pwd).lower()
        if include_pwd not in password_options.keys():
            raise Exception("Password option has to be one of 'Yes (save password) | No (mask password)")

        self._writeln('showconfig')
        #self._query("Do you want to mask")
        self._query("Choose the password option")
        time.sleep(1)
        self._writeln(password_options[include_pwd])
        #self._query_response(str(include_pwd))
