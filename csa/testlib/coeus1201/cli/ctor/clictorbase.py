#!/usr/bin/env python
# $Id: //prod/main/sarf_centos/testlib/coeus1201/cli/ctor/clictorbase.py#1 $
...
default_timeout = 300
try:
    ctor_logfile = '%s/tmp/ctor.log' % os.environ['IAF2_HOME']
except:
    ctor_logfile = '/tmp/ctor.log'
...
def get_sess(host=None, dev_mode=False, timeout=30):
    ...
    password = 'ironport'
    try:
        log_file = open('%s/tmp/clictorbase.log' % os.environ['IAF2_HOME'] , 'w', 0)
    except:
        log_file = open('/tmp/clictorbase.log', 'w', 0)
...
class IafCliConfiguratorBase(object,Logger):
    ...
    def __init__(self, sess):
        ...
        self._global_err_dict = {
            ('You must enter a' + EOL_REGEX, expect.REGEX) : IafCliValueError,
            ('The value cannot.+$', expect.REGEX) : IafCliValueError,
            ('The value should' + EOL_REGEX, expect.REGEX) : IafCliValueError,
            ('The value must ' + EOL_REGEX, expect.REGEX) : IafCliValueError,
            ('The value may ' + EOL_REGEX, expect.REGEX) : IafCliValueError,
            ('That value must ' + EOL_REGEX, expect.REGEX) : IafCliValueError,
            ('That value is not valid.', expect.EXACT) : IafCliValueError,
            ('Not a valid ' + EOL_REGEX, expect.REGEX) : IafCliValueError,
            ('Value must be ' + EOL_REGEX, expect.REGEX) : IafCliValueError,
            ('Must be a ' + EOL_REGEX, expect.REGEX) : IafCliValueError,
            ('Must be at ' + EOL_REGEX, expect.REGEX) : IafCliValueError,
            ('Must be an ' + EOL_REGEX, expect.REGEX) : IafCliValueError,
            ('Please answer ' + EOL_REGEX, expect.REGEX) : IafCliValueError,
            ('Valid characters are' + EOL_REGEX, expect.REGEX) : IafCliValueError,
    ...
    def _writeln(self, line=''):
        ...
        return self._sess.writeln(_str)
    ...
    def _query_select_list_item(self, selector, exact_match=False,
        ...
        idx = self._select_list_item(selector, text_block=input_list_text_block, exact_match=exact_match)
        return idx
    ...
    def _process_input(self, param_map, do_restart=True, timeout=300):
        ...
        while 1:
            ...
            self._debug('question={q} key={k} answer={a}'.format(
                 q=str(question_text), k=key, a=answer_str) )
    ...
    def _to_the_top(self, newlines, timeout=default_timeout):
        text = ''
        self._debug('self._to_the_top(%d)' % newlines)
    ...
    def _end_command(self, newlines=5):
        """
        End cli command by entering up to newlines 'empty'
        and then sending interrupt
        """
        try:
...
if __name__ == '__main__':
    # Demonstrate IafConfiguratorBase methods.
    #
    # "tail -F /tmp/clictorbase.log" # or whatever ctor_logfile
    # is set to.  To view the CLI interaction as this script
    # is run.

    from iafframework import iafcfg
    host = iafcfg.get_hostname()
    sess = get_sess(host)
    ctor = IafCliConfiguratorBase(sess)

    ctor._writeln('version')
    ver_out = ctor._wait_for_prompt()
    print ver_out
