#!/usr/bin/env python
# $Id: //prod/main/sarf_centos/testlib/coeus1201/cli/ctor/grep.py#1 $
...
class grep(clictorbase.IafCliConfiguratorBase):
    ...
    def __call__(self, log_name=REQUIRED, regex=DEFAULT, insensitive=DEFAULT, non_matching=DEFAULT):

        self.clearbuf()
        self._writeln(self.__class__.__name__)
        self._expect([("Currently configured logs", EXACT)], timeout=3)
        self._query_select_list_item(log_name)
        self._query_response(regex)
        self._query_response(insensitive)
        self._query_response(non_matching)
        # Do not tail logs
...
if __name__ == '__main__':
    ...
    grep = grep(cli_sess)
    print grep(log_name='gui_logs', regex='Info: Begin Logfile')
