#!/usr/bin/env python
# $Id: //prod/main/sarf_centos/testlib/coeus1201/cli/ctor/reboot.py#1 $
...
class reboot(clictorbase.IafCliConfiguratorBase):
    def __call__(self, confirm_continue=YES, just_return=None, delay=None):
        # added for delay handling.
        command = 'reboot'

        if delay is not None:
            command = ' '.join([command, delay])
...
if __name__ == '__main__':
    ...
    rt(YES)
    sal.net.socket.wait_for_port(host, 22, 300)
