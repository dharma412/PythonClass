#!/usr/bin/env python
# $Id: //prod/main/sarf_centos/testlib/coeus1201/cli/ctor/upgrade.py#1 $ $DateTime: 2019/08/14 09:58:47 $ $Author: uvelayut $
...
class upgrade(clictorbase.IafCliConfiguratorBase):
    ...
    def __call__(self, version=None, downloadonly=None, seconds=0, input_dict=None, **kwargs):
        """ perform an upgrade install.
            version: version to upgrade WSA to.
       ...
       #set the build string to search for in the upgrade list
        v_arr = version.split('-')
        assert len(v_arr) == 2, "Version string %s is not valid" % version
        build_str = '%s build %s' % tuple(v_arr)
        inputs['build'] = build_str

        # set the build string to search for in the upgrade list
        # ok, let's fire up the CLI and do the upgrade
        self.clearbuf()
        self._writeln('upgrade')

        if downloadonly == 'n':
            self.downloadinstall(version, seconds, input_dict, **kwargs)
        else:
            return self
    _get_build_string = lambda self, version: \
         '%s build %s upgrade For web' % tuple(version.split('-'))
    ...
    def downloadinstall(self, version=None, seconds=5, input_dict=None, **kwargs):
        ...
        print "------------------BUILD NBUMBER----------", inputs['build']

        param_map = IafCliParamMap(end_of_command='Choose the operation you want to perform')
        param_map['build'] = ['upgrade For', DEFAULT, 1]
        param_map['save_cfg'] = ['save the current configuration', DEFAULT]
        param_map['email_cfg'] = ['email the current configuration', DEFAULT]
        param_map['save_pw'] = ['password option', DEFAULT, 2]
        #param_map['save_pw'] = ['include passwords', DEFAULT]
        param_map['continue'] = ['Do you wish to proceed with the upgrade', YES]
        ...
        self._process_input(param_map, do_restart=False, timeout=300)
        ...
        if self._get_major_build_number(version) < 80:
        ...
        # sleep one more second than that to allow connections to fully close
        try:
            self._query_response(str(seconds))
        except TimeoutError:
            # Only error out if we timeout and are not at a subprompt
            self._restart()
            raise ConfigError('Upgrade could not be executed. Please check ' \
                              'debug log for details.')
        except ExpectError:
            buf = self.getbuf()
            if buf.find('Rebooting the system...') == -1:
                raise ConfigError('Upgrade could not be executed.')

        time.sleep(int(seconds) + 1)

        # wsa reboot initiated
        try:
            self._restart()
        except:
            self._sess = None
    ...
    def install(self, seconds=5, input_dict=None, **kwargs):
        ...
        param_map['qstn_proceed'] = ['proceed with the upgrade',DEFAULT]
        param_map['qstn_return'] = ['Return to continue',DEFAULT]
