...
def set_flag(fd, flag):
    """ Set a behavior (flag) from the termios module. """
    mode = tcgetattr(fd)
    _set_flag(mode, flag)
...
def flag_string(fd):
    """ Return a string that shows the status of the flags that can be set
        from termios.tcgetattr(fd).
    """
    mod = sys.modules[__name__]
...
respectively. Multiple flags of this form may be supplied.
    ...
    if not args:  # return a report string
        ...
        for arg in args:  # should be set of strings, possibly with leading "-"
            cmd = {"cbreak": setcbreak, "raw": setraw, "cooked": cooked}.get(arg, None)
            if cmd is None:
                _set_flag(mode, arg)
            else:
                cmd(fd)
...
def cooked(fd, when=TCSANOW):
    """ Set the attr that shows we have cooked data to be processed. """
    mode = tcgetattr(fd)
...
def get_intr_char(fd=0):
    """ convenience method to get an attribute from the termios.tcgetattr() method. """
    mode = tcgetattr(fd)
    return mode[CC][VINTR]


def interrupt(fd, intr=None):
    """ Send an interrupt to the terminal (fd) we're interacting with. """
    if intr is None:
        mode = tcgetattr(fd)
        intr = mode[CC][VINTR]
    tcflush(fd, TCOFLUSH)
...
class PagedIO(object):
    """PagedIO(outfile, infile, [inputprompt], [pagerprompt])
Combines a write stream and a read stream into one read/write object. Writing
automatically pages output according to screen size.  """

    def __init__(self, prompt="> ", pagerprompt=None):
        ...
        if self.stdin.isatty():  # page only if input is a tty
            self.set_size()
    ...
    def close(self):
        """ Shut down the PagedIO connection. """
        self.unregister()
        self.stdout = None
        self.stdin = None
        self.closed = 1
...
object, via the handler method. It cannot be garbage collected until unregistered.
"""
    ...
    def raw_input(self, prompt=""):
        self._writtenlines = 0
        # the standard readline keeps the newline at the end.
        return "%s\n" % (raw_input(prompt or self.prompt),)
    ...
    def page_write(self, data):
        """ Write data to the "screen" one page at a time. """
        ld = len(data)
        rows = self.rows - 1
        ...
        try:
            ...
            while 1:
                try:
                    ...
                    if why[0] == EINTR:  # interrupted system call
                        continue
                    else:
                        raise
        finally:
...
def get_key(prompt=""):
    """ Get a response to a question given in 'prompt'. See _unittests/test_emailtest.py to see
        this function used.
    """
    c = ""
    so = sys.stdout
...
if __name__ == "__main__":
    ...
    long_s = "\n".join(map(lambda i: "abdefdg%d" % i, range(35)))
    po = PagedIO()
    try:
        for l in xrange(100):
            po.write("%d. Now is the time for all good men....%s\n" % (l, "x" * (l / 10)))
        po.write("xxx" * 100)
        for i in xrange(5):
            po.write("\n")
        for i in xrange(5):
