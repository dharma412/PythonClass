...
"""
#: Reference Symbols: shparser

# $Id: //prod/main/sarf_centos/testlib/sal/deprecated/thirdparty/shparser.py#1 $

import os
from sal.deprecated.thirdparty.fsm import FSM, ANY
...
Feed the parser, callback gets an argv list for each completed suite."""
    ...
    def parseFile(self, fo):
        """ Parse an argv list from a file. """
        data = fo.read(4096)
        while data:
            self.feed(data)
            data = fo.read(4096)
    ...
    def feed(self, text):
        """ Process text with the FSM, which will call callback
            functions based on various transitions added below.
        """
        text = self._buf + text
        i = 0
        for c in text:
    ...
    def interact(self, ps1="shparse> ", ps2="more> "):
        """ Interact with the user sitting at the terminal. This will
            feed the raw_inputed text to the FSM and call callback functions
            based on various transitions added below.
        """
        try:
            while 1:
                line = raw_input(ps1)
...
def _test(argv):
    sh = ShellParser(_print_argv)
    sh.feedline('/usr/bin/prog -q -N "" -t tparm -b 1024 -f "bogus one"')
    sh.interact()
