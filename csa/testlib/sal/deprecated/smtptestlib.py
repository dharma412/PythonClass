...
# vim:ts=4:sw=4
# $Id: //prod/main/sarf_centos/testlib/sal/deprecated/smtptestlib.py#1 $
# start with a copy of the stock smtplib...

#: Reference Symbols: smtptestlib
...
Note: This is a modified version of the standard smtplib.py module.
Ref: http://pydoc.org/2.2/smtplib.html
'''
...
#
# Extensive modifications by Keith Dart <kdart@ironport.com>. 

# TODO from IAF refactoring (06/03/09): need to investigate if this module is 
#       indeed needed
...
def encode_base64(s, eol=None):
    """Encode string(s) using base64. eol arg not used."""
...
class SSLFakeSocket:
    """A fake socket object that really wraps a SSLObject (from socket.ssl()).
...
class SSLFakeFile:
    """A fake file like object that really wraps a SSLObject (from socket.ssl()).
...
class SMTP(object):
    ...
    def __init__(self, host='', port=25, bindto=None, logfile=None):
        ...
        if host:
            ...
            # if code != 220:
            # we sometimes make a custom code in HAT
            # and test that very change ! - so we cannot
            # be restricted by 220 - but this puts a new
            # limit of 220 OR 221
            if code not in (220, 221):
    ...
    def connect(self, host='localhost', port=0, bindto=None):
...
def get_mailer(host="", port=SMTP_PORT):
    """return SMTP(str(host), int(port))"""
    return SMTP(str(host), int(port))
...
if __name__ == '__main__':
    def test(argv):
        ...
        while 1:
            ...
            msg = msg + line
        server = SMTP()
        server.connect('localhost', 9025, 'localhost')
