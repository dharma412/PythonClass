#!/usr/bin/env python
# $Id: //prod/main/sarf_centos/testlib/zeus1380/gui/my_favorites/my_favorites.py#1 $ $DateTime: 2020/05/25 00:19:30 $ $Author: sarukakk $
...
TASKS_TABLE = "xpath=//table[contains(@class, 'cols tasks-table')]"
EDIT_LINK = lambda name: "%s//a[normalize-space(text()) = '%s']" % (TASKS_TABLE, name)
ALL_FAVORITES = "//table[contains(@class, 'tasks-table')]//a"
#NAME_BY_IDX = lambda index: "%s//tr[td][%s]//a/text()" % (TASKS_TABLE, index)
...
LANDING_PAGE_MARK = lambda name: "%s//tr[normalize-space(td/a) = '%s']/td[1]" % (TASKS_TABLE, name)
CONFIRM_LANDING_PAGE_BUTTON = "//div[@id = 'confirmation_dialog']//button[text() ='OK']"
...
DESCRIPTION_INPUT = MY_FAVORITES_FORM + "//textarea[@name='description']"
...
class MyFavorites(GuiCommon):
    ...
    def _navigate_to_add_form(self):
        ...
        self.click_link(ADD_FAVORITE_MENU_LINK)
    ...
    def my_favorites_add(self, path, name=None,
        ...
        *Parameters:*
        - `path` List with the menu path to the page that should be favorited.
          Also it could be string in form 'Tab Name -> Menu Name -> Menu Item Name'.
          Required.
        - `name` Display name to give to the new favorite. If
          it's not provided then default name will be given by feature. Optional.
        ...
        *Examples:*
        | ${path}= | Create List | Management Appliance | Network | IP Interfaces |
    ...
    def my_favorites_edit(self, name,
        ...
        *Examples:*
        | ${path}= | Create List | Management Appliance | Network | DNS |
    ...
    def my_favorites_set_landing_page(self, name):
        ...
        *Examples:*
        ...
        cell = LANDING_PAGE_MARK(name)
        make_home_button = cell + "/div[@class='make-button']"
        home_mark = cell.lstrip('xpath=') + "/div[@class='make-button-selected']"

        self._debug('home_mark = %s' % (home_mark,))
        count = int(self.get_matching_xpath_count(home_mark))
        self._debug('found %s times' % (count,))
        if count != 0:
