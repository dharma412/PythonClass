# $Id: //prod/main/sarf_centos/testlib/zeus1380/gui/email/tracking/message_tracking.py#1 $
...
MESSAGE_FILTER = "xpath=//input[@id='message_filter_name']"
...
class MessageTracking(GuiCommon):
    ...
    def get_keyword_names(self):
        return ['email_message_tracking_search',
                'email_message_tracking_get_page_count',
                'email_message_tracking_get_total_result_count',
                'email_message_tracking_get_message_details',]

    def _open_page(self):
        self._navigate_to('Email', 'Message Tracking', 'Message Tracking')

    def email_message_tracking_search(self,
                                sender_data=None,
                                ...
                                mesg_id_header=None,
                                ironport_mid=None,
                                cisco_ironport_host=None,
                                items_per_page=None,
                                query_timeout=None,
                                max_results=None,
                                dmarc_failures=None,
                                dmarc_failures_from_domain=None,
                                dmarc_actions=None,
        ...
        - `cisco_ironport_host`: ESA machine to show data for. Can be
          All Hosts or host name of ESA machine (e.g. vm30esa0048.ibqa)
          - `items_per_page`: items per result page. Either 20, 50,
          100 or 250. Default 20.
        - `dmarc_failures`: DMARC Failures checkbox. Should be ${True} or ${False}
          - `dmarc_failures_from_domain`: From Domain text field for DMARC. String
          - `dmarc_actions`: DMARC Actions if message events contain dmarc_failures,
          - `dlp_policy`: DLP Policy Name if message events contain
          Array containing either of _no action_, _quarantined_, _rejected_,
        ...
        self._fill_mesg_event_data(message_event, dlp_policy, url_clicked, vof_threat_category_name,
              ...
              macro_detection_file_types, amp_protection_file_types, amp_protection_malicious_file_types,
              amp_protection_mailflow_directions, content_filter_name, message_filter_name, content_filters_mail_flow_direction,
              ...
              sender_domain_reputation_types,dmarc_failures,dmarc_failures_from_domain, dmarc_actions)
        ...
        if cisco_ironport_host is not None:
            host_option_locator = "xpath=//select[@id='host']" \
                                  "/option[starts-with(normalize-space(), '%s')]" \
                                  % (cisco_ironport_host,)
            if not self._is_element_present(host_option_locator):
                raise ValueError("'%s' not found in 'Cisco Ironport Host' list" \
                                 % (cisco_ironport_host,))
            host_to_select = self.get_text(host_option_locator)
            self.select_from_list("xpath=//select[@id='host']", host_to_select)
        self.click_button("xpath=//input[@id='submitButton']", "don't wait")

        if items_per_page is not None:
            items_per_page_locator = "xpath=//select[@id='wd_pageSize']"
            if not self._is_element_present(items_per_page_locator):
                return []
            self.select_from_list(items_per_page_locator, items_per_page)
        ...
        searchlist._get_results(sender_ip_search_options)
        return searchlist
    ...
    def _fill_mesg_event_data(self, message_event, dlp_policy, url_clicked, vof_threat_category_name,
        ...
        message_event_option = {'virus positive':'id=event_virus',
                                ...
                                'message filters':'id=message_filters',
        ...
        dmarc_failure_action = {'no action': 'id=dmarc_no_action',
                                'quarantined': 'id=dmarc_quarantined',
                                'rejected': 'id=dmarc_rejected',
                                }
        dmarc_failures_list = {True:'id=event_dmarc_failed',
                          False:'id=event_dmarc_passed',
                         }
        if message_event:
        ...
        if dmarc_failures and self._is_checked(
                message_event_option['dmarc failures']) and dmarc_failures_from_domain:
            self.input_text("xpath=//input[@id='dmarc_domain_from']",
                            dmarc_failures_from_domain)
        if dmarc_actions and self._is_checked(
                message_event_option['dmarc failures']):
            self.select_checkbox(dmarc_failures_list[dmarc_failures])
            for action in dmarc_actions:
                self.select_checkbox(dmarc_failure_action[action.lower()])
        ...
        if self._is_checked(message_event_option['message filters']):
            if message_filter_name:
                self.input_text(MESSAGE_FILTER, message_filter_name)

    def email_message_tracking_get_page_count(self, result_list):
        ...
        Use this method to get page count with message results
         ...
         Example:
         | ${page_count} = | Email Message Tracking Get Page Count |
         | ... | ${result_list} |
         """
        return result_list.get_page_count()

    def email_message_tracking_get_total_result_count(self, result_list):
    ...
    def email_message_tracking_get_message_details(self,
                                             mid,
        ...
        *Example*:
        | ${res}= | Email Message Tracking Get Message Details | 4 | continue_search=3 months |
        | Log Dictionary | ${res} |
        ...
        if get_last_event:
            ...
            ("%s//tr//span[contains(@style,'background-color')]/parent::td/preceding-sibling::th" % (processing_details_loc,))
...
class SearchResultList(object):
    ...
    @set_speed(0, 'gui')
    def get_results_on_page(self,sender_ip_search_options,page=None):
        ...
        while rows:
            ...
            if sender_ip_search_options != "rejected_connections":
    ...
    def _get_results(self,sender_ip_search_options):
        pages = self.get_page_count()
        if pages == 0:
            self.gui._info("No results found.")
            return
        for page in range(pages):
            self.result_list[str(page+1)] = self.get_results_on_page(sender_ip_search_options)
            if page < (pages-1):
                self._go_next()
