#!/usr/bin/env python
# $Id: //prod/main/sarf_centos/testlib/zeus1380/gui/management/network/smtp_routes.py#1 $ $DateTime: 2020/05/25 00:19:30 $ $Author: sarukakk $
...
CONFIG_FILES_LIST = "//select[@id='impexpfile']"
...
DELETE_ALL_CHECKBOX = "//input[@name='del_0' and @type='checkbox']"
...
class SmtpRoutes(GuiCommon):
    ...
    def _click_delete_routes_checkboxes(self, domains):
        ...
        for domain in domains_to_del:
            ...
            if self._is_element_present(delete_link):
                ...
                raise ValueError('"%s" receiving domain does not exist' %\
                                 (domain,))
    def _delete_all_routes_list(self):
        if self._is_element_present(DELETE_ALL_CHECKBOX):
           self._set_checkbox(True, DELETE_ALL_CHECKBOX)
           self._info('Clicked "Delete All Routes" Checkbox')
        else:
            raise guiexceptions.GuiFeatureDisabledError(
                 'There are no configured routes list to delete.')
    ...
    def _select_file_to_import(self, filename):
        ...
        if filename in config_files:
            self.select_from_list_by_label(CONFIG_FILES_LIST,filename)
    ...
    def smtp_routes_delete(self, recv_domain):
        ...
        *Parameters*
            - `recv_domain`: domain(s) for which routes will be deleted.
              ...
              If recv_domain is equal to All then all the smtp route will be deleted.
