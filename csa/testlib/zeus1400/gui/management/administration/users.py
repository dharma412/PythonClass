...
from users_def.tfauth_settings import TfAuthSettings
from users_def.tf_priv_settings import SecondFactorPrivSettings
...
GROUP_SELECT_BOX = lambda index: 'id=ldap_groups_mapping[%s][role]' % (index,)
...
TF_AUTH_ENABLED_CHECKBOX= "//input[@id='sf_auth_enabled']"
TF_AUTH_EDIT_BUTTON = "//input[@type='button' and contains(@onclick, 'FormEditSecondFactorAuth')]"
...
#MATCHED_CONTENT_VISIBILITY_CHECKBOX = 'url_info_visibility'
URL_TRACKING_PRIVILEGE_ROLES = ('admin', 'operators', 'readonly', 'emailadmin', 'helpdesk')
URL_TRACKING_PRIVILEGE_MAP = lambda role:'xpath=//input[@name="url_info_visibility[]" and @value="%s"]' % (role,)
...
PROTOCOL_SELECT_BOX = lambda index: 'id=service_hosts[%s][auth_type]' % (index,)
...
PW_EXP_GRACE_CHECKBOX= 'enable_password_grace'
PW_EXP_GRACE_TEXTBOX= 'password_grace_period'
...
PWD_REP_SEQ_CHECKBOX= 'password_no_rep_seq_chars'
...
class Users(GuiCommon):
    ...
    def get_keyword_names(self):
        ...
        return ['users_add_user',
                ...
                'users_twofactor_auth_is_enabled',
                'users_twofactor_auth_enable',
                'users_twofactor_auth_disable',
                'users_twofactor_auth_edit',
                'users_twofactor_auth_privileges',

                'users_edit_account_locking',
                'users_lock_account',
                'disable_account_locking',
                'users_unlock_account',
                'users_edit_reset_rules',
                'users_edit_reset_rules_disable',
                'users_get_reset_passphrase_days',
                'users_edit_password_rules',
                'users_edit_url_tracking_privileges',
    ...
    def _get_tf_auth_controller(self):
        if not hasattr(self, '_tf_auth_controller'):
            self._tf_auth_controller = TfAuthSettings(self)
        return self._tf_auth_controller

    def _click_add_user_button(self):
        self.click_element(ADD_USER_BUTTON)
    ...
    def _get_predefined_roles(self):
        if self._is_element_present(PREDEFINED_ROLES_LIST):
            self.click_element(PREDEFINED_ROLES_LIST)
    ...
    def _set_user_role(self, user_role):
        ...
        predefined_roles = [label.strip() for label in predefined_roles]

        if user_role in predefined_roles:
            self._click_radio_button(PREDEFINED_ROLE_RADIOBUTTON)
            self.select_from_list_by_label(PREDEFINED_ROLES_LIST, user_role)
        elif user_role in custom_roles:
            self._click_radio_button(CUSTOM_ROLE_RADIOBUTTON)
            self.select_from_list_by_label(CUSTOM_ROLES_LIST, user_role)
        else:
    ...
    def _select_auth_type(self, auth_type):
        self.select_from_list_by_label(AUTH_TYPES_LIST, auth_type)
    ...
    def _fill_radius_server_row(self, server, index):
        for entry, link in zip(
            ...
            if len(server) == 5:
                ...
                value = 'pap'
            protocol_option = "%s" % (value,)
            self.select_from_list_by_value(PROTOCOL_SELECT_BOX(index),
            protocol_option)
        ...
        if auth_type is not None:
            ...
            if auth_type.upper() not in auth_types:
                raise ValueError('"%s" auth type for RADIUS is not valid' %\
                                 (auth_type,))
            self.select_from_list_by_label(locator, auth_type.upper())
    ...
    def _select_ldap_auth_query(self, query):
        ...
        self.select_from_list_by_label(LDAP_AUTH_QUERY_LIST, query)
    ...
    def _fill_group_mapping_row(self, group, index):
        for entry, link in zip((group[0],), ('group_name',)):
            ...
            self.select_from_list_by_label(GROUP_SELECT_BOX(index), group[1])
    ...
    def _edit_account_locking_rules(self, lock_failed_login, display_message,login_attempt):
        if login_attempt:
            if self._is_element_present(LOGIN_ATTEMPT_LIMIT):
                self.input_text(LOGIN_ATTEMPT_LIMIT, text=login_attempt)
    ...
    def disable_account_locking(self):

        self._open_page()

        self._click_edit_settings_button()
        if self._is_editable(FAILED_LOGIN_ATTEMPTS_TEXTBOX):
            self.click_element(ENABLE_FAILED_LOGIN_CHECKBOX)

        self._click_submit_button(wait=False, skip_wait_for_title=True, \
                                  accept_confirm_dialog=True)

    def _edit_account_reset_rules(self, admin_change, password_expiration, display_reminder,grace_period):

        if admin_change:
            if not self._is_checked(ADMIN_CHANGE_RESET_CHECKBOX):
                self._select_checkbox_value(ADMIN_CHANGE_RESET_CHECKBOX, admin_change, None)

        if password_expiration is not None:
            if self._is_checked(ENABLE_PW_EXP_CHECKBOX):
                self.input_text(PW_EXP_PERIOD_TEXTBOX, password_expiration)
            else:
                self._select_checkbox_value(ENABLE_PW_EXP_CHECKBOX, password_expiration, PW_EXP_PERIOD_TEXTBOX)
        ...
        if grace_period and \
            ...
            raise guiexceptions.ConfigError('Password expiration must be enabled to '\
                    'use grace period')
        if display_reminder:
            if self._is_checked(DISPLAY_PW_EXP_REMINDER_CHECKBOX):
                self.input_text(PW_EXP_REMINDER_TEXTBOX, display_reminder)
            else:
                self._select_checkbox_value(DISPLAY_PW_EXP_REMINDER_CHECKBOX,
                    display_reminder, PW_EXP_REMINDER_TEXTBOX)

        if grace_period:
            if self._is_checked(PW_EXP_GRACE_CHECKBOX) and grace_period is not None:
                self.input_text(PW_EXP_GRACE_TEXTBOX, grace_period)
            else:
                self._select_checkbox_value(PW_EXP_GRACE_CHECKBOX,
                                        grace_period, PW_EXP_GRACE_TEXTBOX)

    def _edit_account_reset_rules_disable(self, admin_change, password_expiration):

        if admin_change=='off':
            if self._is_checked(ADMIN_CHANGE_RESET_CHECKBOX):
                self.click_element(ADMIN_CHANGE_RESET_CHECKBOX)

        if password_expiration=='off':
            if self._is_checked(ENABLE_PW_EXP_CHECKBOX):
                self.click_element(ENABLE_PW_EXP_CHECKBOX)

    def _get_account_reset_passphrase_days(self):

        self._open_page()

        self._click_edit_settings_button()

        if not self._is_checked(ENABLE_PW_EXP_CHECKBOX):
            self.click_element(ENABLE_PW_EXP_CHECKBOX)

        return (self.get_value(PW_EXP_PERIOD_TEXTBOX))


    def _edit_account_password_rules(self, req_min_chars, req_alpha, req_number,
        req_special_char, ban_username, rep_sequential_char, ban_reuse):
        if req_min_chars:
        ...
        for locator, value, text_locator in (
            ...
            (PWD_REP_SEQ_CHECKBOX, rep_sequential_char, None),
    ...
    def users_add_user(self, username, fullname, password=None, user_role=None,
        ...
        Examples:
            ...
            | ${sys_gen_passwd} | user_name | full_user_name | user_password | generate_passphrase=yes |
        """
        self._open_page()
        ...
        sys_gen_passwd = self._fill_password_textbox(password,generate_passphrase)
    ...
    def users_edit_user(self, username, fullname=None, password=None, user_role=None, generate_passphrase=None):
        ...
        if password is not None or generate_passphrase is not None:
            sys_gen_passwd = self._fill_password_textbox(password,generate_passphrase)
 ...
 |
    ...
    def users_twofactor_auth_is_enabled(self):
        """Get Two Factor Authentication feature state
            ...
            *Examples:*
            | ${sf_auth_is_enabled}= | Users Twofactor Auth Is Enabled |
        """

        TF_AUTH_NOT_ENABLED_MARK = 'Two-Factor Authentication is disabled. '
        return not self._is_text_present(TF_AUTH_NOT_ENABLED_MARK)

    def users_twofactor_auth_enable(self, settings):
        """ Enable Two Factor Authentication feature and sets
         ...
         """
        self._open_page()
        self.click_button(TF_AUTH_EDIT_BUTTON)
        self._get_tf_auth_controller().set(settings)
        self._click_submit_button()

    def users_twofactor_auth_disable(self):
        """Disable Two Factor Authentication feature.
               Will do nothing is the feature is already disabled

               *Examples:*
               | Users Twofactor Auth Disable |

                  """
        self._open_page()
        if not self.users_twofactor_auth_is_enabled():
            return
        self.click_button(TF_AUTH_EDIT_BUTTON)
        self._unselect_checkbox(TF_AUTH_ENABLED_CHECKBOX)
        self._click_submit_button()

    def users_twofactor_auth_edit(self,settings):
        """Edit Two Factor Authentication feature settings.
            ...
            *Examples:*
            ...
            | Run Keyword If | not ${is_enabled} |
            | ... | Users two factor Auth Enable | ${radius_sf_auth_settings} |
            | Append to List | ${radius_servers} |
            | ... | sma19.sma,1814,asdfghjkl,, |
            | Users two factor Auth Edit | ${radius_sf_auth_settings} |
            """
    ...
    def users_twofactor_auth_privileges(self,settings):
        ...
        *Examples:*
        ...
        | ... | Help Desk User | ${True} |
        | ... | My Cusom Role | ${False} |
        | Users two factor Auth Privileges | ${privileges_map} |
        """
        self._open_page()
        if not self.users_twofactor_auth_is_enabled():
            raise guiexceptions.ConfigError('Two-Factor Authentication ' \
    ...
    def users_edit_account_locking(self, lock_failed_login=None, display_message=None,login_attempt=None):
        ...
        Examples:
            | Users Edit Account Locking | lock_failed_login=15 | display_message=test |login_attempt=5
        """
        self._edit_account_rules(self._edit_account_locking_rules,
            lock_failed_login, display_message,login_attempt)

    def users_edit_reset_rules(self, admin_change=None, password_expiration=None,
        display_reminder=None,grace_period=None):
        ...
        self._edit_account_rules(self._edit_account_reset_rules, admin_change,
            password_expiration, display_reminder,grace_period)

    def users_edit_reset_rules_disable(self, admin_change=None, password_expiration=None,
        ):
        """Edit password reset settings.
        ...
        Examples:
            | Users Edit Reset Rules Disable | admin_change=off | password_expiration=off |

        """
        self._edit_account_rules(self._edit_account_reset_rules_disable, admin_change,
            password_expiration)

    def users_get_reset_passphrase_days(self):
        """Edit password reset settings.

        Examples:
            | ${reset_passphrase_days}=  Users Get Reset Passphrase Days|
            | Log  ${reset_passphrase_days}|

        """
        return self._get_account_reset_passphrase_days()
    ...
    def users_edit_password_rules(self, req_min_chars=None, req_alpha=None,
        ...
        rep_sequential_char=None,ban_reuse=None):
        ...
        Examples:
            | Users Edit Password Rules | req_min_chars=20 | req_alpha=on | req_number=off | req_special_char=on | ban_username=on |rep_sequential_char=on| ban_reuse=10 |
        """

        self._edit_account_rules(self._edit_account_password_rules,
            req_min_chars, req_alpha, req_number, req_special_char,
            ban_username, rep_sequential_char, ban_reuse)
    ...
    def users_edit_url_tracking_privileges(self, url_tracking_privileges=None,
        admin=None, operators=None, readonly=None, emailadmin=None, helpdesk=None):
        """Edit URL Tracking Privileges

        Parameters:
            - `url_tracking_privileges`: Allow access to URL Matched Content
              in Message Tracking results. Boolean.
              If this argument defined additional arguments will be ignored (legacy behavior).
              If this argument is set to true then tracking privileges will be enabled
              for all user categories and othenwise if argument is set to false
              no user categories will have tracking privileges.
            - `admin`: Allow users with Administrators role
                to have access to URL Matched Content
            - `operators`: Allow users with Operators role
                to have access to URL Matched Content
            - `readonly`: Allow users with Read-Only Operators role
                to have access to URL Matched Content
        ...
        Examples:
            | Users Edit URL Tracking Privileges | url_tracking_privileges=${True} |
            | Users Edit URL Tracking Privileges | url_tracking_privileges=${False} |
        ...
        if url_tracking_privileges is not None:
            for role in URL_TRACKING_PRIVILEGE_ROLES:
                self._set_checkbox(url_tracking_privileges,
                    URL_TRACKING_PRIVILEGE_MAP(role),)
        else:
            locals_dict = locals()
            for role in URL_TRACKING_PRIVILEGE_ROLES:
                if locals_dict[role] is not None:
