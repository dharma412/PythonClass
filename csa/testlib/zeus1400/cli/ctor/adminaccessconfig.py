#!/usr/bin/env python
# $Id: //prod/main/sarf_centos/testlib/zeus1380/cli/ctor/adminaccessconfig.py#2 $ $DateTime: 2020/06/18 05:28:57 $ $Author: mrmohank $
...
class adminaccessconfig(clictorbase.IafCliConfiguratorBase):
    ...
    def encryptconfig(self):
        self._sess.clearbuf()
        self._query_response('ENCRYPTCONFIG')
        return EncryptconfigAdminAccessConfig(self._get_sess())
...
class EncryptconfigAdminAccessConfig(clictorbase.IafCliConfiguratorBase):
    ...
    def get_encyption_status(self):
        self._expect(
            ['Config encryption is currently disabled',
             'Config encryption is currently enabled'])
    ...
    def edit_encrypt_config(self, enable, wait_time):

        if enable:
            self._expect('you want to enable encryption of sensitive data', \
                         EXACT, timeout=3)
            self._query_response('Y')

        else:
            self._expect('you want to disable encryption of sensitive data', \
                         EXACT, timeout=3)
            self._query_response('Y')
        self._expect('Enter the number of seconds to wait before', EXACT, timeout=3)
        self._query_response(wait_time)
