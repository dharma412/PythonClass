#!/usr/bin/python
# $Id: //prod/main/sarf_centos/testlib/zeus1380/cli/ctor/certconfig.py#3 $
# $DateTime: 2020/07/01 01:42:07 $
# $Author: mrmohank $
"""
...
class certconfig(ccb.IafCliConfiguratorBase):
    ...
    def cert_authority_status(self):
        self._query_response('CERTAUTHORITY')
        return self._read_until('Choose the operation')

    def cert_auth_type(self, cert_type):
        self.cert_authority_status()
        self._writeln(cert_type.upper())
        return self._read_until('Choose the operation')

    def cert_authority_toggle_status(self, cert_type='custom', enable=True):
        self.cert_auth_type(cert_type)
        if enable:
    ...
    def cert_authority_print(self, cert_type='custom'):
        self.cert_auth_type(cert_type)
        self._writeln('PRINT')
        return self._read_until('Choose the operation')
    ...
    def cert_authority_export(self, cert_number, export_file_name, cert_type='custom', export_all=False):
        self.cert_auth_type(cert_type)
        self._writeln('EXPORT')
        self._read_until('Do you want to export all custom ca certificates')
        if export_all:
            self._writeln('Y')
            self._read_until('Enter a name for the exported file')
            self._writeln(export_file_name)
            return self._read_until('Choose the operation')
        else:
            self._writeln('N')
            self._read_until('Select the custom ca certificate you wish to export')
            self._writeln(cert_number)
            self._read_until('Enter a name for the exported file')
            self._writeln(export_file_name)
            return self._read_until('Choose the operation')

    def cert_authority_import(self, file_name, fqdn_validation):
        self.cert_auth_type('CUSTOM')
        self._writeln('IMPORT')
        self._read_until('Enter the name of the file on machine')
        self._writeln(file_name)
        if fqdn_validation:
    ...
    def cert_authority_delete(self, cert_number, confirm_delete):
        self.cert_auth_type('CUSTOM')
        self._writeln('DELETE')
        self._read_until('Select the custom ca certificate you wish to delete')
        self._writeln(cert_number)
        self._read_until('Are you sure you want to delet')
        self._writeln(confirm_delete)
        out = self._read_until('Choose the operation')
        self._to_the_top(3)
        return out
...
class certconfigSetup(ccb.IafCliConfiguratorBase):
    ...
    def __call__(self, one_cert, replace, intermediate, cert_list, fqdn):
        ...
        if one_cert and not same_cert:
            ...
            if replace:
                self._enter_certs(intermediate, cert_list, fqdn)
        else:
    ...
    def _enter_certs(self, intermediate, cert_list, fqdn=DEFAULT):
        expect_list = ('Inbound:', 'Outbound:', 'HTTPS', 'LDAPS')
        error_messages = ['invalid FQDN', 'Validation Error',
                          'Common Name or SAN:dNSName']
        for cert_tup in cert_list:
            ...
            if fqdn == 'yes':
                self._expect('Fully Qualified Domain Name', EXACT, timeout=3)
                self._query_response(fqdn)
                fqdn_status=self._read_until(['Choose the operation'])
                self._info(fqdn_status)
                for message in error_messages:
                    if message in fqdn_status:
                        raise ConfigError('failed due to certificate pair with invalid FQDN')
                else:
                    pass
