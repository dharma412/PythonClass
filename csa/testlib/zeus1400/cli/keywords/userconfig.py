...
class UserConfig(CliKeywordBase):
    ...
    def user_config_new(self,user_name, full_name, password=None, group=DEFAULT, 
                        admin_passphrase=None, generate_passwd=None):
        """Adds new user.
    ...
    def user_config_password_assign(self, user_name, admin_passphrase=None,
                                    new_password=None, generate_passwd=None):
        ...
        Parameters:
        - `user_name`:        username of existing user to be edited.
        -  `admin_passphrase` : Admin pass phrase
        - `new_password`:    new password to replace existing one.
        - `generate_passwd` : yes or no

        Example:
        | User Config Password Assign | foo | newpasswd |
        |${sys_gen_passwd}= | User Config Password Assign | foo | generate_passwd=yes |
        """
        variables = common.Variables.get_variables()
        admin_passphrase = admin_passphrase or variables["${DUT_ADMIN_SSW_PASSWORD}"]
        kwargs = {
        ...
        return self._cli.userconfig().password().assign(**kwargs)
    ...
    def user_config_policy_passwordstrength(self, min_chars=DEFAULT, case_letters=DEFAULT, numbers=DEFAULT,
                                            spec_chars=DEFAULT, sim_name=DEFAULT, sequential_chars=DEFAULT,
                                            reuse=DEFAULT, num_recent=DEFAULT, word_list=DEFAULT,
        ...
        - `sim_name`: Reject passwords similar to the username. Either 'Yes' or 'No'.
                'No' or last specified option is present by default.
        - `sequential_chars`: Are three or more repetitive or sequential. Either 'Yes' or 'No'.
                         'No' or last specified option is present by default.
        ...
        kwargs = {
            ...
            'sequential_chars' : self._process_yes_no(sequential_chars),
