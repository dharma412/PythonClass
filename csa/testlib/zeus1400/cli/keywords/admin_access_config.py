#!/usr/bin/env python
# $Id: //prod/main/sarf_centos/testlib/zeus1380/cli/keywords/admin_access_config.py#2 $ $DateTime: 2020/06/18 05:28:57 $ $Author: mrmohank $
...
class AdminAccessConfig(CliKeywordBase):
    ...
    def get_keyword_names(self):
        return ['admin_access_config_csrf',
                ...
                'admin_access_config_encryptconfig',
                'admin_access_config_encryptconfig_status',
                ]
    ...
    def admin_access_config_encryptconfig(self, enable=False, wait_time=0):
        """Enabling Encrypt config or disable encrypt config

        adminaccessconfig > encryptconfig

        *Parameters:*
        - `enable`: To perform encryption Either True, or False
                       by default False
        - `wait_time` : number of seconds to wait before forcibly closing connections

        *Examples:*
        | Admin Access Config Encryptconfig | enable=True| wait_time=5 |
        | Admin Access Config Encryptconfig | enable=False |
        | Admin Access Config Encryptconfig |
        """
        self._cli.adminaccessconfig().encryptconfig().edit_encrypt_config(enable,wait_time)

    def admin_access_config_encryptconfig_status(self):
        """status of encryptconfig
        ...
        *Examples:*
        ...
        """
        return  self._cli.adminaccessconfig().encryptconfig().get_encyption_status()
