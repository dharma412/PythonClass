#!/usr/bin/env python
# $Id: //prod/main/sarf_centos/testlib/zeus1380/gui/management/administration/feature_keys.py#2 $ $DateTime: 2020/06/04 20:56:34 $ $Author: mrmohank $
...
class FeatureKeys(GuiCommon):
    ...
    def _get_installed_keys(self):
        ...
        for row in xrange(starting_row, num_of_rows):
            key_values = map(lambda column: self._get_table_cell('xpath=%s.%s.%s' % (INSTALLED_KEYS_TABLE, row, column)),xrange(num_of_columns))
