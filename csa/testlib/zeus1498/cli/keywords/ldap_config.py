#!/usr/bin/env python
# $Id: //prod/main/sarf_centos/testlib/zeus1380/cli/keywords/ldap_config.py#1 $ $DateTime: 2020/05/25 00:19:30 $ $Author: sarukakk $
...
class LDAPConfig(CliKeywordBase):
    ...
    def ldap_config_setup(self, interface=DEFAULT, treat_negative=DEFAULT, ldap_server_certificate=DEFAULT):
        ...
        -  `ldap_server_certificate`: Validate LDAP server certificate.

        Examples:
        | LDAP Config Setup | Management | Yes | Yes |
        | LDAP Config Setup | Auto | No | No |
        """
        return self._cli.ldapconfig().setup(interface,
            self._process_yes_no(treat_negative), self._process_yes_no(ldap_server_certificate))
