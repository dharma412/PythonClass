#!/usr/bin/env python
# $Id: //prod/main/sarf_centos/testlib/zeus1380/cli/keywords/ssl_config.py#1 $ $DateTime: 2020/05/25 00:19:30 $ $Author: sarukakk $
...
class SslConfig(CliKeywordBase):
    ...
    def get_keyword_names(self):
        return ['ssl_config_inbound',
                ...
                'ssl_config_edit_peer_cert_fqdn',
                'ssl_config_get_peer_cert_fqdn_status']
    ...
    def ssl_config_edit_peer_cert_fqdn(self, *args):
        """Edit PEER_CERT_FQDN ssl settings.

        CLI command: sslconfig > peer_cert_fqdn

        *Parameters:*
        - `enable`: Pass ${True} to enable PEER_CERT_FQDN 
                    Pass ${False} to disable PEER_CERT_FQDN 
        ...
        *Examples:*
        ...
        """
        kwargs = self._convert_to_dict(args)
        self._cli.sslconfig().set_peer_cert_fqdn(**kwargs)

    def ssl_config_get_peer_cert_fqdn_status(self):
        """Keyword to get peer cert fqdn Status

        *Parameters*
        will return the status of the peer cert fqdn status
        |Enabled | Disabled |

        Examples:
        |${status}= | SSL Config Get Peer Cert Fqdn Status |
        """
        return self._cli.sslconfig().peer_cert_fqdn_status()
