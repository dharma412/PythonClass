#!/usr/bin/env python
# $Id: //prod/main/sarf_centos/testlib/phoebe1350/gui/monitor/quarantines/search.py#1 $ $DateTime: 2020/01/06 01:25:43 $ $Author: saurgup5 $
    ...
    def _press_view_all_messages_button(self):
        ...
        if self._is_element_present(QUARANTINE_SEARCH_TABLE_VIEW_ALL_MESSAGES_BUTTON):
            if 'disabled' not in self.get_element_attribute\
                ("xpath=" + QUARANTINE_SEARCH_TABLE_VIEW_ALL_MESSAGES_BUTTON, "class"):
    ...
    def _grep_row(self,
        ...
        data.mid = self.get_element_attribute\
            ("%s//input[@type='checkbox']" % \
             ...
             (QUARANTINE_SEARCH_TABLE_MESSAGE_LIST, row_idx, 1), "value")
        # skip 1st(mid) column
...
#        self.select_from_dropdown_list(option[0], option[1])
        if option[1] is not None:
            self.select_from_list_by_label(option[0], option[1])
