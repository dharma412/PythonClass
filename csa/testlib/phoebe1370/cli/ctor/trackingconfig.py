#!/usr/bin/env python
# $Id: //prod/main/sarf_centos/testlib/phoebe1350/cli/ctor/trackingconfig.py#2 $
...
class trackingconfig(clictorbase.IafCliConfiguratorBase):
    ...
    def limittrackingevents(self, input_dict=None, **kwargs):
        param_map = clictorbase.IafCliParamMap(end_of_command='Choose the operation')
        param_map['enable_truncate_tracking_events'] = \
                   ['enable Tracking Events to be truncated', DEFAULT]

        param_map.update(input_dict or kwargs)

        self._query_response('LIMITTRACKINGEVENTS')
        return self._process_input(param_map)
