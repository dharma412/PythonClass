#!/usr/bin/env python
# $Id: //prod/main/sarf_centos/testlib/phoebe1350/cli/ctor/clictorbase.py#2 $
...
try:
    # IAF2 import
    import utils.multiprompt as multiprompt
except:
    # SARF alternative
...
default_timeout = 60
ctor_logfile = '{0}/tmp/ctor_{1}.log'.format(os.getenv('SARF_HOME', ''),
                                             uuid.uuid4().hex)
debug = True
...
class IafCliFeatureNotEnabled(IafCliError): pass
class IafCliCtorNotImplementedError(Exception): pass

# These exceptions are temporary defined here since they are usually raised
# by ctor lib.  Once bug 79302 is fixed, these could be included or removed.
class DuplicateEntry(IafCliError): pass
class UnknownOptionError(IafCliError): pass

def get_sess(host=None, dev_mode=False, timeout=30):
    ...
    name = 'admin'
    password = 'ironport'
    log_path = '{0}/tmp/clictorbase_{1}.log'.format(os.getenv('SARF_HOME', ''),
                                                    uuid.uuid4().hex)
    log_file = open(log_path, 'w', 0)
...
# Decorator for the configuration functions of the IafCliConfiguratorBase class
def process_cli(end_of_command, second_level_command):
    # The actual decorator
    def my_decorator(target):
        # The wrapper that actually replaces the target function
        def wrapper(self, input_dict=None, **kwargs):
            self._get_new_param_map(end_of_command)
            target(self, input_dict, **kwargs)
            self.param_map.update(input_dict or kwargs)

            self._query_response(second_level_command)
...
class CallWrapper(type):
    def __init__(cls, name, bases, attrs_dict):
        #=======================================================================
        # Special case of __call__ patching
        # New style classes call this method
        # from a class object instead of an instance object
        #=======================================================================
        super(CallWrapper, cls).__init__(name, bases, attrs_dict)
        if '__call__' in attrs_dict:
            previous_docstr = attrs_dict['__call__'].__doc__
            wrapper_obj = lambda self, *args, **kwargs: \
                cls._exception_wrapper(self, attrs_dict['__call__'],
                                       *args, **kwargs)
            setattr(wrapper_obj , '__name__', '__call__')
            setattr(wrapper_obj, '__doc__', previous_docstr)
            cls.__call__ = wrapper_obj


class IafCliConfiguratorBase(object, Logger):
    __metaclass__ = CallWrapper
    ...
    def __init__(self, sess):
        ...
        self._sub_prompt = re.compile('(\[(.*)]> )|^>|.*:>', re.MULTILINE)
        ...
        self._patch_cli_calls()

    def _patch_cli_calls(self):
        # Monkey patching of public methods
        for name, value in inspect.getmembers(self):
            if not name.startswith('_') and inspect.ismethod(value):
                wrapper_obj = functools.partial(self._exception_wrapper, value)
                setattr(wrapper_obj, '__name__', name)
                setattr(wrapper_obj, '__doc__', value.__doc__)
                setattr(self, name, wrapper_obj)

    def _exception_wrapper(self, method, *args, **kwargs):
        try:
            ...
            try:
                ...
                self._debug('>>> Exception hook of "{0} -> {1}": '\
                      'CLI has been successfully restarted and can '\
                      'execute all the forthcoming commands.'.\
                      format(self.__class__.__name__, method.__name__))
            except Exception as e:
                self._debug('!> {0}'.format(unicode(e)))
                self._debug('>>> Failed to restart CLI session of CLI '\
                      'command "{0} -> {1}". All the forthcoming '\
                      'CLI commands may fail.'.\
                      format(self.__class__.__name__, method.__name__))
            raise exc_info[0], exc_info[1], exc_info[2]

    def __call__(self):
        self._writeln(self._top_level_command)
        return self

    def _set_local_err_dict(self, err_dict, use_global_err_dict=True):
        if use_global_err_dict:
            self._local_err_dict = self._global_err_dict.copy()
        else:
            self._local_err_dict = {}
    ...
    def _get_new_param_map(self, end_of_command):
        """
        Handle setup of param_map, REQUIRED, and DEFAULT.
        """
        global REQUIRED, DEFAULT
        try:
            self.param_map = IafCliParamMap(end_of_command=end_of_command)

        except NameError:
            self.param_map = self.IafCliParamMap(end_of_command=end_of_command)
    ...
    def _query(self, *args, **kwargs):
        ...
        # query _sub_prompt
        if len(args) == 0:
            m = self._expect((self._sub_prompt, expect.REGEX), timeout=timeout)
            self._set_last_default(self._sub_prompt, self._get_expectindex(), m)
            return 0
    ...
    def _process_input(self, param_map, do_restart=True, timeout=5):
    ...
    def _find_answer_key(self, question_text, answer_map):
        ...
        for answer_key in answer_map.keys():
            ...
            if question_text.find(answer_map[answer_key]['text_string']) >= 0:
                if not answer_map.has_been_answered(answer_key):
                    return answer_key
        else:
    ...
    def _set_last_default(self, patt_list, expect_index, expect_match):
        ...
        assert expect_match
        if type(patt_list) is list or type(patt_list) is tuple:
    ...
    def _check_feature(self, feature_name):
        return check_feature(self, feature_name)
...
class IafCliParamMap:
    ...
    def __init__(self, end_of_command=None):
        self._map = sal.containers.odict.odict()
    ...
    def _get_answer_str(self, param_name):
        ...
        assert self._map[param_name]['answer'] != None, \
         "can't find entry in answers map for '" + param_name + "'\n" + \
         " map=" + str(self._map)
...
class IafCliInputListObject:
    ...
    def parse_text(self, text_block):
        ...
        input_list_dict = sal.containers.odict.odict()
        input_list = text_block.split('\n')
        new_list = []
        for line1 , line2 in zip(input_list, input_list[1:]):
            line1 = line1.strip()
            line2 = line2.strip()
            m = re.search('^\s?(\d+[\.|\:|\)|\s] .*)', line1)
            m2 = re.search('(^[a-zA-Z]+.*)',line2)
            if m == None:
                continue
            new_value = m.group(1)
            if m2 != None:
                new_value = m.group(1) + " " + m2.group(1)
        ...
        for line in new_list:
            ...
            if m == None:
                ...
                m = re.search('^(\d+)\) (.*)', line)
                if m == None:
                    # COEUS webcache->ignore->domains-style "input list"
                    m = re.search('^(\d+)\: (.*)', line)
                    if m == None:
