...
class SecureX(NGGuiCommon):
    ...
    def securex_login(self, server, client_id, client_secret):
        ...
        self.wait_for_angular()
        if self._is_visible(SecurexWidget.authenticate_error):
            self._debug('Authenticate Error')
            self.click_element(SecurexWidget.authenticate_cancel)
            raise UserInputError('Unknown Client or Wrong password')
    ...
    def securex_get_active_case_details(self):
        cases = {}
        if self._is_visible(Casebook.case_detail_title):
            cases = {'title': self.get_text(Casebook.case_detail_title),
                     'created': self.get_text(Casebook.case_detail_created),
                     'owner': self.get_text(Casebook.case_detail_owner)}
        return cases
    ...
    def _get_currents_incidents_in_case(self):
        if not self._wait_until_element_is_present(Incident.current_incidents, timeout=10):
            return []
    ...
    def securex_case_delete(self):
        ...
        if self._is_visible(Casebook.delete_case):
            ...
            if not self._wait_until_element_is_present(Casebook.delete_case_confirm_dialog, timeout=5):
                raise ElementNotFoundError('Failed to get Confirm dialog')
            self.click_element(Casebook.delete_case_button)
            self.wait_for_angular()
        else:
    ...
    def securex_settings_ribbon_clear_storage(self):
        """
        This keyword to do the secure x settings reset
        :return:
        """
        self.securex_open_settings()
        if self._is_visible(SecurexWidget.securex_clear_storage):
            self.click_element(SecurexWidget.securex_clear_storage)
            self._wait_until_element_is_present(SecurexWidget.reset_dialog, timeout=10)
            self.click_element(SecurexWidget.reset_confirm)
            self.wait_for_angular()
        else:
            raise ElementNotFoundError('Failed to do securex clear storage')

    def securex_settings_casebook_auto_open(self, enable=True):
        """
    ...
    def _pivot_toolbar_action(self, case_action):
        ...
        if case_action == 'New Case':
            menu_buttons = self.get_element_count(SecurexWidget.pivot_menu_buttons)
            if menu_buttons < 3:
                self.click_button(SecurexWidget.pivot_menu_add_active_case)
            else:
    ...
    def securex_case_pivot_menu(self, title=None, case_action=None):
        ...
        for observable in self.find(SecurexWidget.pivot_menu, first_only=False):
            ...
            status = self._wait_until_element_is_present(SecurexWidget.tippy_popper, timeout=5)
            self._debug("POPPER STATUS :%s" %status)
...
class SecurexWidget:
    ...
    authenticate_error = "//*[@class='glyphicon glyphicon-times-circle ctr-authentication__error-message']"
    ...
    authenticate_cancel = "//*[@translate='buttons.cancel']"
    ...
    securex_clear_storage = "//*[@id= 'ats-security-ribbon-application']/div/div[1]/table/tbody/tr[3]/td[2]/button"
    ...
    pivot_menu_buttons = "//*[@class='ats-ribbon-components__pivot-menu__header__toolbar']/button"
...
class Casebook:
    ...
    delete_case_confirm_dialog = "//*[@class='ats-ribbon-components__modal__header']"
    delete_case_button  = "//*[@class='ats-ribbon-components__modal__footer']/button[text()='Delete']"
    ...
    case_detail_title = "//*[@class='ats-ribbon-components__key-value-table']/div[1]/div[2]/div/div"
    case_detail_created = "//*[@class='ats-ribbon-components__key-value-table']/div[2]/div[2]"
    case_detail_owner = "//*[@class='ats-ribbon-components__key-value-table']/div[3]/div[2]"
