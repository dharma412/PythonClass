...
# $Id: //prod/main/sarf_centos/testlib/phoebe1350/gui/admin/log_subscriptions_def/subscription_files.py#1 $ $DateTime: 2020/01/06 01:25:43 $ $Author: saurgup5 $
...
FILE_DOWNLOAD = lambda row_index: "%s/tbody/tr[%s]/td[1]/a" % \
                                (FILES_TABLE, row_index)
...
class SubscriptionFiles(object):
    ...
    def download_file(self, filename):
        files_count = int(self.gui.get_matching_xpath_count(LOG_ROWS))
        for row_index in xrange(2, files_count + 1):
            if filename == self.gui.get_text(COLUMN_DATA(row_index,1)):
                self.gui.click_element(FILE_DOWNLOAD(row_index), "don't wait")
