...
class URLReputationConditionProperty(ContentFilterProperty):
    ...
    UNTRUSTED_RADIO = "//input[@id='UrlReputationCondition_url_reputation_Untrusted']"
    QUESTIONABLE_RADIO = "//input[@id='UrlReputationCondition_url_reputation_Questionable']"
    NEUTRAL_RADIO = "//input[@id='UrlReputationCondition_url_reputation_Neutral']"
    FAVORABLE_RADIO = "//input[@id='UrlReputationCondition_url_reputation_Favorable']"
    TRUSTED_RADIO = "//input[@id='UrlReputationCondition_url_reputation_Trusted']"
    CUSTOMRANGE_RADIO = "//input[@id='UrlReputationCondition_url_reputation_Custom']"
    UNKNOWN_RADIO = "//input[@id='UrlReputationCondition_url_reputation_Unknown']"
    ...
    def set(self, new_value):
        ...
        *Examples:*
        ...
        converted_dict = dict((k.lower(), v) for k,v in new_value.iteritems())
        if 'url reputation type' in converted_dict.keys():
            value = converted_dict['url reputation type']
        ...
        if 'use a url reputation whitelist' in converted_dict.keys():
            value = converted_dict['use a url reputation whitelist']
            self.gui.select_from_list(self.URL_WHITELIST_REPUTATION_COMBO,value)
        if 'untrusted reputation url' in converted_dict.keys():
            self.gui._click_radio_button(self.UNTRUSTED_RADIO)
        if 'questionable reputation url' in converted_dict.keys():
            self.gui._click_radio_button(self.QUESTIONABLE_RADIO)
        if 'neutral reputation url' in converted_dict.keys():
            self.gui._click_radio_button(self.NEUTRAL_RADIO)
        if 'favorable reputation url' in converted_dict.keys():
            self.gui._click_radio_button(self.FAVORABLE_RADIO)
        if 'trusted reputation url' in converted_dict.keys():
            self.gui._click_radio_button(self.TRUSTED_RADIO)
        if 'unknown reputation url' in converted_dict.keys():
            self.gui._click_radio_button(self.UNKNOWN_RADIO)
        if 'customrange reputation url min value' in converted_dict.keys():
            value = converted_dict['customrange reputation url min value']
            self.gui._click_radio_button(self.CUSTOMRANGE_RADIO)
            self.gui.input_text(self.CUSTOMRANGE_MIN_EDIT,value)
        if 'customrange reputation url max value' in converted_dict.keys():
            value = converted_dict['customrange reputation url max value']
            self.gui._click_radio_button(self.CUSTOMRANGE_RADIO)
            self.gui.input_text(self.CUSTOMRANGE_MAX_EDIT,value)
        if 'scan type' in converted_dict.keys():
            value = converted_dict['scan type']
        ...
        if 'add sources' in converted_dict.keys():
            value = converted_dict['add sources']
            for list in value:
                self.gui.select_from_list(self.AVAILABLE_SOURCE_COMBO,list)
            self.gui.click_button(self.SOURCE_ADD_BUTTON, "don't wait")
        if 'remove sources' in converted_dict.keys():
            value = converted_dict['remove sources']
            for list in value:
...
class StripAttachmentWithMacroProperty(ContentFilterProperty):
    ...
    ADD_BUTTON = "//*[@id='ruleform_StripAttachmentWithMacro']//input[@value='Add >']"
...
class AttachmentFileInfoProperty(ContentFilterProperty):
    ...
    FILEHASH_RADIO = "//input[@id='AttachmentFileInfo_radio_cf_filehash']"
    FILEHASH_OP_COMBO = "//select[@id='AttachmentFileInfo_filehash_options']" 
    ...
    def set(self, new_value):
        ...
        *Parameters:*
        ...
        or
        | File Hash List | <value> |
        <value> is existing File Hash name
        or
        ...
        *Examples:*
        ...
        for key, value in new_value.iteritems():
            ...
            elif key == 'File Hash List':
                self.gui._click_radio_button(self.FILEHASH_RADIO)
                self.gui.select_from_list(self.FILEHASH_OP_COMBO,
                                          value)
...
class ReputationScoreProperty(ContentFilterProperty):
    ...
    NOSCORE_RADIO = "//input[@id='ReputationScore_radio_unknown']"
...
class ForgedEmailDetectionActionProperty(ContentFilterProperty):
    @classmethod
    def get_name(cls):
        return 'Forged Email Detection Action'
    ...
    def set(self, new_value):
        ...
        *Parameters:*
        ...
        Dictionary can contain the next items:
        | Forged Email Detection Action | <value is ignored> |

        | ${new_value}= | Create Dictionary | Forged Email Detection Action |
        | ... | ololo |
        """
        for key, value in new_value.items():
            if key == 'Forged Email Detection Action':
                pass
            else:
                handle_option_not_found_event(key, self.get_name(),
                                              inspect.getdoc(self.set))

class ForgedEmailDetectionConditionProperty(ContentFilterProperty):
    CONTENT_DICTIONARY = "//select[@id='Forged_Email_Detection_content_dict']"
    SIMILARITY_SCORE = "//input[@id='Forged_Email_Detection_threshold']"
    EXCEPTION_LIST = "//select[@id='Forged_Email_Detection_exception_list']"

    @classmethod
    def get_name(cls):
        return 'Forged Email Detection Condition'

    def set(self, new_value):
        for key, value in new_value.items():
            if key == 'Content dictionary':
                self.gui.select_from_list(self.CONTENT_DICTIONARY, value)
            elif key == 'Similarity Score':
                self.gui.input_text(self.SIMILARITY_SCORE, value)
            elif key == 'Exception List':
                self.gui.select_from_list(self.EXCEPTION_LIST, value)
            else:
                handle_option_not_found_event(key, self.get_name(),
                                              inspect.getdoc(self.set))
...
class StripAttachmentByFileInfoProperty(ContentFilterProperty):
    ...
    FILEHASH_RADIO = "//input[@id='StripAttachmentByFileInfo_radio_cf_filehash']"
    FILEHASH_OP_COMBO = "//select[@id='StripAttachmentByFileInfo_filehash_options']"
...
class URLReputationActionProperty(ContentFilterProperty):
    ...
    UNTRUSTED_RADIO = "//input[@id='UrlReputationAction_url_reputation_Untrusted']"
    QUESTIONABLE_RADIO = "//input[@id='UrlReputationAction_url_reputation_Questionable']"
    NEUTRAL_RADIO = "//input[@id='UrlReputationAction_url_reputation_Neutral']"
    FAVORABLE_RADIO = "//input[@id='UrlReputationAction_url_reputation_Favorable']"
    TRUSTED_RADIO = "//input[@id='UrlReputationAction_url_reputation_Trusted']"
    CUSTOMRANGE_RADIO = "//input[@id='UrlReputationAction_url_reputation_Custom']"
    UNKNOWN_RADIO = "//input[@id='UrlReputationAction_url_reputation_Unknown']"
        ...
        *Examples:*
        ...
        for key, value in new_value.items():
            if  key.lower() == 'url reputation type':
                ...
                self.gui.select_from_list(self.URL_WHITELIST_REPUTATION_COMBO,value)
            elif key.lower() == 'untrusted reputation url':
                self.gui._click_radio_button(self.UNTRUSTED_RADIO)
            elif key.lower() == 'questionable reputation url':
                self.gui._click_radio_button(self.QUESTIONABLE_RADIO)
            elif key.lower() == 'neutral reputation url':
                self.gui._click_radio_button(self.NEUTRAL_RADIO)
            elif key.lower() == 'favorable reputation url':
                self.gui._click_radio_button(self.FAVORABLE_RADIO)
            elif key.lower() == 'trusted reputation url':
                self.gui._click_radio_button(self.TRUSTED_RADIO)
            elif key.lower() == 'unknown reputation url':
                self.gui._click_radio_button(self.UNKNOWN_RADIO)
                ...
                self.gui._is_element_present(self.ALL_MESSAGES_REPUTATION_RADIO)
                self.gui.mouse_over(self.ALL_MESSAGES_REPUTATION_RADIO)
                ...
                self.gui._is_element_present(self.UNSIGNED_MESSAGES_REPUTATION_RADIO)
                self.gui.mouse_over(self.UNSIGNED_MESSAGES_REPUTATION_RADIO)
