#!/usr/bin/env python
# $Id: //prod/main/sarf_centos/testlib/phoebe1350/gui/help/technical_support_def/contact_techsupport_settings.py#1 $ $DateTime: 2020/01/06 01:25:43 $ $Author: saurgup5 $
...
CONTRACT_UID= ('Contact Information userid', "//*[@id='new_cco_name_id']")
...
VALUE_ELEMENT = lambda subgroups_locator, subgroup_idx, value_idx: \
          "%s[%s]//option[%s]" % (subgroups_locator, subgroup_idx, value_idx)
CONTRACT_VALUE = ('Contract ID', "//input[@id='contract_number_id']")
    ...
    def _get_all_available_problem_code_options(self):
        ...
        for subgroup_idx in xrange(1, all_subgroups_count + 1):
            ...
            for value_idx in xrange(first_value_idx, all_values_count + 1):
                value_element = self.gui.get_element_attribute(VALUE_ELEMENT(subgroups_locator, subgroup_idx, value_idx), 'value')
