# $Id: //prod/main/sarf_centos/testlib/zeus1380/gui/management/services/security_appliances.py#1 $
...
APPLIANCES_STATUS_TEXT = lambda appl_type, row:    "//div[@class='group']/s[contains(text(), '%s')]"    "/following::div[@class='text-info'][%s]" % (appl_type, row)
...
EMAIL_APPL_TABLE_PREFIX = "//div[@class='group']/s[contains(text(), 'Email')]"    "/following::"

ADD_EMAIL_APPL_BUTTON = 'AddEmailAppliance'

DELETE_EMAIL_APPL_TABLE_CELL = lambda row: EMAIL_APPL_TABLE_PREFIX +     "table[following-sibling::div[@class='group']][1]//tr[%s]/td[8]/img" % (row,)

EDIT_EMAIL_APPL_TABLE_CELL = lambda row: EMAIL_APPL_TABLE_PREFIX +     "table[following-sibling::div[@class='group']][1]//tr[%s]/td/a" % (row,)
...
WEB_APPL_TABLE_PREFIX = "//div[@class='group']/s[contains(text(), 'Web')]"    "/following::"

ADD_WEB_APPL_BUTTON = 'AddWebAppliance'

DELETE_WEB_APPL_TABLE_CELL = lambda row: WEB_APPL_TABLE_PREFIX +     "table[not(following-sibling::div[@class='group'])][1]//tr[%s]/td[9]/img" % (row,)

EDIT_WEB_APPL_TABLE_CELL = lambda row: WEB_APPL_TABLE_PREFIX +     "table[not(following-sibling::div[@class='group'])][1]//tr[%s]/td[1]/a" % (row,)
...
WEB_CONFIRM_ICCM_DISABLE_BUTTON =    "//button[@type='button' and contains(text(), 'Ok')]"
...
class SecurityAppliances(GuiCommon):
    ...
    def _check_appliances_status(self, appl_type, services=False,
        ...
        services_disabled_msg = 'No centralized services are currently'            ' available'
    ...
    def _get_appliance_data(self, appl_type, name):
        ...
        _APPLIANCE_ROW = _TABLE_LOC + "//tr/td[1]/a[normalize-space(text())='" +                          name + "']/parent::*/parent::*"
