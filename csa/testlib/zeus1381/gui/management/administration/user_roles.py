# $Id: //prod/main/sarf_centos/testlib/zeus1381/gui/management/administration/user_roles.py#3 $  $DateTime: 2020/08/10 11:37:41 $ $Author: sumitada $
...
ENABLE_LOG_SUBSCRIPTIONS = 'id=logunrestricted'
DISABLE_LOG_SUBSCRIPTIONS = 'id=lognone'
...
class UserRoles(GuiCommon):
    ...
    def _fill_email_user_role_page(self, name, description, reporting_access,
        report_type, tracking_access, quarantine_access, logsubscription_access):
        if name is not None:
            self._fill_role_name_textbox(name)
        ...
        if logsubscription_access is not None:
            self._select_logsubscription_access(logsubscription_access)
    ...
    def _select_logsubscription_access(self, access):

         logsubscription_locators = {
             True: ENABLE_LOG_SUBSCRIPTIONS,
             False: DISABLE_LOG_SUBSCRIPTIONS}
         self._click_radio_button(logsubscription_locators[access])
    ...
    def user_roles_email_role_add(self, name, description=None,
        ...
        quarantine_access=None, quarantine_type="spam", logsubscription_access=None):
        ...
        - `quarantine_access`: enable access privileges for spam quarantine.
           Boolean.
        - `logsubscription_access`: enable access privileges for
        ...
        | User Roles Email Role Add | emailrole | reporting_access=${False} |^M
        | ... | tracking_access=${False} | logsubscription_access=${True}
        ...
        #self._open_page(check_email_table=True)
        if quarantine_access == True:
            start_row = 3
            num_of_columns = 8
            table = EMAIL_ROLES_TABLE
            extraopt = '/img'
            num_of_rows = int(self.get_matching_xpath_count(USER_ROLE_TABLE_CELL(table, '*', num_of_columns, extraopt)))
            for row in xrange(start_row, start_row + num_of_rows):
                role_name = self.get_text(USER_ROLE_TABLE_CELL(table, row, 1))
                if role_name == name:
                    user_quarantine = USER_ROLE_TABLE_CELL(table, row, 2)
                    quarantine_link = user_quarantine + '/a'
                    self.click_element(quarantine_link)
                    quarantine_locators = {
                        'spam' : '//input[@name="include_isq"]',
                        'file_analysis' : '//input[@value="File Analysis"]',
                        'outbreak' : '//input[@value="Outbreak"]',
                        'policy'  : '//input[@value="Policy"]',
                        'unclassified' : '//input[@value="Unclassified"]',
                        'virus' : '//input[@value="Virus"]',
                    }
                    self.select_checkbox(quarantine_locators[quarantine_type])
                    self._click_submit_button()
    ...
    def user_roles_email_role_edit(self, name, new_name=None, description=None,
        ...
        self._fill_email_user_role_page(new_name, description,
            reporting_access, report_type, tracking_access, quarantine_access, logsubscription_access)
    ...
    def user_roles_email_role_duplicate(self, name, new_name=None, description=None,
        reporting_access=None, report_type=None, tracking_access=None,
        quarantine_access=None, logsubscription_access=None):
