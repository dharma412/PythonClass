# $Id: //prod/main/sarf_centos/testlib/zeus1380/gui/email/quarantine/spam_quarantine.py#1 $ $DateTime: 2020/05/25 00:19:30 $ $Author: sarukakk $
...
ADVANCED_SEARCH_LINK = '//a[contains(text(),"Advanced Search")]'
SEARCH_BUTTON = '//input[@value=\'Search\' and @type=\'submit\']'
...
TODAY_RANGE_RADIO ='//input[@id=\'date1\']'
WEEK_RANGE_RADIO = '//input[@id=\'date2\']'
DATE_RANGE_RADIO = '//input[@id=\'date3\']'
...
MSG_MID_COLUMN = lambda row: '%s[%s]/td[1]/div/input' % (MSG_INFO_ROW, row)
...
class SpamQuarantine(GuiCommon):
    ...
    def get_keyword_names(self):
        return ['spam_quarantine_search_page_open',
                ...
                'spam_quarantine_release_safelist_messages']
    ...
    def _select_date_range(self, date_range):
        ...
        if isinstance(date_range, basestring):
            if date_range in date_range_map:
                self.click_element(date_range_map[date_range])
        elif isinstance(date_range, tuple):
            self.click_element(DATE_RANGE_RADIO)
            if len(date_range) != 2:
    ...
    def _user_release_safelist_action(self):
        self.select_from_list(USER_ACTIONS_LIST, '-- Release and Add to Safelist')
        self.click_button(USER_SUBMIT_BUTTON, 'don\'t wait')
        self.capture_page_screenshot("safelist_screenshot")
        self.click_button(CONFIRM_BUTTON)
        self.capture_page_screenshot("safelist_screenshot")
    ...
    def _search_msgs(self, date_range, header_name, header_cmp, header_value,
        ...
        self._clear_search()
        self.capture_page_screenshot("screenshot")
        ...
        if self._is_element_present(SEARCH_BUTTON):
    ...
    def _get_all_msgs_on_current_page(self, columns):
        ...
        for row in xrange(1, rows_num + 1):
            msg_dict = {}
            msg_dict['mid'] = self.get_element_attribute("xpath=" +MSG_MID_COLUMN(row),"value")
    ...
    def _do_isq_action(self, action_method):
        for i in xrange(100):
            ...
            try:
                ...
                self._info('Action already checked')
                break
            action_method()
    ...
    def spam_quarantine_release_safelist_messages(self,
                                        is_admin=False,
                                        date_range=None,
                                        header_name=None,
                                        header_cmp=None,
        ...
        release_action = self._user_release_safelist_action
        self._do_isq_action(release_action)
