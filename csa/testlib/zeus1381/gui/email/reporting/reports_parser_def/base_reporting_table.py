# $Id: //prod/main/sarf_centos/testlib/zeus1381/gui/email/reporting/reports_parser_def/base_reporting_table.py#1 $
...
class BaseReportingTable(object):
    ...
    EXPORT_LINK_DEST_URL = lambda self: "{0}".format(self.EXPORT_LINK())
    ...
    @set_speed(0, 'gui')
    def export_as(self, dest_path=None, username=DUT_ADMIN, password=DUT_ADMIN_PASSWORD):
        ...
        link = self.gui.get_element_attribute(self.EXPORT_LINK_DEST_URL(),"onclick")
        link = re.search(r'SectionExport\(\'([^\']+)', link).groups()[0]
        print '>>> export link: {0}'.format(link)
        #file_in_memory = download_file_from_dut(self.gui.dut, link, username, password)
        _title = self.gui.get_text("xpath=//*[@id='report_title']/h1").strip()
        _start_time=time.strftime("%Y-%m-%d %H:%M:%S")
        self.gui.click_element(self.EXPORT_LINK_DEST_URL(), 'don\'t wait')
        download_dir = os.path.join(os.getenv("SARF_HOME"), "tmp")
        filename = WaitForDownload(self.gui.dut).wait_for_download(_title,\
                   start_time=_start_time, timeout=120, download_directory=download_dir)
        file_in_memory = open(filename,'rb+')
        try:
