#!/usr/bin/env python
# $Id: //prod/main/sarf_centos/testlib/zeus1380/cli/ctor/delivernow.py#1 $

"""
    IAF 2 CLI ctor - delivernow
"""


import clictorbase

from sal.exceptions import ConfigError


class delivernow(clictorbase.IafCliConfiguratorBase):

    def __call__(self, how='all', host=None):
        if how == 'host' and host is None:
            raise ConfigError('Hostname to deliver messages to must be '\
                              'specified')

        self._sess.writeln('delivernow')
        self._query_select_list_item(how, timeout=30)
        if how == 'host':
            self._query_response(host)
        # Not sure if this is needed
        self._wait_for_prompt()

if __name__ == '__main__':
    # if cli_sess is already defined, we have a CLI session object from
    # the validator we can use; if not, we must create one
    try:
        cli_sess
    except NameError:
        cli_sess = clictorbase.get_sess()

    dn = delivernow(cli_sess)
    dn(how='host', host='qa04.qa')
    dn()
