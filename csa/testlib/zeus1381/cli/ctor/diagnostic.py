#!/usr/bin/env python
# $Id: //prod/main/sarf_centos/testlib/zeus1350/cli/ctor/diagnostic.py#1
...
class diagnostic(clictorbase.IafCliConfiguratorBase):
    ...
    def reload(self, sure=NO,wipedata=NO):
        newlines=2
        ...
        self._writeln(sure)
        if is_no(sure):
            curr_status = self._read_until('Choose the operation you want')

        # if previous answer is yes, confirmation will be asked one more time and proceed to wipe data option
        if is_yes(sure) and\
           self._query('*really* sure you want to continue') == 0:
             self._writeln(sure)
             self._query('Do you want to wipe also')
             self._writeln(wipedata)
             curr_status = self._read_until('Reverting to "testing" preconfigure install mode.')

        return curr_status

    def services(self):
        self._query_response('services')
        return diagnosticServices(self._get_sess())
...
class diagnosticServices(clictorbase.IafCliConfiguratorBase):
    newlines = 1
    def reporting(self,operation,timeout=60):
        """Reporting associated services"""
        self._query_response('reporting')
        self._read_until("Choose the operation")
        self._query_response(operation)
        self._expect('\n')
        curr_status = self._read_until("Choose one of the following")
        return curr_status

    def tracking(self,operation,timeout=60):
        """Tracking associated services"""
        self._query_response('tracking')
        self._read_until("Choose the operation")
        self._query_response(operation)
        self._expect('\n')
    ...
    def euqweb(self,operation,timeout=60):
        """End User Quarantine GUI"""
        self._query_response('euqweb')
        self._read_until("Choose the operation")
        self._query_response(operation)
        self._expect('\n')
        curr_status = self._read_until("Choose one of the following")
    ...
    def webui(self,operation,timeout=60):
        """Web GUI"""
    ...
    def smart_license(self,operation,timeout=60):
        """Smart Licensing Agent"""
        self._query_response('smart_license')
        self._read_until("Choose the operation")
        self._query_response(operation)
        self._expect('\n')
        curr_status = self._read_until("Choose one of the following")
...
if __name__ == '__main__':
    ...
    ctor().services().reporting()
    ctor().services().tracking()
    ctor().services().euqweb()
    ctor().services().webui()
    ctor().services().smart_license()