#!/usr/bin/env python
# $Id: //prod/main/sarf_centos/testlib/phoebe1350/cli/keywords/listenerconfig.py#1 $ $DateTime: 2020/01/06 01:25:43 $ $Author: saurgup5 $
...
class Listenerconfig(CliKeywordBase):
    ...
    def get_keyword_names(self):
        return ['listenerconfig_new',
                ...
                'listenerconfig_edit_hostaccess_edit_sendergroup_ext_threat_feeds_src_add',
                'listenerconfig_edit_hostaccess_edit_sendergroup_ext_threat_feeds_src_delete',
                'listenerconfig_edit_hostaccess_edit_sendergroup_ext_threat_feeds_src_print',
    ...
    def listenerconfig_new(self, *args):
        ...
        *Parameters*
        ...
        - `sdr_check`: Enable Sender Domain Reputation
        - `virus_outbreak_filters` : Enable Virus Outbreak Filters. Either 'yes' or 'no'
        - `advanced_phishing_protection` : Would you like to enable Advanced Phishing Protection scanning. Either 'yes' or 'no'  
        - `graymail_detection` : Enable Graymail Detection. Either 'yes' or 'no'
        - `content_filters` : Would you like to enable Content Filters. Either 'yes' or 'no'
        - `message_filters` : Would you like to enable Message Filters. Either 'yes' or 'no'
    ...
    def listenerconfig_edit_hostaccess_edit_sendergroup_ext_threat_feeds_src_add(self, intf_name, name, source_name):
        """
        This is used to move a host's position in the sender group in the Host
        Access Table

        listenerconfig -> edit -> hostaccess -> edit -> sendergroup -> External Threat Feeds source -> add

        *Parameters*
        - `intf_name`: Name of the listener interface
        - `name`: Name for this sender group
        - `source_name`: List of the sources to be added.
              Values- index, index range, sources name(s) or combination of all.

        *Examples*
        | Listenerconfig Edit Hostaccess Edit Sendergroup Ext Threat Feeds Src Add |
        | InBoundMail | UNKNOWNLIST | 1 |
        | Listenerconfig Edit Hostaccess Edit Sendergroup Ext Threat Feeds Src Add |
        | InBoundMail | UNKNOWNLIST | 1-10 |
        | Listenerconfig Edit Hostaccess Edit Sendergroup Ext Threat Feeds Src Add |
        | InBoundMail | UNKNOWNLIST | hailataxii, aleinvault |
        | Listenerconfig Edit Hostaccess Edit Sendergroup Ext Threat Feeds Src Add |
        | InBoundMail | UNKNOWNLIST | 1-10,ailenvault |
        """
        self._cli.listenerconfig().edit(name=intf_name).hostaccess().\
             edit('sender group', name, None).externalthreatfeeds().add(source_name)

    def listenerconfig_edit_hostaccess_edit_sendergroup_ext_threat_feeds_src_delete(self, intf_name, name, source_name):
        """
        This is used to move a host's position in the sender group in the Host
        Access Table

        listenerconfig -> edit -> hostaccess -> edit -> sendergroup -> External Threat Feeds Source -> delete

        *Parameters*
        - `intf_name`: Name of the listener interface
        - `name`: Name for this sender group
        - `source_name`: range or indices of the external threat feed sources to be added.

        *Examples*
        | Listenerconfig Edit Hostaccess Edit Sendergroup Ext Threat Feeds Src Delete |
        | InBoundMail | UNKNOWNLIST | 1 |
        | Listenerconfig Edit Hostaccess Edit Sendergroup Ext Threat Feeds Src Delete |
        | InBoundMail | UNKNOWNLIST | 1-4 |
        | Listenerconfig Edit Hostaccess Edit Sendergroup Ext Threat Feeds Src Delete |
        | InBoundMail | UNKNOWNLIST | hailataxii, alienvault |
        | Listenerconfig Edit Hostaccess Edit Sendergroup Ext Threat Feeds Src Delete |
        | InBoundMail | UNKNOWNLIST | 1-7, hailataxii |
        """
        self._cli.listenerconfig().edit(name=intf_name).hostaccess().\
             edit('sender group', name, None).externalthreatfeeds().delete(source_name)

    def listenerconfig_edit_hostaccess_edit_sendergroup_ext_threat_feeds_src_print(self, intf_name, name):
        """
        This is used to move a host's position in the sender group in the Host
        Access Table

        listenerconfig -> edit -> hostaccess -> edit -> sendergroup -> external threat feeds source -> print
        ...
        *Examples*
        | ${countries}= | Listenerconfig Edit Hostaccess Edit Sendergroup Ext Threat Feeds Src Print |
        | InBoundMail | UNKNOWNLIST |
        """
        return self._cli.listenerconfig().edit(name=intf_name).hostaccess().\
             edit('sender group', name, None).externalthreatfeeds().Print()

    def listenerconfig_edit_hostaccess_edit_sendergroup_policy(self, intf_name,
             name, access_behavior, change_policy_parameters_bool, hat_params):
        """
