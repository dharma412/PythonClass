#! /usr/bin/env python
# $Id: //prod/main/sarf_centos/testlib/phoebe1350/cli/ctor/snmpconfig.py#1 $
...
class snmpconfig(clictorbase.IafCliConfiguratorBase):
    ...
    def setup(self, input_dict=None, **kwargs):
        ...
        param_map['ip_interface'] = ['Choose an IP interface for SNMP requests.', DEFAULT, True]
        param_map['snmp_port'] = ['Which port shall the SNMP daemon listen on', DEFAULT]
        param_map['snmpv3_security_level'] = ['SNMPv3 security level', DEFAULT]
        param_map['snmpv3_auth_type'] = ['SNMPv3 authentication type', DEFAULT]
        param_map['snmpv3_privacy_protocol'] = ['SNMPv3 privacy protocol', DEFAULT]
        param_map['snmpv3_passphrase'] = ['SNMPv3 authentication passphrase', [REQUIRED, REQUIRED]]
        param_map['snmpv3_privacy_passphrase'] = ['SNMPv3 privacy passphrase', [REQUIRED, REQUIRED]]
        param_map['set_other_passwords'] = ['set other passwords', NO]
        ...
        param_map['snmpv1v2_request'] = ['Shall SNMP V2c requests be serviced from IPv4 addresses?', DEFAULT]
        param_map['snmpv1v2_network'] = ['From which IPv4 networks shall SNMP V1/V2c requests be allowed?', DEFAULT]
        param_map['snmpv1v2_request_ipv6'] = ['Shall SNMP V2c requests be serviced from IPv6 addresses?', DEFAULT]
        param_map['snmpv1v2_network_ipv6'] = ['From which IPv6 networks shall SNMP V1/V2c requests be allowed?', DEFAULT]
        param_map['snmp_trap_version'] = ['the version for SNMP traps', DEFAULT]
        ...
        param_map['conn_fail_url'] = ['enter the URL to check for connectivity failure', DEFAULT]
        param_map['mem_util_threshold'] = ['threshold would you like to set for memory utilization', DEFAULT]
...
if __name__ == '__main__':
    ...
    sc().setup(input_dict={'enable_snmp': YES,
			   ...
			   'snmpv3_privacy_passphrase': ['ironport', 'ironport']})
    sc().setup(input_dict={'enable_snmp': YES,
                           'snmp_port': '161',
			   'snmpv3_auth_type':'MD5',
