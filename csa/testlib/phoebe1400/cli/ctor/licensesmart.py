#!/usr/bin/env python
# $Id: //prod/main/sarf_centos/testlib/phoebe1350/cli/ctor/licensesmart.py#1 $ $DateTime: 2020/01/06 01:25:43 $ $Author: saurgup5 $
...
class licensesmart(ccb.IafCliConfiguratorBase):
    ...
    def enable(self, enable_sl_on_all_machines_in_cluster):
        self._query_response('ENABLE')
        self._expect(['Do you want to enable Smart Software Licensing for all',
                      'Type "Y" if you want to continue, or type "N"',], timeout=5)
        if self._expectindex == 0:
            self._query_response(enable_sl_on_all_machines_in_cluster)
    ...
    def register(self, input_dict=None, **kwargs):
        ...
        param_map['register_sl_on_all_machines_in_cluster'] = [
            'register Smart Software Licensing across machines', REQUIRED]
    ...
    def _status_parse_auth_register_renewal_date(self):
        ...
        if 'no communication attempted' not in self.smart_licensing_status['last authorization renewal attempt status']:
            self.smart_licensing_status.update({'last authorization renewal attempt date':''})
            last_auth_renew = self.smart_licensing_status['last authorization renewal attempt status']
            auth_renew_status, auth_renew_date = re.split(r'on', last_auth_renew)
            self.smart_licensing_status['last authorization renewal attempt status'] = auth_renew_status.strip()
