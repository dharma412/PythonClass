...
import clictorbase
from clictorbase import IafCliConfiguratorBase, DEFAULT, REQUIRED, \
...
class scanconfig(clictorbase.IafCliConfiguratorBase):
    ...
    def protected_attachment_config(self, input_dict=None, **kwargs):
        ...
        return scanconfigProtectedAttachementConfig(self._get_sess())


class scanconfigProtectedAttachementConfig(clictorbase.IafCliConfiguratorBase):
    _newline = 2
    
    def config(self, input_dict=None, **kwargs):
        # Just come out of PROTECTEDATTACHMENTCONFIG subprompt
        self._to_the_top(2)
    ...
    def status(self, **kwargs):
        ...
        output = self.getbuf()
        if output:
            key, value = re.search(r'(Scanning of .* for inbound mails):\s+(disabled|enabled)', output,
                                   re.MULTILINE).groups()
            status[key] = value
            key, value = re.search(r'(Scanning of .* for outbound mails):\s+(disabled|enabled)', output,
                                   re.MULTILINE).groups()
            status[key] = value
        return status

    def new_password(self, input_dict=None, **kwargs):
        param_map = IafCliParamMap(end_of_command='User Defined Passwords')
        param_map['priority'] = ['Enter a priority for the new password', DEFAULT]
        param_map['password'] = ['Enter the new password', REQUIRED]
        param_map.update(input_dict or kwargs)

        self._query_response('NEW')
        self._process_input(param_map)
    
    def edit_password(self, input_dict=None, **kwargs):
        param_map = IafCliParamMap(end_of_command='User Defined Passwords')
        param_map['priority_or_password_to_edit'] = [
            'Enter the priority or the password that you want to edit', DEFAULT]
        param_map['new_priority'] = ['Enter the new priority', DEFAULT]
        param_map['new_password'] = ['Enter the new password', DEFAULT]
        param_map.update(input_dict or kwargs)

        self._query_response('EDIT')
        self._process_input(param_map)

    def swap_password(self, input_dict=None, **kwargs):
        param_map = IafCliParamMap(end_of_command='User Defined Passwords')
        param_map['first_password_priority'] = [
            'Enter the priority of the first password that you want to switch', REQUIRED]
        param_map['second_password_priority'] = [
            'Enter the priority of the second password that you want to switch', REQUIRED]
        param_map.update(input_dict or kwargs)

        self._query_response('SWAP')
        self._process_input(param_map)

    def delete_password(self, input_dict=None, **kwargs):
        param_map = IafCliParamMap(end_of_command='User Defined Passwords')
        param_map['password_priority'] = [
            'Enter the priority of the password that you want to delete', REQUIRED]
        param_map.update(input_dict or kwargs)
    ...
    def print_password(self):
        ...
        output = self.getbuf()
        if re.search(
            r'Priority:\s+Password:\s+-+\s+-+\s+(.*)\n.*User Defined Passwords', output, re.DOTALL):
            result = re.search(
                r'Priority:\s+Password:\s+-+\s+-+\s+(.*)\n.*User Defined Passwords', output, re.DOTALL).group(1)
            for line in result.strip().split('\r\n'):
                passwords.append({'priority': line.split()[0], 'password': line.split()[1]})
