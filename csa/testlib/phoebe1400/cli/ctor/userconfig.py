#!/usr/bin/env python
# $Id: //prod/main/sarf_centos/testlib/phoebe1350/cli/ctor/userconfig.py#1 $
...
class userconfig(clictorbase.IafCliConfiguratorBase):
    ...
    class SequentialCharacterError(IafCliError): pass
    class UsernameVariationError(IafCliError): pass
    ...
    def __init__(self, sess):
        ...
        self._set_local_err_dict({
             ...
             ('must not contain three or more repetitive or sequential characters', EXACT) : self.SequentialCharacterError,
             ('not be a username or one of its variations', EXACT) : self.UsernameVariationError,
             })

    def __call__(self, batch_mode=False):
        if not batch_mode:
    ...
    def batch_add(self, user_name, full_name, group, password, batch_mode=False):
        """
        userconfig add
        """

        batch_cmd = 'userconfig add'
        params = [user_name, full_name, group, password]
        for param in params:
            batch_cmd += ' ' + param
    ...
    def delete(self, admin_passphrase=None, user_name=REQUIRED, confirm=DEFAULT, batch_mode=False):
        if not batch_mode:
            admin_passphrase = admin_passphrase or DUT_ADMIN_SSW_PASSWORD
            ...
            batch_cmd = 'userconfig delete ' + user_name
...
class userconfigExternal(clictorbase.IafCliConfiguratorBase):
    ...
    def devopssetup(self, input_dict=None, **kwargs):
        ...
        param_map['role'] = ['Assign a role to', DEFAULT]
        param_map['group_attribute'] = ['group attribute(s) to be matched with the saml', DEFAULT]
        param_map['sso_string'] = ['toggle string(s) for Dev Ops SSO', DEFAULT]
        param_map.update(input_dict or kwargs)
        return self._process_input(param_map)
...
class userconfigRole(clictorbase.IafCliConfiguratorBase):
	...
	param_map['reports'] = ['reports', DEFAULT, True]
        ...
        param_map['logs'] = ['logs', DEFAULT, True]
        param_map.update(input_dict or kwargs)
        self._query_response('NEW')
        return self._process_input(param_map)
	...
	param_map['new_name'] = ['new name for the user', DEFAULT]
        ...
        param_map['trace'] = ['trace', DEFAULT, True]
        param_map['quarantines'] = ['quarantines', DEFAULT, True]
        param_map['logs_type'] = ['type of access to logs?', DEFAULT, True]
        param_map.update(input_dict or kwargs)
        self._query_response('EDIT')
        return self._process_input(param_map)
...
class userconfigPolicy(clictorbase.IafCliConfiguratorBase):
    ...
    def passwordstrength(self, input_dict=None, **kwargs):
        ...
        param_map['sequential_characters'] = ['Reject passphrases that contain three or more repetitive or sequential', DEFAULT]
