...
# $Id: //prod/main/sarf_centos/testlib/phoebe1350/gui/manager/hat.py#1 $
...
IMPORT_HAT_BUTTON = '//input[@value="Import HAT..."]'
IMPORT_HAT_FILENAME = '//select[@name="file_server"]'
IMPORT_HAT_CONFIRM_BUTTON = '//button[contains(text(), "Import")]'
IMPORT_HAT_CANCEL_BUTTON = '//button[contains(text(), "Cancel")]'
EXPORT_HAT_BUTTON = '//input[@value="Export HAT..."]'
EXPORT_HAT_FILENAME = '//input[@name="file_server"]'
EXPORT_HAT_OVERWRITE_BUTTON = '//button[contains(text(), "Overwrite")]'
EXPORT_HAT_CANCEL_BUTTON = '//button[contains(text(), "Cancel")]'
...
class hat(AccessTableBase):
    ...
    def _get_senders(self):
        ...
        for row in range(2, rows + 1):
            ...
            for col_num, dict_key in enumerate(sender_dict_keys):
                col_data = self.get_text("%s/tbody/tr[%s]/td[%d]" % (SENDER_TABLE, row, col_num + 1))
                sender_dict[dict_key] = col_data.encode('utf-8')
    ...
    @go_to_access_table(HAT_PAGE_PATH, LISTENER_OPTS)
    def hat_export(self, listener, filename, overwrite=True):
        ...
        *Examples:*
        | Hat Export | Inbound | inbound_hat_settings |
        """
        self.click_button(EXPORT_HAT_BUTTON)
        self.input_text(EXPORT_HAT_FILENAME, filename)
        self._click_submit_button()
        if self._is_element_present(EXPORT_HAT_OVERWRITE_BUTTON):
            if overwrite:
                self.click_button(EXPORT_HAT_OVERWRITE_BUTTON, "don't wait")
            else:
                self.click_button(EXPORT_HAT_CANCEL_BUTTON, "don't wait")

    @go_to_access_table(HAT_PAGE_PATH, LISTENER_OPTS)
    def hat_import(self, listener, filename, cancel_import=False):
        ...
        *Examples:*
        | Hat Import | Inbound | inbound_hat_settings |
        | Hat Import | Inbound | inbound_hat_settings | cancel_import=${True} |
        """
        self.click_button(IMPORT_HAT_BUTTON)
        self.select_from_list(IMPORT_HAT_FILENAME, filename)
        self.click_button(SUBMIT_BUTTON, "don't wait")
        if self._is_element_present(IMPORT_HAT_CONFIRM_BUTTON):
            if not cancel_import:
                self.click_button(IMPORT_HAT_CONFIRM_BUTTON)
            else:
                self.click_button(IMPORT_HAT_CANCEL_BUTTON, "don't wait")
