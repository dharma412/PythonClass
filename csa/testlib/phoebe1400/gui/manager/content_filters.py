...
# $Id: //prod/main/sarf_centos/testlib/phoebe1350/gui/manager/content_filters.py#1 $
...
CONDITION_LOGIC_ANY_OR_ALL_SELECT = "//select[@name='conditionLogicAnyOrAll']"
...
class ContentFilters(GuiCommon):
    ...
    def get_keyword_names(self):
        return ['content_filter_add',
                ...
                'get_filter_condition_page_text',
    ...
    def content_filter_add(self, filter_type, name, description=None,
        ...
        *Examples:*
        ...
        if rule:
            self.select_from_list(CONDITION_LOGIC_ANY_OR_ALL_SELECT, rule)
    ...
    @go_to_filter
    def get_filter_condition_page_text(self, filter_type, name):
        """ Validates the text added in the DomainReputation Child Page

        *Parameters:*       
        - `filter_type': Type of filter: Incoming or Outgoing
        - `text_xpath': xpath of the text
        - `condition_xpath': xpath of the condition available in the list
        - `sub_condition' : xpath of sub-condition if any
        - `category: category of the selectors
        """
        DomainReputation_condition_xpath= '//*[@id="tab_DomainReputation"]/a'
        DomainReputation_childPage_xpath= "//input[@id='DomainReputation_dr_sdr_child1_age']"
        text_xpath= '//*[@id="error_field_DomainReputation_domain_age_msg"]/div[1]'
        self.click_button(ADD_FILTER)
        self.input_text(FILTER_NAME, name)
        self.click_button(ADD_CONDITION)
        self.click_button(DomainReputation_condition_xpath)
        self._click_radio_button(DomainReputation_childPage_xpath)
        return self.get_text(text_xpath)
