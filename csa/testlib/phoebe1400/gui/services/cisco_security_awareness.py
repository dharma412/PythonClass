...
from common.gui.guiexceptions import ConfigError, GuiValueError, GuiError
...
UPDATE_LIST = "//input[@value='Update List' and @type='button']"
...
class  CiscoSecurityAwarwness(GuiCommon):
    ...
    def csa_repeat_clickers_update(self):
        if self._is_element_present(UPDATE_LIST):
            self.click_button(UPDATE_LIST)
        else:
            raise guiexceptions.GuiError('could not find "Update List" button')
