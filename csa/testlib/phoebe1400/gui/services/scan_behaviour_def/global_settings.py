...
PWD_PROTECTED_ATTACHMENTS_PROBABLE_PASSWORD_PRIORITY = lambda index: \
    '//input[@id="user_passwords[%s][Pri]"]' % index
PWD_PROTECTED_ATTACHMENTS_PROBABLE_PASSWORD_VALUE = lambda index: \
    '//input[@id="user_passwords[%s][Pswd]"]' % index
PWD_PROTECTED_ATTACHMENTS_PROBABLE_PASSWORD_ADD_BUTTON = '//input[@id="user_passwords_domtable_AddRow"]'
PWD_PROTECTED_ATTACHMENTS_PROBABLE_PASSWORD_DELETE_BUTTON = lambda index: '//tr[@id="user_passwords_row%s"]/td[3]/img' % index
PWD_PROTECTED_ATTACHMENTS_PROBABLE_PASSWORD_COMBO = ('Probable Password for Analysis', None)
PWD_PROTECTED_ATTACHMENTS_PROBABLE_PASSWORD_VALUE_REGEX = 'user_passwords\[(\d+)\]\[Pswd\]'
PWD_PROTECTED_ATTACHMENTS_PROBABLE_PASSWORD_VALUE_XPATH = lambda index: 'user_passwords[%s][Pswd]' % index
PWD_PROTECTED_ATTACHMENTS_PROBABLE_PASSWORD_PRIORITY_REGEX = 'user_passwords\[(\d+)\]\[Pri\]'
PWD_PROTECTED_ATTACHMENTS_PROBABLE_PASSWORD_PRIORITY_XPATH = lambda index: 'user_passwords[%s][Pri]' % index
...
class GlobalSettings(InputsOwner):
	...
	self.gui.click_button(UNSCANNABLE_DE_ADVANCED_ARROW, "don't wait")	
        ...
        if 'Probable Password for Analysis' in new_value:
            passwords_to_delete = []
            passwords_to_edit = []
            prioritiess_to_edit = []
            passwords_to_add = []
            password_data = new_value['Probable Password for Analysis']

            for data in password_data:
                if re.search(r'delete\|(\S+)', data):
                    passwords_to_delete.append(re.search(r'delete\|(\S+)', data).group(1))
                elif re.search(r'edit-password\|(\S+)', data):
                    passwords_to_edit.append(re.search(r'edit-password\|(\S+)', data).group(1))
                elif re.search(r'edit-priority\|(\S+)', data):
                    prioritiess_to_edit.append(re.search(r'edit-priority\|(\S+)', data).group(1))
                elif re.search(r'add\|(\S+)', data):
                    passwords_to_add.append(re.search(r'add\|(\S+)', data).group(1))
                else:
                    raise GuiValueError('Unrecognized action "%s". Allowed values are - ' \
                        'add | edit | delete' % re.search(r'(\S+)\|(\S+)', data).group(1))

            # Get existing password values
            password_values = self._get_table_row_values(
                PWD_PROTECTED_ATTACHMENTS_PROBABLE_PASSWORD_VALUE_REGEX,
                PWD_PROTECTED_ATTACHMENTS_PROBABLE_PASSWORD_VALUE_XPATH)
            # Get existing priority values
            priority_values = self._get_table_row_values(
                PWD_PROTECTED_ATTACHMENTS_PROBABLE_PASSWORD_PRIORITY_REGEX,
                PWD_PROTECTED_ATTACHMENTS_PROBABLE_PASSWORD_PRIORITY_XPATH)

            if passwords_to_delete:
                for password in passwords_to_delete:
                    if password not in password_values:
                        raise GuiValueError('Password %s not found.' % password)
                    for index, row_value in enumerate(password_values):
                        if password == row_value:
                            self.gui._info('Deleting password "%s"' % password)
                            self.gui.click_button(
                                PWD_PROTECTED_ATTACHMENTS_PROBABLE_PASSWORD_DELETE_BUTTON(index), "don't wait")
                            self.gui._info("Deleted password")
                else:
                    # Check if all passwords are deleted.
                    # If yes then UI will throw error during submit.
                    new_password_values = self._get_table_row_values(
                        PWD_PROTECTED_ATTACHMENTS_PROBABLE_PASSWORD_VALUE_REGEX,
                        PWD_PROTECTED_ATTACHMENTS_PROBABLE_PASSWORD_VALUE_XPATH)
                    if new_password_values and not new_password_values[0]:
                        self.gui.input_text(PWD_PROTECTED_ATTACHMENTS_PROBABLE_PASSWORD_PRIORITY_XPATH(
                            len(password_values) - 1), '0')
                        self.gui.input_text(PWD_PROTECTED_ATTACHMENTS_PROBABLE_PASSWORD_VALUE_XPATH(
                            len(password_values) - 1), 'CsAutoSmplePwd')

            if passwords_to_edit:
                for password in passwords_to_edit:
                    old_password = password.split('|')[0]
                    new_password = password.split('|')[1]
                    if old_password not in password_values:
                        raise GuiValueError(
                            'Password %s not found.' % old_password)
                    for index, row_value in enumerate(password_values):
                        if old_password == row_value:
                            self.gui._info('Editing password "%s" to "%s"' % (old_password, new_password))
                            self.gui.input_text(
                                PWD_PROTECTED_ATTACHMENTS_PROBABLE_PASSWORD_VALUE_XPATH(index), new_password)
                            self.gui._info("Edited password")
            
            if prioritiess_to_edit:
                for priority in prioritiess_to_edit:
                    old_priority = priority.split('|')[0]
                    new_priority = priority.split('|')[1]
                    if old_priority not in priority_values:
                        raise GuiValueError(
                            'Password with priority %s not found.' % old_priority)
                    for index, row_value in enumerate(priority_values):
                        if old_priority == row_value:
                            self.gui._info('Editing priority "%s"' % password)
                            self.gui.input_text(
                                PWD_PROTECTED_ATTACHMENTS_PROBABLE_PASSWORD_PRIORITY_XPATH(index), new_priority)
                            self.gui._info("Edited priority")

            if passwords_to_add:
                if password_values and password_values[0]:
                    for index in range(len(passwords_to_add)):
                        row_index = index + len(password_values) if password_values[0] != 'CsAutoSmplePwd' else index
                        password_priority = passwords_to_add[index].split('|')[0]
                        password_value = passwords_to_add[index].split('|')[1]
                        self.gui._info('Adding password "%s" with priority "%s"' % (
                            password_value, password_priority))
                        self.gui.input_text(
                            PWD_PROTECTED_ATTACHMENTS_PROBABLE_PASSWORD_PRIORITY_XPATH(row_index), password_priority)
                        self.gui.input_text(
                            PWD_PROTECTED_ATTACHMENTS_PROBABLE_PASSWORD_VALUE_XPATH(row_index), password_value)
                        if index < len(passwords_to_add) - 1:
                            self.gui.click_button(
                                PWD_PROTECTED_ATTACHMENTS_PROBABLE_PASSWORD_ADD_BUTTON, "don't row")
                        self.gui._info('Password added')
                else:
                    if len(passwords_to_add) > 1:
                        for index in range(len(passwords_to_add)):
                            password_priority = passwords_to_add[index].split('|')[0]
                            password_value = passwords_to_add[index].split('|')[1]
                            self.gui._info('Adding password "%s" with priority "%s"' % (
                                password_value, password_priority))
                            self.gui.input_text(
                                PWD_PROTECTED_ATTACHMENTS_PROBABLE_PASSWORD_PRIORITY_XPATH(index), password_priority)
                            self.gui.input_text(
                                PWD_PROTECTED_ATTACHMENTS_PROBABLE_PASSWORD_VALUE_XPATH(index), password_value)
                            if index < len(passwords_to_add) - 1:
                                self.gui.click_button(
                                    PWD_PROTECTED_ATTACHMENTS_PROBABLE_PASSWORD_ADD_BUTTON, "don't row")
                            self.gui._info('Password added')
                    else:
                        password_priority = passwords_to_add[0].split('|')[0]
                        password_value = passwords_to_add[0].split('|')[1]
                        self.gui._info('Adding password "%s" with priority "%s"' % (
                            password_value, password_priority))
                        self.gui.input_text(
                            PWD_PROTECTED_ATTACHMENTS_PROBABLE_PASSWORD_PRIORITY_XPATH(0), password_priority)
                        self.gui.input_text(
                            PWD_PROTECTED_ATTACHMENTS_PROBABLE_PASSWORD_VALUE_XPATH(0), password_value)
                        self.gui._info('Password added')
        else:
            if self.gui._is_editable(PWD_PROTECTED_ATTACHMENTS_PROBABLE_PASSWORD_PRIORITY_XPATH(0)):
                # Get existing password values
                password_values = self._get_table_row_values(
                    PWD_PROTECTED_ATTACHMENTS_PROBABLE_PASSWORD_VALUE_REGEX,
                    PWD_PROTECTED_ATTACHMENTS_PROBABLE_PASSWORD_VALUE_XPATH)
                # As of now we cannot submit an emtpy password hence this tweak.
                if password_values and not password_values[0]:
                    self.gui.input_text(PWD_PROTECTED_ATTACHMENTS_PROBABLE_PASSWORD_PRIORITY_XPATH(0), '1')
                    self.gui.input_text(PWD_PROTECTED_ATTACHMENTS_PROBABLE_PASSWORD_VALUE_XPATH(0), 'CsAutoSmplePwd')
                    self.gui._info('Added temporary password "test123" to avoid GUI error')

    def _get_table_row_values(self, row_regex, table_regex):
        val_list = []
        row_pattern = re.compile(row_regex)
        text_fields = self.gui._get_all_fields()
        for field in text_fields:
            result = row_pattern.search(field)
            if result:
                value = self.gui.get_value(table_regex(int(result.group(1))))
