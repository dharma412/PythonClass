...
from tracking import DlpViolationSeverities, DlpAction, DmarcAction, EtfSelectIocs, GeoLocation
from tracking import MacroMailFlowDirection, MacroFileTypes, InPolicyOrVirusQuarantine, OutbreakFilters
from tracking import Remediation, URLCategories, WebInteractionTrackingMailflowDirection, MailFlowDirection
...
from tracking import APPForwarding, ToastWidget, ExportResults
...
class Tracking(NGGuiCommon):
    ...
    def get_keyword_names(self):
        return ['message_tracking_search', 'message_tracking_get_message_details',
                ...
                'get_total_tracking_message_count',
    ...
    def message_tracking_search(self,
                                ...
                                mail_policy_name=None,
                                mail_flow_direction=None,
        ...
        if mail_policy_name:
            self.click_element(MESSAGE_TRACKING.MESSAGE_EVENT % (MESSAGE_TRACKING.MAIL_POLICY))
            self.input_text(MESSAGE_TRACKING.MAIL_POLICY_NAME, mail_policy_name)
            if mail_flow_direction:
                for mail_flow in mail_flow_direction:
                    self.select_custom_checkbox(getattr(MailFlowDirection, mail_flow))
        ...
        if wait_time and wait_time >=0 :
            self._wait_until_element_is_present(MESSAGE_TRACKING.SEARCH_BUTTON, timeout=10)
            self.click_button(MESSAGE_TRACKING.SEARCH_BUTTON)
            self.wait_for_angular()

            self._debug('waiting for wait time to over:%s'%wait_time)
            time.sleep(int(wait_time))
            self.click_ng_button(MESSAGE_TRACKING.SHOW_FILTER, '')
            self.click_ng_button(MESSAGE_TRACKING.MODIFY, 'Modify All Criteria')
            self._wait_until_element_is_present(MESSAGE_TRACKING.SEARCH_BUTTON, timeout=10)
            self.click_button(MESSAGE_TRACKING.SEARCH_BUTTON)
            self.wait_for_angular()
        else:
    ...
    def _get_tracking_data(self, message_index=None, more_details=None):
        ...
        tmp_data[self.get_text(SearchResult.policy_match_header %(message_index))] = self.get_text(SearchResult.policy_match_value % (message_index))
        tmp_data[self.get_text(SearchResult.sender_group %(message_index))] = self.get_text(SearchResult.sender_group_value % (message_index))
        tmp_data[self.get_text(SearchResult.sender_ip %(message_index))] = self.get_text(SearchResult.sender_ip_value % (message_index))
        tmp_data[self.get_text(SearchResult.sender %(message_index))] = self.get_text(SearchResult.sender_value % (message_index))
        tmp_data[self.get_text(SearchResult.recipient %(message_index))] = self.get_text(SearchResult.recipient_value % (message_index))
        tmp_data[self.get_text(SearchResult.subject %(message_index))] = self.get_text(SearchResult.subject_value % (message_index))
        tmp_data[self.get_text(SearchResult.sbrs_score %(message_index))] = self.get_text(SearchResult.sbrs_score_value % (message_index))
	try:
    ...
    def message_tracking_get_message_details(self, mid=None, more_details=None):
        """
        ...
        for each_msg in range(int(self.get_text(SearchResult.total_count))):
            ...
            if ',' in current_mid:
                current_mid_id= map(str.strip, current_mid.encode('utf-8').split(','))
    ...
    def _select_all_to_remediate(self):
        ...
        for each_scroll in range(int(latest_mid)):
            ...
            if reached_max > 6:
                ...
                self.select_ngsma_search_result_checkbox(SearchResult.select_all)
    ...
    def message_tracking_remediate(self, mid=None, batch_name=None, \
        ...
        self.element_should_be_visible(MESSAGE_TRACKING.SEARCH_CRITERIA)
        ...
        self.set_selenium_speed('1s')
        if self.ng_button_is_visible(SearchResultRemediate.Remediate_button):
    ...
    def toast_widget_location(self):
        """
        This keyword will return the toast widget location
        :return:  x, y
        """
        if self.toast_widget_visible():
    ...
    def toast_widget_visible_time(self):
        """
        This keyword will return the toast widget visible time
        :return: visible time in seconds (s)
        """
        start_time = time.time()
        duration = 20
    ...
    def toast_widget_content(self):
        """
        This keyword will return the toast widget content
    ...
    def toast_widget_cancel(self):
        """
        This keyword to cancel the toast widget action
        """
        if self.toast_widget_visible():
    ...
    def toast_widget_view_details(self, wait_time=0):
        ...
        while (time.time() - start_time <= wait_time):
            if self.toast_widget_visible():
                if self._is_visible(ToastWidget.view_details):
                    self._info('view details visible')
                    self.click_element(ToastWidget.view_details)
                    break

    def get_total_tracking_message_count(self):
        """
        This keyword to get the total tracking message count
        :return: total message count on the search view
        """
        reached_max = 0
        self._wait_until_element_is_present(SearchResult.result_table_details %(0) + SearchResult.message_mid,
                                            timeout=20)
        latest_mid = self.get_text(SearchResult.result_table_details %(0) + SearchResult.message_mid)
        if ',' in latest_mid:
            latest_mid = latest_mid.split(',')[0]
        self.set_selenium_speed('0.05s')
        for each_scroll in range(int(latest_mid)):
            self.prev_count = self.get_text(SearchResult.total_count)
            for count in range(5):
    ...
    def export_tracking_results(self, results):
        """
        Keyword to Export Tracking results
        :param results: All_Results|Page_View_Results
        :return: raise DataNotFoundError if results not found
        """
        self.click_element(ExportResults.Export)
        if results not in ExportResults.Tracking_Results:
            raise DataNotFoundError('Failed to find the policy report:%s' %results)
        self.click_element(ExportResults.Tracking_Results[results])
