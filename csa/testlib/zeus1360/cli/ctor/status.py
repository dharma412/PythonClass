#!/usr/bin/env python
# $Id: //prod/main/sarf_centos/testlib/zeus1360/cli/ctor/status.py#1 $
"""
SARF Command Line Interface (CLI)
command: - status
"""

import clictorbase

import common.util.featurekeytool as FeatureKeyTool
...
class status(clictorbase.IafCliConfiguratorBase):
    def __call__(self, detail_bool=True):
        ...
        while status_output.lower().find('status as of:') == -1 \
                 and tries <= 5:
...
class StatusInfoObject:
    ...
    def __init__(self, status_output):
        ...
        self.error_parameter_message = 'Only names of existing parameters should be used.'
        self.error_index_message = 'Incorrect index was used.'
        self.clean_output = status_output[:status_output.rfind('\n')]
        ...
        'messages_in_quarantine':       'Messages In Quarantine    %d',
        'destinations_in_memory':       'Destinations In Memory    %d',
        'kilobytes_used':               'Kilobytes Used            %d',
        'kilobytes_in_quarantine':      '  Kilobytes In Quarantine %d',
        ...
        for feature_line in features_out.split('\n'):
            ...
            # seconds_remaining
            dut = None
            fk = FeatureKeyTool.GenericFeatureKey(dut, None, None)
    ...
    def get_system(self, name, idx=0):
        ...
        # Returns:
        #   A string.
        if not ((name == 'up_since' and idx in (0,1)) or (idx == 0)):
            raise ValueError(self.error_index_message)

        try:
            return self.system[name][idx]
        except:
            raise ValueError(self.error_parameter_message)
