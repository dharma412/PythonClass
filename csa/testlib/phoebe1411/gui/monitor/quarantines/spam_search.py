#!/usr/bin/env python
# $Id: //prod/main/sarf_centos/testlib/phoebe1350/gui/monitor/quarantines/spam_search.py#1 $ $DateTime: 2020/01/06 01:25:43 $ $Author: saurgup5 $
...
class SpamSearch(QuarantinesCommon):
    ...
    *Search in Spam Quarantine*\n
    ...
    def _spam_search(self,
        ...
        if header_name:
            self.select_from_list_by_label\
    ...
    def _admin_delete_action(self):
        self._wait_until_element_is_present(SPAM_QUARANTINE_DELETE_BUTTON, timeout=10) 
    ...
    def _do_isq_action(self, action_method, items_per_page):
        while True:
            if self._is_text_present('No items found'):
                break
            self._set_items_per_page(items_per_page)
            self._wait_until_element_is_present(SPAM_QUARANTINE_SEARCH_TABLE_SELECT_ALL_MIDS, timeout=10)
    ...
    def _grep_row(self,
        ...
        data.mid = self.get_element_attribute\
            ("%s//input[@type='checkbox']" % \
             QUARANTINE_SEARCH_TABLE_CELL_DATA\
             (SPAM_QUARANTINE_SEARCH_TABLE_RESULTS_CONTAINER, row_idx, 1), "value")
        # skip 1st(mid) column
