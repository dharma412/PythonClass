...
TG_OK_BUTTON  = "//button[@id='yui-gen23-button']"
...
REMOVE_AND_REGISTER_BUTTON= "//button[@id='yui-gen29-button']"
DEREGISTER_CONFIRM_BUTTON  = "//button[contains(text(),'Submit')]"
ENABLE_SMART_LICENSE_ON_ALL_MACHINES = "//input[@id='sl_enable_cluster']"
REGISTER_SMART_LICENSE_ON_ALL_MACHINES = "//input[@id='register_cluster']"
...
class SmartLicenseConfig(GuiCommon):
    """Keywords for ESA GUI interaction with 'System Administration-->
    ...
    def _register(self, **kwargs):
        ...
        if settings:
            if not settings.has_key('sl_action'):
                if self._is_element_present(REGISTER_SSM_BUTTON):
                    ...
                    raise guiexceptions.GuiControlNotFoundError(
                        'Register option is not available', 'System Administration -> Smart Software Licensing')
            ...
            if settings.has_key('register_sl_on_all_machines_in_cluster'):
                if settings['register_sl_on_all_machines_in_cluster']:
                    self._select_checkbox(REGISTER_SMART_LICENSE_ON_ALL_MACHINES)
                else:
                    self._unselect_checkbox(
                        REGISTER_SMART_LICENSE_ON_ALL_MACHINES)
            ...
            if self._is_element_present(REGISTER_BUTTON):
                ...
                if settings.has_key('sl_action'):
                    if self._is_element_present(REMOVE_AND_REGISTER_BUTTON):
                        ...
                        raise guiexceptions.GuiError('Confirmation message is not displayed')
            else:
                raise guiexceptions.GuiError('Register button is not available')
        ...
        """Helper function
          ...
          Input Dictionary:
             {u'Evaluation Period Remaining': u'89 days, 4 hr, 54 min, 42 sec',
              ...
              u'Product Instance Name': u'vm30esa0099.ibqa',
              u'Virtual Account': u'ESA 4'}
    ...
    def smart_license_enable(self, **kwargs):
        """Enables SmartLicensing license mode

        *Parameters:*
         - `enable_sl_on_all_machines_in_cluster`: Pass ${True} to enable Smart License
            on all machines in the cluster. Pass ${False} to disable it.

        *Examples:*
        | Smart License Enable |
        | Smart License Enable | enable_sl_on_all_machines_in_cluster=${True} |
        | Smart License Enable | enable_sl_on_all_machines_in_cluster=${False} |

        """
        if self._is_element_present(ENABLE_BUTTON):
            self.click_button(ENABLE_BUTTON)
            if self._is_element_present(ENABLE_SMART_LICENSE_ON_ALL_MACHINES):
                # This element should be present in Cluster Mode
                if kwargs.has_key('enable_sl_on_all_machines_in_cluster'):
                    if kwargs['enable_sl_on_all_machines_in_cluster']:
                        self._select_checkbox(ENABLE_SMART_LICENSE_ON_ALL_MACHINES)
                    else:
                        self._unselect_checkbox(ENABLE_SMART_LICENSE_ON_ALL_MACHINES)
            ...
            raise guiexceptions.GuiControlNotFoundError(
                'Enable option is not available', 'System Administration -> Smart Software Licensing')
    ...
    @go_to_page(PAGE_PATH)
    def smart_license_configure_url(self, **kwargs):
        """Configure URL
        ...
        if settings:
            ...
            if transport_settings.lower() == 'direct':
                ...
                if settings.has_key('url'):
                    ...
                    raise ValueError(
                        'url argument is mandatory for "%s" option' % transport_settings)
            else:
    ...
    @check_smart_licensing(True)
    @go_to_page(PAGE_PATH)
    def smart_license_register(self, **kwargs):
        ...
        *Examples:*
        ...
        | Smart License Register                               |
        | ... | register_sl_on_all_machines_in_cluster=${True} |
    ...
    @check_smart_licensing(True)
    @go_to_page(PAGE_PATH)
    def smart_license_perform_action(self, **kwargs):
        """
