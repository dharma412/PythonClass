# $Id: //prod/main/sarf_centos/testlib/phoebe1350/gui/admin/saml.py#1 $
...
PAGE_PATH = ('System Administration', 'SAML')
...
CUSTOMER_SP_EDIT_LINK = lambda name: "%s//a[normalize-space()='%s']" % (CUSTOMER_TABLE, name)
CUSTOMER_SP_DELETE_BUTTON = lambda name: "%s//td[.//a[normalize-space()='%s']]" \
                        "/following-sibling::td/img[@title='Delete...']" % (CUSTOMER_TABLE, name)
CUSTOMER_IDP_EDIT_LINK = lambda name: "%s//a[normalize-space()='%s']" % (CUSTOMER_TABLE, name)
...
DEVOPS_SP_EDIT_LINK = lambda name: "%s//a[normalize-space()='%s']" % (DEVOPS_TABLE, name)
DEVOPS_IDP_EDIT_LINK = lambda name: "%s//a[normalize-space()='%s']" % (DEVOPS_TABLE, name)
...
class Saml(GuiCommon):
    ...
    def _saml_add_sp(self, profile_name, settings):
        ...
        try:
            ...
            raise guiexceptions.GuiValueError('Invalid arguments found %s' %error)
    ...
    def _saml_add_idp(self, profile_name, settings):
        ...
        remove_elem = ['Organization URL', 'Organization Display Name', 'SP Entity ID', \
                       ...
                       'PKCS Certificate Passphrase', 'Certificate Select From List']
        for element_remove in remove_elem:
    ...
    def saml_add_sp_and_idp(self, sp_name, idp_name, settings = {}):
        ...
        *Parameters:*
        ...
        | Certificate Upload Action | We can Choose 'Select From List' or 'Upload Certificate Key' or
            'Upload PKCS Certificate' . If we choose Upload Certificate Key we should use SP Certificate, Private Key
            and Certificate Passphrase parameters. If we choose Select From List , we should use
            Certificate Select From List parameter. If we Choose Upload PKCS Certificate we should use
        ...
        | Certificate Select From List | Common name of the certificate from cert store. |
    ...
    @go_to_page(PAGE_PATH)
    def saml_delete_sp_idp(self, **kwargs):
