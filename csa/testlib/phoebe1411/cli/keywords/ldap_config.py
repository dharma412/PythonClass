#!/usr/bin/env python
# $Id: //prod/main/sarf_centos/testlib/phoebe1350/cli/keywords/ldap_config.py#1 $ $DateTime: 2020/01/06 01:25:43 $ $Author: saurgup5 $

from common.cli.clicommon import (CliKeywordBase, YES, NO, DEFAULT)
...
class LDAPConfig(CliKeywordBase):
    ...
    def get_keyword_names(self):
        return [
            ...
            'ldap_config_add_ldapaccept',
            'ldap_config_edit_ldapaccept_name',
            'ldap_config_edit_ldapaccept_query',
            'ldap_config_delete_ldapaccept',
            'ldap_config_add_routing',
            'ldap_config_edit_routing_name',
            'ldap_config_edit_routing_query',
            'ldap_config_edit_routing_attribute',
            'ldap_config_delete_routing',
            'ldap_config_add_masquerade',
            'ldap_config_edit_masquerade_name',
            'ldap_config_edit_masquerade_query',
            'ldap_config_edit_masquerade_attribute',
            'ldap_config_edit_masquerade_overwrite',
            'ldap_config_delete_masquerade',
            'ldap_config_add_ldapgroup',
            'ldap_config_edit_ldapgroup_name',
            'ldap_config_edit_ldapgroup_query',
            'ldap_config_edit_ldapgroup_attribute',
            'ldap_config_delete_ldapgroup',
            'ldap_config_add_smtpauth',
            'ldap_config_edit_smtpauth_name',
            'ldap_config_edit_smtpauth_query',
            'ldap_config_edit_smtpauth_type',
            'ldap_config_edit_smtpauth_max_connections',
            'ldap_config_delete_smtpauth',
        ]

    def ldap_config_new(self, name, address, base=DEFAULT, use_ssl=DEFAULT,
        ...
        *Examples:*
        ...
        | LDAP Config New | myLDAP | sully.qa | base=dc=qa | yes |
        | ... | password based | admin | ironport | yes | port=389 |
        ...
        *Exceptions:*
        ...
        if username is not None:
            ...
            input_dict.update({'password': password, 'username': username, 'retype_passwd': password})
    ...
    def ldap_config_edit_server_authtype(self, name, auth_type=DEFAULT,
        ...
        *Examples:*
        ...
        if password is not None:
            input_dict.update({'passphrase': password})
    ...
    def ldap_config_add_ldapaccept(self, server_name, query_name,
                                   query=DEFAULT,
                                   testing=NO, test_addr='test@example.com'):
        """Configure whether a recipient address should be accepted
        or bounced/dropped.

        *Parameters:*
        - `server_name`: name of the server configuration to enable query for.
        Mandatory.
        - `query_name`: name of accept query to activate.
        - `query`: query to decide between accepting an address
        or bouncing/dropping.
        - `testing`: test this query. Defaults to "NO".
        - `test_addr`: test using this address. Mandatory if `testing` is set
        to "YES".

        *Examples:*
        | LDAP Config Add LDAPAccept | myLDAP | test.ldapaccept | (mail={a}) |
        ...
        *Exceptions:*
        ...
        if input_dict.get('testing') == 'Y':
            if None in (identity, password):
                raise ConfigError('Email address must be provided '\
                    'in order to perform the test of the query')
            input_dict.update({'test_addr': test_addr,})

        self._cli.ldapconfig().edit(server_name).ldapaccept(input_dict)

    def ldap_config_edit_ldapaccept_name(self, server_name, query_name):
        """Change the LDAP Accept query name.

        *Parameters:*
        - `server_name`: name of the server configuration to edit.
        - `query_name`: name for the LDAP Accept query.

        *Examples:*
        | LDAP Config Edit LDAPAccept Name | myLDAP | query.name |
        """
        self._cli.ldapconfig().edit(server_name).ldapaccept().name(query_name)

    def ldap_config_edit_ldapaccept_query(self, server_name, query):
        """Change the LDAP Accept query.

        *Parameters:*
        - `server_name`: name of the server configuration to edit.
        - `query`: LDAP Accept query.

        *Examples:*
        | LDAP Config Edit LDAPAccept Query | myLDAP | (mail={a})
        """
        self._cli.ldapconfig().edit(server_name).ldapaccept().query(query)

    def ldap_config_delete_ldapaccept(self, server_name):
        """Delete LDAP Accept query.

        *Parameters:*
        - `server_name`: name of the server configuration to edit.

        *Examples:*
        | LDAP Config Delete LDAPAccept | myLDAP |
        """
        self._cli.ldapconfig().edit(server_name).ldapaccept().delete()

    def ldap_config_add_routing(self, server_name, query_name=DEFAULT,
                                query=DEFAULT, recpt_addr=DEFAULT,
                                mailhost=DEFAULT, sending=DEFAULT,
                                testing=NO, test_addr='test@example.com'):
        """Configure message routing via LDAP query.

        *Parameters:*
        - `server_name`: name of the server configuration to edit.
        Mandatory.
        - `query_name`: name of accept query to activate.
        - `query`: query to decide between accepting an address or
        bouncing/dropping.
        - `recpt_addr`: attribute which contains rfc822 email address for the
        recipients.
        - `mailhost`: attribute which contains the alternate mailhost.
        - `sending`: mode of sending. Mandatory. Can be one of three:
        | 1 | Configure MAILROUTINGADDRESS only -
        Rewrite the Envelope Recipient (and leave MAILHOST unconfigured) |
        | 2 | Configure MAILHOST only -
        Send the messages to an alternate mail host (and
        leave MAILROUTINGADDRESS unconfigured) |
        ...
        *Examples:*
        | LDAP Config Add Routing | myLDAP | test.ldaprouting | (mailLocalAddress={a}) |
        | ... | mailRoutingAddress | mailHostyes | 3 | yes | test@example.com |
        ...
        *Exceptions:*
        ...
        self._cli.ldapconfig().edit(server_name).ldaprouting(input_dict)

    def ldap_config_edit_routing_name(self, server_name, query_name=DEFAULT):
        """Change the LDAP Routing query name.

        *Parameters:*
        - `server_name`: name of the server configuration to edit. Mandatory.
        - `query_name`: name for the LDAP Accept query.

        *Examples:*
        | LDAP Config Edit Routing Name | myLDAP | query.name |
        """
        self._cli.ldapconfig().edit(server_name).ldaprouting().name(query_name)

    def ldap_config_edit_routing_query(self, server_name, query=DEFAULT):
        """Change the LDAP Routing query.

        *Parameters:*
        - `server_name`: name of the server configuration to edit. Mandatory.
        - `query`: LDAP Routing query.

        *Examples:*
        | LDAP Config Edit Routing Query | myLDAP | (mailLocalAddress={a}) |
        """
        self._cli.ldapconfig().edit(server_name).ldaprouting().query(query)

    def ldap_config_edit_routing_attribute(self, server_name, \
            sending, recpt_addr=DEFAULT, mailhost=DEFAULT):
        """Change the way LDAP Routing works.
        ...
        *Parameters:*
        ...
        Rewrite the Envelope Recipient (and leave MAILHOST unconfigured) |
        | 2 | Configure MAILHOST only -
        Send the messages to an alternate mail host
        (and leave MAILROUTINGADDRESS unconfigured) |
        | 3 | Configure both attributes |
        - `recpt_addr`: attribute which contains rfc822 email address
        for the recipients.
        - `mailhost`: attribute which contains the alternate mailhost.

        *Examples:*
        | LDAP Config Edit Routing Attribute | myLDAP |
        | ... | 3 | mailRoutingAddress | mailHost |
        """
        self._cli.ldapconfig().edit(server_name).ldaprouting().attribute(
                sending, recpt_addr, mailhost)

    def ldap_config_delete_routing(self, server_name):
        """Delete the LDAP Routing query.

        *Parameters:*
        - `server_name`: name of the server configuration to edit. Mandatory.

        *Examples:*
        | LDAP Config Delete LDAPRouting | myLDAP |
        """
        self._cli.ldapconfig().edit(server_name).ldaprouting().delete()

    def ldap_config_add_masquerade(self, server_name, query_name=DEFAULT,
                                   query=DEFAULT, attribute=DEFAULT,
                                   overwrite=DEFAULT,
                                   testing=NO, test_addr='test@example.com'):
        """Configure domain masquerading via LDAP query.

        *Parameters:*
        - `server_name`: name of the server configuration to edit. Mandatory.
        - `query_name`: name of masquerading query to activate.
        - `query`: LDAP query to use.
        - `attribute`: externally visible rfc822 email address attribute.
        - `overwrite`: should results of the returned attribute replace the
        entire friendly portion of the original recipient? Default is "Yes".
        - `testing`: test this query. Defaults to "NO".
        - `test_addr`: test using this address. Mandatory if `testing`
        is set to "YES".

        *Examples:*
        | LDAP Config Add Masquerade | myLDAP | test.masquerade |
        | ... | (mailRoutingAddress={a}) | mailLocalAddress | Yes |
        | ... | Yes | test@example.com |
        ...
        *Exceptions:*
        ...
        self._cli.ldapconfig().edit(server_name).masquerade(input_dict)

    def ldap_config_edit_masquerade_name(self, server_name, query_name=DEFAULT):
        """Change the LDAP domain masquerading query name.
        ...
        *Examples:*
        | LDAP Config Edit Masquerade Name | myLDAP | query.name |
        """
        self._cli.ldapconfig().edit(server_name).masquerade().name(query_name)

    def ldap_config_edit_masquerade_query(self, server_name, query=DEFAULT):
        """Change the LDAP domain masquerading query.

        *Parameters:*
        - `server_name`: name of the server configuration to edit. Mandatory.
        - `query`: LDAP Masquerading query.

        *Examples:*
        | LDAP Config Edit Masquerade Query | myLDAP | (mailRoutingAddress={a}) |
        """
        self._cli.ldapconfig().edit(server_name).masquerade().query(query)

    def ldap_config_edit_masquerade_attribute(self, server_name, attribute=DEFAULT):
        """Change the LDAP domain masquerading attribute.
        ...
        *Examples:*
        | LDAP Config Edit Masquerade Attribute | myLDAP | mailLocalAddres |
        """
        self._cli.ldapconfig().edit(server_name).masquerade().attribute(attribute)

    def ldap_config_edit_masquerade_overwrite(self, server_name, overwrite=DEFAULT):
        """Control original recipient overwriting with LDAP attribute.

        *Parameters:*
        - `server_name`: name of the server configuration to edit. Mandatory.
        - `overwrite`: should overwriting of entire friendly portion of the
        original recipient be enabled. Default is "Yes".

        *Examples:*
        | LDAP Config Edit Masquerade Overwrite | myLDAP | No |
        """
        self._cli.ldapconfig().edit(server_name).masquerade().overwrite(overwrite)

    def ldap_config_delete_masquerade(self, server_name):
        """Delete the LDAP Masquerade query.

        *Parameters:*
        - `server_name`: name of the server configuration to edit. Mandatory.

        *Examples:*
        | LDAP Config Delete Masquerade | myLDAP |
        """
        self._cli.ldapconfig().edit(server_name).masquerade().delete()

    def ldap_config_add_ldapgroup(self, server_name, query_name=DEFAULT,
                                  query=DEFAULT, attribute=DEFAULT,
                                  testing=NO, test_addr='test@example.com',
                                  test_group=DEFAULT):
        """Configure a query whether a sender or recipient is in a specified
        LDAP group.

        *Parameters:*
        - `server_name`: name of the server configuration to edit. Mandatory.
        - `query_name`: name for the LDAP Group query.
        - `query`: LDAP Group query.
        - `attribute`: group membership attribute.
        - `testing`: test this query. Defaults to "NO".
        - `test_addr`: test using this address.
        Mandatory if `testing` is set to "YES".
        - `test_group`: test using this group name.
        Mandatory if `testing` is set to "YES".

        *Examples:*
        | LDAP Config Add LDAPGroup | myLDAP | test.ldapgroup | (&(objectClass=posixGroup)(cn={g})(memberUid={u})) |
        | ... | groupMembers | yes | test@example.com | somegroup |

        *Exceptions:*
        - `ConfigError`: in case no email address and group was provided
        when performing query testing.
        """
        input_dict = {
            'name': query_name,
            'query': query,
            'grp_member': attribute,
            'testing': testing,
        }
        if input_dict.get('testing') == 'Y':
            if None in (test_addr,test_group):
                raise ConfigError('Email address and group must be provided '\
                    'in order to perform the test of the query')
            input_dict.update({'test_addr': test_addr,})
            input_dict.update({'test_group': test_group,})

        self._cli.ldapconfig().edit(server_name).ldapgroup(input_dict)

    def ldap_config_edit_ldapgroup_name(self, server_name, query_name=DEFAULT):
        """Change the LDAP Group query name.
        ...
        *Examples:*
        | LDAP Config Edit LDAPGroup Name | myLDAP | query.name |
        """
        self._cli.ldapconfig().edit(server_name).ldapgroup().name(query_name)

    def ldap_config_edit_ldapgroup_query(self, server_name, query=DEFAULT):
        """Change the LDAP Group query.
        ...
        *Examples:*
        | LDAP Config Edit LDAPGroup Query | myLDAP | (&(objectClass=posixGroup)(cn={g})(memberUid={u})) |
        """
        self._cli.ldapconfig().edit(server_name).ldapgroup().query(query)

    def ldap_config_edit_ldapgroup_attribute(self, server_name, attribute=DEFAULT):
        """Change the LDAP Group attribute.
        ...
        *Examples:*
        | LDAP Config Edit LDAPGroup Attribute | myLDAP | groupMembers |
        """
        self._cli.ldapconfig().edit(server_name).ldapgroup().attribute(attribute)

    def ldap_config_delete_ldapgroup(self, server_name):
        """Delete LDAP Group query.

        *Parameters:*
        - `server_name`: name of the server configuration to edit. Mandatory.

        *Examples:*
        | LDAP Config Delete LDAPGroup | myLDAP |
        """
        self._cli.ldapconfig().edit(server_name).ldapgroup().delete()

    def ldap_config_add_smtpauth(self, server_name, query_name=DEFAULT,
                                 query=DEFAULT, authtype=DEFAULT,
                                 maxthreads=DEFAULT, testing=NO,
                                 test_user_id='testuser', isq_passwd=DEFAULT):
        """Configure SMTP authentication.

        *Parameters:*
        - `server_name`: name of the server configuration to edit. Mandatory.
        - `query_name`: name for the LDAP SMTPAuth query.
        - `query`: LDAP SMTPAuth query.
        - `authtype`: Select bind or query authentication.
        - `maxthreads`: Set the number of connections for this query.
        - `testing`: test this query. Defaults to "NO".
        - `test_user_id`: test using this user identity.
        Mandatory if `testing` is set to "YES".
        - `isq_passwd`: test using this password.
        Mandatory if `testing` is set to "YES".
        ...
        *Exceptions:*
        ...
        self._cli.ldapconfig().edit(server_name).smtpauth(input_dict)

    def ldap_config_edit_smtpauth_name(self, server_name, query_name=DEFAULT):
        """Change the SMTPAuth query name.

        *Parameters:*
        - `server_name`: name of the server configuration to edit. Mandatory.
        - `query_name`: name for the SMTPAuth query.

        *Examples:*
        | LDAP Config Edit SMTPAuth Name | myLDAP | query.name |
        """
        self._cli.ldapconfig().edit(server_name).smtpauth().name(query_name)

    def ldap_config_edit_smtpauth_query(self, server_name, query=DEFAULT):
        """Change the SMTPAuth query.
        ...
        *Examples:*
        | LDAP Config Edit SMTPAuth Query | myLDAP | (uid={u}) |
        """
        self._cli.ldapconfig().edit(server_name).smtpauth().query(query)

    def ldap_config_edit_smtpauth_type(self, server_name, authtype=DEFAULT):
        """Select LDAP BIND method or LDAP query for SMTP authentication.

        *Parameters:*
        - `server_name`: name of the server configuration to edit. Mandatory.
        - `authtype`: Authenticate with an LDAP BIND or by fetching the
        password as an attribute. Possible values:
        | 1 | LDAP BIND |
        | 2 | Fetch password as attribute |

        *Examples:*
        | LDAP Config Edit SMTPAuth Type | myLDAP | Fetch password as attribute |
        """
        self._cli.ldapconfig().edit(server_name).smtpauth().smtpauth_type(authtype)

    def ldap_config_edit_smtpauth_max_connections(self, server_name, maxthreads=DEFAULT):
        """Configure the number of concurrent connectionsto dedicate to
        SMTPAuth query.

        *Parameters:*
        - `server_name`: name of the server configuration to edit. Mandatory.
        - `maxthreads`: maximum number of concurrent connections to dedicate
        to SMTPAuth query. Default is 1.

        *Examples:*
        | LDAP Config Edit SMTPAuth Max Connections | myLDAP | 4 |
        """
        self._cli.ldapconfig().edit(server_name).smtpauth().maxthreads(maxthreads)

    def ldap_config_delete_smtpauth(self, server_name):
        """Delete the LDAP SMTPAuth query.

        *Parameters:*
        - `server_name`: name of the server configuration to edit. Mandatory.

        *Examples:*
        | LDAP Config Delete SMTPAuth | myLDAP |
        """
        self._cli.ldapconfig().edit(server_name).smtpauth().delete()
