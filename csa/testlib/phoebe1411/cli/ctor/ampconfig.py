# $Id: //prod/main/sarf_centos/testlib/phoebe1350/cli/ctor/ampconfig.py#1 $
...
class ampconfigCASE(ccb.IafCliConfiguratorBase):
    def setup(self, input_dict=None, **kwargs):
        ...
        param_map['protection_confirm_disable']      = ['The system will no longer check messages for malwares. Are you sure you want to disable?',YES]
        param_map['file_analysis_confirm_disable']   = ['The system will no longer run file analysis for malware scan. Are you sure you want to disable?',YES]
            ...
            if kwargs['modify_filetypes'].lower() == 'no':
                ...
                if 'modify_filetypes' in kwargs and is_yes(kwargs['modify_filetypes']):
                    ...
                    for group in file_groups:
                        ...
                        self._writeln('')
                self._query_response(kwargs['processing_timeout'])
                self._to_the_top(1)
    ...
    def advanced(self, input_dict=None, **kwargs):
        ...
        param_map1['edit_passphrase']             = ['passphrase', YES]
        param_map1['password']                    = ['enter new passphrase', DEFAULT]
        ...
        if 'Choose a file analysis server' in lines:
            ...
            if kwargs['file_analysis_server_select'] == 'Private analysis cloud':
            ...
            self._expect(['Use Existing File Reputation Proxy'])
            if kwargs.has_key('use_existing_reputation_proxy'):
                if self._expectindex == 0:
                    self._query_response(kwargs['use_existing_reputation_proxy'])
                    if kwargs['use_existing_reputation_proxy'].lower() == 'no':
                        self._expect(['Do you want to change proxy detail'])
                        if self._expectindex == 0:
                            self._query_response(kwargs['change_proxy_settings'])
                            if kwargs['change_proxy_settings'].lower() == 'yes':
                                self._query_response(kwargs['file_analysis_server_detail'])
                                self._query_response(kwargs['file_analysis_proxy_port'])
                                self._query_response(kwargs['file_analysis_proxy_username'])
                                self._expect(['Edit passphrase'])
                                self._query_response(kwargs['edit_password'])
                                if kwargs['edit_password'].lower() == 'yes':
                                    self._expect(['Please enter new passphrase'])
                                    if self._expectindex == 0:
                                        self._writeln(kwargs['file_analysis_proxy_password'])
                                        self._to_the_top(1)
                            else:
                                self._to_the_top(1)
                    else:
    ...
    def setup_print(self, input_dict=None, **kwargs):
