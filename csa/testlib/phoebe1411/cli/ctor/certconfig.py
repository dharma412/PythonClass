...
class certconfig(ccb.IafCliConfiguratorBase):
    ...
    def default_warning(self):
        ...
        Note : If ESA in Cluster and if mode is set to machine or group,
        we might have to select from list of options below

        What would you like to do?
            1. Switch modes to edit at mode "Cluster mode".
            2. Start a new, empty configuration at the current mode.
            3. Copy settings from another cluster mode to the current mode.
        """
        self._expect(['What would you like to do', 'You are currently using a demonstration certificate'], timeout=15)
        if self._expectindex == 0:
            self._query_response('2')
            self._expect(['You are currently using a demonstration certificate', ], timeout=15)
            self._writeln()
...
class certconfigCertificate(ccb.IafCliConfiguratorBase):
    ...
    def new(self, input_dict=None, **kwargs):
        ...
        param_map['fqdn_validation'] = ['check if Common Name is in Fully Qualified', DEFAULT]
        ...
        param_map['another_email'] = ['Add another member', DEFAULT, NO]
        ...
        param_map['fqdn'] = ['Do you want to check if Common Name is in Fully Qualified Domain Name(FQDN) format', DEFAULT]
    ...
    def import_cert(self, cert_name, filename, password, fqdn_validation=None):
        """
        ...
        if fqdn_validation:
            self._expect(["Do you want to check if Common Name",], timeout=5)
            self._query_response(fqdn_validation)
        else:
            self._expect(["Do you want to check if Common Name", ], timeout=5)
            self._query_response('N')
        self._to_the_top(2)

    def paste(self, name, cert, key, cert_overwrite=False, inter_cert_list=None,
              remove_cert_list=None, fqdn_validation=None):
        """
        ...
        self._add_intermediate_cert(inter_cert_list,remove_cert_list, fqdn_validation)

    def _add_intermediate_cert(self, inter_cert_list=None, remove_cert_list=None, fqdn_validation=None):
        if inter_cert_list:
        ...
        if fqdn_validation:
            self._expect(
                [("check if Common Name is in Fully Qualified", EXACT)], timeout=5)
            self._query_response(fqdn_validation)
        else:
            if self._query("check if Common Name is in Fully Qualified", timeout=10):
                self._query_response('N')
    ...
    def export_cert(self, cert_name, filename, password):
        ...
        self._query_select_list_item(cert_name)
        self._query_response(filename)
        self._query('Create a passphrase for file')
        self._writeln(password)
        self._query('Confirm the passphrase for file')
        self._writeln(password)
        self._to_the_top(2)
...
class certconfigCustom(ccb.IafCliConfiguratorBase):
    ...
    def import_cert(self, input_dict=None, **kwargs):
        """
          certconfig -> CERTAUTHORITY -> CUSTOM -> IMPORT
        """
        param_map = ccb.IafCliParamMap(end_of_command='Choose the operation you want to perform')
        param_map['filename'] = ['Enter the name of the file on machine', ccb.REQUIRED]
        param_map['check_format'] = ['check if Common Name or SAN:dNSName or both are in Fully Qualified Domain Name(FQDN) format', NO] 
        param_map.update(input_dict or kwargs)
        self._query_response('IMPORT')
        self._process_input(param_map)
        self._to_the_top(1)

    def export_cert(self,filename, export_all_certificates=DEFAULT, export_cert_name=None):
        """
          certconfig -> CERTAUTHORITY -> CUSTOM -> EXPORT
        """
        self._query_response('EXPORT')
        self._query_response(export_all_certificates)
        if not is_yes(export_all_certificates):
            self._query_select_list_item(export_cert_name)
        self._query_response(filename)
        self._to_the_top(3)
    ...
    def delete(self, input_dict=None, **kwargs):
        ...
        """
        param_map = IafCliParamMap(end_of_command='Choose the operation')
        param_map['cert'] = ['custom ca certificate you wish to delete',DEFAULT,1]
        param_map['confirm'] = ['you want to delete',DEFAULT]
        param_map.update(input_dict or kwargs)
        self._query_response('DELETE')
        return self._process_input(param_map)
