...
class systemsetup(clictorbase.IafCliConfiguratorBase):
    ...
    def configure_interface(self, input_dict=None, **kwargs):
        ...
        #Need to resolve Data 4/5 interface for C690 Models
        self.param_map['data4_enable'] = \
            ['use the Ethernet interface labeled Data 4',NO]
        self.param_map['data5_enable'] = \
            ['use the Ethernet interface labeled Data 5', NO]
        self.param_map['%s_assign_second_ip' % _ethernet] = \
                 ['Would you like to assign a second IP address for the "Data 1" interface',NO]
        if kwargs.has_key('%s_assign_second_ip' % _ethernet):
            self.param_map['%s_assign_second_ip' % _ethernet] = \
                    ['Would you like to assign a second IP address for the "Data 1" interface', kwargs['%s_assign_second_ip' % _ethernet]]
            self.param_map['%s_second_nickname' % _ethernet] = ['Please create a nickname for the "Data 1"', REQUIRED]
            self.param_map['%s_second_enable_ip4' % _ethernet] = ['Would you like to configure an IPv4 address', DEFAULT]
            self.param_map['%s_second_ipv4_address' % _ethernet] = ['IPv4 Address', NO_DEFAULT]
            self.param_map['%s_second_ipv4_netmask' % _ethernet] = ['Netmask', DEFAULT]
            self.param_map['%s_second_enable_ipv6' % _ethernet] = ['Would you like to configure an IPv6 address', DEFAULT]
            self.param_map['%s_second_ipv6_address' % _ethernet] = ['IPv6 Address', REQUIRED]
            self.param_map['%s_second_ipv6_prefix' % _ethernet] = ['Prefix length', DEFAULT]
    ...
    def configure_scanning_engines(self, input_dict=None, **kwargs):
        ...
        self.param_map['share_limited_data_with_service_logs'] = \
                ['Allow the sharing of limited data with Service Logs?', DEFAULT]
        #senderbase
    ...
    def configure_time_settings(self, input_dict=None, **kwargs):
        ...
        self.param_map['ntp_authentication'] = ['enable NTP authentication?', DEFAULT]
    ...
    def __call__(self, input_dict=None, **kwargs):
        ...
        """
        #eula
        old_passphrase = Misc(None, None).get_admin_password(self._hostname)
