#!/usr/bin/env python
# $Id: //prod/main/sarf_centos/testlib/phoebe1350/cli/ctor/diagnostic.py#1 $
...
class diagnostic(clictorbase.IafCliConfiguratorBase):
    ...
    def __call__(self, dut=None, dut_version=None):
    ...
    def reload(self, sure=NO, really_sure=NO, wipe=NO, timeout=900):
        ...
        if is_yes(sure):
            self._query('Are you *really* sure you want to continue')
            self._writeln(really_sure)
            if is_yes(really_sure):
                self._query('Do you want to wipe also')
                self._writeln(wipe)
            SysTools(self.dut, self.dut_version).wait_until_dut_reboots(timeout,wait_for_ports="22,80")
        else:
...
class diagnosticNetwork(clictorbase.IafCliConfiguratorBase):
    ...
    def ndpshow(self):
        ...
        self._query_response('ndpshow')
        self._expect('\n')
        raw = self._read_until('Choose the operation')
        self._restart()
        return raw

    def smtpping(self, hostname_or_ip = 'aol.com', network_interface = "1",
...
class diagnosticReporting(clictorbase.IafCliConfiguratorBase):
    ...
    def deletedb(self, confirm=NO, timeout=300):
        """Reinitialize the reporting database"""
        self._query_response('DELETEDB')
        self._query_response(confirm)
        curr_status = self._read_until("Choose the operation", timeout)
        self._to_the_top(self.newlines)
        return  curr_status
...
class networkTcpdump(clictorbase.IafCliConfiguratorBase):
    ...
    def stop(self):
        ...
        pcap_dict = {}
        pcap_dict['file_name'] = \
            re.search(r'File Name:(\s+|\n)?(.*)\r\n', \
                curr_status, re.M|re.I).group(2)
        pcap_dict['file_size'] = \
            re.search(r'File Size:(\s+|\n)?(.*)\r\n', \
                curr_status, re.M|re.I).group(2)
        pcap_dict['duration'] = \
            re.search(r'Duration:(\s+|\n)?(.*)\r\n', \
                curr_status, re.M|re.I).group(2)
        pcap_dict['limit'] = \
            re.search(r'Limit:(\s+|\n)?(.*)\r\n', \
                curr_status, re.M|re.I).group(2)
        pcap_dict['interface'] = \
            re.search(r'Interface\(s\):(\s+|\n)?(.*)\r\n', \
                curr_status, re.M|re.I).group(2)
        pcap_dict['filter'] = \
            re.search(r'Filter:(\s+|\n)?(.*)\r', \
                curr_status, re.M|re.I).group(2)
