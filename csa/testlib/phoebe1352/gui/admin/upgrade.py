...
# $Id: //prod/main/sarf_centos/testlib/phoebe1350/gui/admin/upgrade.py#2 $ $DateTime: 2020/01/09 22:29:40 $ $Author: saurgup5 $
...
class SystemUpgrade(GuiCommon):
    ...
    @go_to_page(PAGE_PATH)
    @check_for_pending_changes
    def system_upgrade_download(self, **kwargs):
        ...
        *Examples:*
        ...
        """
        self.click_button(UPGRADE_OPTIONS_BUTTON)
        self._continue_or_cancel_upgrade(kwargs)
        if self._continue_upgrade:
            controller = self._get_cached_controller(SystemUpgradeWizard)
            settings = {UPGRADE_FILE_KEY: kwargs['dest_version'],
