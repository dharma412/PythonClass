...
class Misc(BackdoorInterface, UtilCommon, Misc):
    ...
    def heimdall_rollover_log(self, log_name):
        time_stamp = datetime.datetime.fromtimestamp(time.time()).strftime('%Y%m%dT%H%M%S')
        source_file = "/data/log/heimdall/%s/%s.current" % (log_name, log_name)
        backup_file = "/data/log/heimdall/%s/%s.@%s.s" % (log_name, log_name, time_stamp)
        self.run_on_dut("cp %s %s" % (source_file, backup_file))
        self.run_on_dut("echo '' > %s" % source_file)
        self._info('Successfully rolled over %s logs')
