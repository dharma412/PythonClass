...
jiralist = {}
jiralist["external_view_failures"] = {}
jiralist["internal_view_failures"] = {}
...
def get_dns_records(id, dh=None):
    """
    ...
    dnsrecord_text = fetch_dnsfile(EXTERNAL_FILE)
    ...
    #regex = ".*%s.* |%s*-.*|\d{1,3}.+ PTR [esa|sma]{0,9}.+%s.*|%s.[a-zA-Z0-9.]+com. 3600 IN MX 5 mx[1-2]\.|esa.%s.[a-zA-Z0-9.]+com. IN TXT \"v=spf1 ip4:[0-9].+\s-all\""%
...
def perform_validation(clsid, status_list, failed_clus_list, skipped_clus_list):
    ...
    dns_recordsList_dedicated, alloc_name  = get_dns_records(clsid, dh=getdhurl)
    logging.info("[Start Validation]: Processing Cluster : {}".format(alloc_name))
    ...
    c = Cluster.objects.get(name=alloc_name)
    config_obj = Configuration(c.bundle.id)
    pov_mode_enabled = config_obj.get_all().get('pov_mode_enabled')
    if pov_mode_enabled:
        logging.info("8: Check Journal Records for Dedicated Customer :{}".format(alloc_name))
        journalvalidate = re.findall("{}.[a-zA-Z0-9.]+com. 3600 IN MX 5".format(alloc_name), str(dns_recordsList_dedicated))
        print(journalvalidate)
...
def wrapperforlbvalidation(clsid, statuslist, failedlist, skippedlist):
    perform_lbvalidation(clsid, statuslist, failedlist, skippedlist)
    #logging.info("------------------------------------------------")
    #logging.info("Dedicated Cluster Validation")
    #logging.info("Success List :{} \n{}".format(len(statuslist), statuslist))
    #logging.info("Failed List :{} \n{}".format(len(failedlist), failedlist))
...
def lb_customer_verification():
    ...
    for i in splitlist:
        for clsid in i:
            process = multiprocessing.Process(target=wrapperforlbvalidation, args=(clsid, statuslist, failedlist, skippedlist))
            processes.append(process)
            process.start()
...
def perform_lbvalidation(clsid, status_list_lb, failed_clus_list_lb, skippedlist):
    ...
    getdhurl = getdhurl.split("-")[0]
    dns_records_lb, alloc_name = get_dns_records(clsid, dh=getdhurl)
...
def lb_customer_internal_verification():
    ...
    Retuns    : None
    """
    lb_ip4_server, failed_lb_servers, lb_ip6_server, duplicate_servers  = [], [], [], []
...
def dedicate_customer_internal_verification():
    """
    Purpose   : Validate all the internal records of the dedicated customers.
    """
    dedicated_ip4_servers, dedicated_ip6_servers, failed_dedicated_servers  = [], [], []
    dns_internal_records = fetch_dnsfile(INTERNAL_FILE)
...
def main():
    ...
    logging.info("****************************************************************")
    logging.info("External View Validation")
    logging.info("Dedicated Customers Validation starts ...")
    #jiralist["external_view_failures"] = dedicated_customer_verification()
    logging.info("Dedicated Customers Validation Ends ...")
    logging.info("\n\n\n")
    logging.info("****************************************************************")

    logging.info("LB Customers Validation Start...s")
    logging.info("------------------------")
    jiralist["external_view_failures"] = lb_customer_verification()
    logging.info("LB Customers Validation Ends ...")

    logging.info("****************************************************************")
    logging.info("Internal View Validation Starts ...")
    logging.info("Dedicated Customer")
    #jiralist["internal_view_failures"] = dedicate_customer_internal_verification()
    logging.info("Internal View Lb customer lb validation")
    #jiralist["internal_view_failures"] = lb_customer_internal_verification()
    logging.info("Internal View Validation Ends ...")
    end = time()
    logging.info("DNS Validation End Tmetaken:%2f" %(end-start))

    '''
    jiralist["external_view_failed"]["abc"] = "Error:MX Record Validation Failed"
    jiralist["external_view_failed"]["xyz"] = "Error:MX Record Validation Failed"
    jiralist["internal_view_failed"]["abc"] = "Error:MX Record Validation Failed"
    raise_jira_ticket(jiralist)
    '''
